...
int ObjectCatalogue::addObjects(const DynamicObjectCollection& objects)
{
    ...
    if((objects.timestamp() < lastUpdateTime_)
        || (objects.timestamp() - lastUpdateTime_ > maxForwardJumpUs_))
    {
        tracks_.clear();
...
{
    return utils::contains_if(tracks_, [id](const ObjectTrack& t) { return t.id() == id; });
}


boost::optional<ObjectTrack> ObjectCatalogue::trackForObject(ObjectId id)
{
    auto trackIt = std::find_if(tracks_.begin(), tracks_.end(), [id](const ObjectTrack& t) { return t.id() == id; });
