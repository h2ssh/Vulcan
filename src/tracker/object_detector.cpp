...
struct AdjGraphVertex
{
    ...
    AdjGraphVertex(const LaserObject* obj) : object(obj) { }
};
...
ObjectDetector::ObjectDetector(const object_detector_params_t& params) : params_(params)
{
...
void ObjectDetector::identifyDynamicRays(const laser::MovingLaserScan& scan, const hssh::LocalPerceptualMap& lpm)
{
    ...
    for (std::size_t n = 0; n < scan.size(); ++n) {
        ...
        bool cellIsProbablyFree =
          lpm.getCost(gridPoint) < 127;   // if cost is less than "0", probably free, so just assume it is for now
...
bool ObjectDetector::doesRayIntersectGlass(const laser::adjusted_ray_t& ray, const hssh::LocalPerceptualMap& lpm)
{
    ...
    if (params_.filterGlassRays) {
        auto rayIntersection =
          utils::trace_ray_until_condition(utils::global_point_to_grid_point(ray.position, lpm),
...
LaserObjectCollection ObjectDetector::mergeCollections(const std::vector<LaserObjectCollection>& collections)
{
    ...
    return LaserObjectCollection(std::move(finalObjects), 3, collections.front().timestamp());
}
