...
FixedObjectModel::FixedObjectModel(void) : type_(Type::invalid), movingIndex_(-1)
{
}


FixedObjectModel::FixedObjectModel(const ObjectBoundary& boundary) : FixedObjectModel()
{
...
ObjectBoundary FixedObjectModel::createBoundaryFromEndpoints(void) const
{
    ...
    float length = isPivoting() ? estimatedArc().radius() : distance_between_points(movingPosition(), fixedPosition());
...
std::size_t count_endpoints_of_type(const Models& endpoints, EndpointType type)
{
    return std::count_if(endpoints.begin(), endpoints.end(), [type](const EndpointModel& model) {
