...
public:
    ...
    void setGoals(const ObjectGoalDistribution& goals) override { goals_ = goals; }

    float overlapWithObject(const LaserObject& object) const override { return 0.0f; }

    int64_t timeLastSeen(void) const override { return timestamp_; }

    int64_t totalTimeSeen(void) const override { return totalTime_; }

    Position position(void) const override { return motion_.position(); }

    velocity_t velocity(void) const override { return motion_.velocity(); }

    object_motion_state_t motionState(void) const override { return motion_.motion(); }

    ObjectBoundary boundary(void) const override { return boundary_; }

    double radius(void) const override { return boundary_.circleApproximation().radius(); }

    ObjectGoalDistribution goals(void) const override { return goals_; }
...
private:
    ...
    {
        ar(timestamp_, totalTime_, motion_, boundary_, goals_);
    }
};
