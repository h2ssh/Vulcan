...
tracking_filter_params_t::tracking_filter_params_t(void) : saveMeasurementData(false)
{
}
...
, params_(params)
{
    ...
    // velocity is not directly observable until the next observation. States are set to zero with high initial
    // uncertainty so that it can quickly corrected during the filtering process.
...
MultivariateGaussian
  LinearMotionTracker::uncertainSeqDeriv(const utils::FixedDurationBuffer<uncertain_measurement_t>& data) const
{
...
void LinearMotionTracker::detectStopsAndTurns(void)
{
    ...
    //     double objectSpeedInFastFilter = sqrt(fastMean[velXIndex]*fastMean[velXIndex] +
    //     fastMean[velYIndex]*fastMean[velYIndex]); if(objectSpeedInFastFilter < 0.2)
    //     {
    ...
    //
    //     // positionState contains predictions from fast filter, and the velocity contains prediction from slow
    //     filter.
    ...
    //
    //     double objectSpeedInFastFilter = sqrt(fastMean[velXIndex]*fastMean[velXIndex] +
    //     fastMean[velYIndex]*fastMean[velYIndex]); double objectSpeedInSlowFilter =
    //     sqrt(slowMean[velXIndex]*slowMean[velXIndex] + slowMean[velYIndex]*slowMean[velYIndex]);
    //
    ...
    //             double objectHeadingDifference = angle_diff_abs(objectHeadingInFastFilter,
    //             objectHeadingInSlowFilter);
    //
...
void LinearMotionTracker::saveMeasurementsIfRequested(int64_t timestamp, const MultivariateGaussian& measurement)
{
    ...
    measurementsOut_ << timestamp << ' ' << measurement[velXIndex] << ' ' << measurement(velXIndex, velXIndex) << ' '
                     << measurement[velYIndex] << ' ' << measurement(velYIndex, velYIndex) << ' '
                     << std::sqrt((measurement[velXIndex] * measurement[velXIndex])
                                  + (measurement[velYIndex] * measurement[velYIndex]))
                     << ' ' << measurement[accelXIndex] << ' ' << measurement(accelXIndex, accelXIndex) << ' '
                     << measurement[accelYIndex] << ' ' << measurement(accelYIndex, accelYIndex) << ' '
                     << std::sqrt((measurement[accelXIndex] * measurement[accelXIndex])
                                  + (measurement[accelYIndex] * measurement[accelYIndex]))
                     << std::endl;
}
                  ...
                  MultivariateGaussian& updated)
{
    ...
    Matrix updatedCov = (arma::eye(stateSize, stateSize) - kalmanGain) * predicted.getCovariance();
