...
LocalTrajectoryRegression::LocalTrajectoryRegression(const LocalTrajectoryRegression& rhs) : LocalTrajectoryRegression()
{
    setupNLOpt();
}
...
boost::optional<velocity_t>
  LocalTrajectoryRegression::fitTrajectory(const std::deque<regression_data_t>& recentTrajectory,
...
double LocalTrajectoryRegression::computeCost(void)
{
    ...
    double regularizer = (priorWeights_.x * std::abs(prediction_.front().x - prior_.x))
      + (priorWeights_.y * std::abs(prediction_.front().y - prior_.y));
