...
{
    in >> matrix.wi00 >> matrix.wi01 >> matrix.wi02 >> matrix.wi10 >> matrix.wi11 >> matrix.wi12 >> matrix.wi20
      >> matrix.wi21 >> matrix.wi22 >> matrix.iw00 >> matrix.iw01 >> matrix.iw02 >> matrix.iw10 >> matrix.iw11
      >> matrix.iw12 >> matrix.iw20 >> matrix.iw21 >> matrix.iw22;
...
Point<float> vulcan::vision::world_to_image_coordinates(const Point<float>& world,
                                                        const homography_matrix_t& homography)
{
...
Point<float> vulcan::vision::image_to_world_coordinates(const Point<int16_t>& image,
                                                        const homography_matrix_t& homography)
{
