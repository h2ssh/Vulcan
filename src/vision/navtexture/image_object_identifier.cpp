...
{
    segmenter = create_graph_based_image_segmenter(params.identifierParams.segmenterType,
                                                   params.identifierParams.segmenterParams);
...
{
    for (auto segmentIt = finalSegments.begin(), endSegment = finalSegments.end(); segmentIt != endSegment;
         ++segmentIt) {
                                                 ...
                                                 std::vector<image_object_t>& objects)
{
    ...
    // Need to use the distorted centers for the image objects because further processing of the objects happens in the
    // distorted image, not the undistorted.
    constructImageObjectsForGroundSegments(groundPlaneIndices,
                                           dynamicIndices,
                                           adjacentIndices,
                                           distortedCenters,
                                           objects);
...
void ImageObjectIdentifier::convertLaserToPixels(const polar_laser_scan_t& scan,
                                                 int imageHeight,
                                                 std::vector<Point<int16_t>>& laserPixels)
{
    /*
     * When converting, ignore all pixels that don't fall within the y-value of the image. These laser pixels would be
     * pure noise, as the homography does not include them. Their values swing around wildly, so disregarding is the
     * best approach.
     */
    ...
    for (size_t n = 0; n < cartesian.numPoints - 1; n += 2) {
        ...
        if (cartesian.scanPoints[n].x > 0 && cartesian.scanPoints[n].y != 0
            && scan.ranges[n] < params.maxLaserDistance) {
            laserPixel = world_to_image_coordinates(cartesian.scanPoints[n], matrix);

            // Only add pixels which have a positive y and less than half the image -- meaning they could appear in the
            // view of the camera
                                                    ...
                                                    std::vector<Point<int16_t>>& undistortedCenters)
{
    ...
    for (auto segmentIt = segments.begin(), segmentEnd = segments.end(); segmentIt != segmentEnd; ++segmentIt) {
        ...
        for (auto pointIt = segmentIt->boundaryPixels.begin(), pointEnd = segmentIt->boundaryPixels.end();
             pointIt != pointEnd;
             ++pointIt) {
            sumX += pointIt->x;
            sumY += pointIt->y;
        ...
        distortedCenters.push_back(
          Point<int16_t>(sumX / segmentIt->boundaryPixels.size(), sumY / segmentIt->boundaryPixels.size()));
    }
...
math::Polygon<int16_t> ImageObjectIdentifier::calculateLaserHull(const std::vector<Point<int16_t>>& laserPixels,
                                                                 int imageWidth)
{
    /*
...
#ifdef DEBUG_GROUND
    std::cout << "INFO:ObjectIdentifier:" << planeSegmentIndices.size() << '/' << segmentCenters.size()
              << " segments on ground plane\n";
#endif
}
...
void ImageObjectIdentifier::matchDynamicPointsToSegments(const laser::dynamic_laser_points_t& points,
...
void ImageObjectIdentifier::convertDynamicPointsToImage(const laser::dynamic_laser_points_t& points,
                                                        std::vector<Point<int16_t>>& dynamic)
{
    // The dynamic points are in the global frame, so they needed to be converted back to the robot frame, where all
    // calculations for the plane analysis occur
...
void ImageObjectIdentifier::findPlaneSegmentsAdjacentToDynamic(const std::vector<size_t>& planeIndices,
                                                               const std::vector<size_t>& dynamicIndices,
                                                               std::vector<size_t>& adjacentIndices)
{
    ...
    for (auto dynIndex = dynamicIndices.begin(), dynEnd = dynamicIndices.end(); dynIndex != dynEnd; ++dynIndex) {
        ...
        for (auto borderIt = dynamicSegment.boundaryPixels.begin(), borderEnd = dynamicSegment.boundaryPixels.end();
             borderIt != borderEnd;
             ++borderIt) {
                                                 ...
                                                 std::vector<size_t>& adjacentIndices)
{
    ...
    // Check to see if the adjacentId exists and has not already been added to the segments. Saves serious processing
    // time as opposed to looking through all the plane indices over and over again
    if (adjacentId == segmentId
        || std::find(adjacentSegmentIds.begin(), adjacentSegmentIds.end(), adjacentId) != adjacentSegmentIds.end()) {
        return;
    }
...
void ImageObjectIdentifier::constructImageObjectsForGroundSegments(const std::vector<size_t>& planeIndices,
                                                                   ...
                                                                   std::vector<image_object_t>& objects)
{
    // Need to establish the colors, centers, and agent properties of an object. The feature descriptor is handled
    // elsewhere.
    ...
    for (size_t n = 0; n < finalSegments.size(); ++n) {
        ...
        objects[n].groundPlane =
          (std::find(planeIndices.begin(), planeIndices.end(), n) != planeIndices.end()) ? 1 : -1;
        objects[n].hasAgent = (std::find(agentIndices.begin(), agentIndices.end(), n) != agentIndices.end()) ? -1 : 0;

        if (objects[n].hasAgent != -1) {
            objects[n].hasAgent =
              (std::find(adjacentIndices.begin(), adjacentIndices.end(), n) != adjacentIndices.end()) ? 1 : 0;
        }
    }
}


void ImageObjectIdentifier::calculateFeatureDescriptorsForGroundSegments(const Image& image,
                                                                         std::vector<image_object_t>& objects)
{
    assert(objects.size() == finalSegments.size());
    ...
    for (size_t n = 0; n < finalSegments.size(); ++n) {
        objects[n].histogram = PixelHistogram::createPixelHistogram(params.histogramType,
                                                                    image,
                                                                    finalSegments[n].pixels,
