...
struct cluster_t
{
    ...
    cluster_t(const cluster_t& toCopy) : segmentIndices(toCopy.segmentIndices)
    {
...
bool assign_clusters(const std::vector<image_segment_t>& segments,
                     const std::vector<cluster_t>& oldClusters,
                     std::vector<cluster_t>& newClusters);
...
void merge_segments(std::vector<image_segment_t>& segments,
...
void recursive_merge_helper(int indexToCheck,
                            ...
                            utils::DisjointSetForest& components,
                            const Image& image);
void find_adjacent_segments(const image_segment_t& segment,
                            const utils::DisjointSetForest& components,
                            std::vector<unsigned int>& adjacent,
                            const Image& image);
int segment_is_in_cluster(unsigned int segmentId,
                          const std::vector<int>& indices,
                          const std::vector<image_segment_t>& segments,
                          const utils::DisjointSetForest& components);
void join_two_segments(image_segment_t& into, image_segment_t& from);
void filter_boundary_pixels(image_segment_t& segment,
                            const vulcan::Image& image,
                            const vulcan::utils::DisjointSetForest& components);
...
inline float cluster_distance(const image_segment_t& segment, const cluster_t& cluster)
{
    ...
    return pow(segment.averageColor[0] - cluster.center[0], 2) + pow(segment.averageColor[1] - cluster.center[1], 2)
                                            ...
                                            std::vector<image_segment_t>& finalSegments)
{
    ...
    for (auto clusterIt = clusters.begin(), endIt = clusters.end(); clusterIt != endIt; ++clusterIt) {
        for (auto indexIt = clusterIt->segmentIndices.begin(), indexEnd = clusterIt->segmentIndices.end();
             indexIt != indexEnd;
             ++indexIt) {
...
void run_k_means(const std::vector<image_segment_t>& segments, int maxIterations, std::vector<cluster_t>& clusters)
{
    ...
    for (auto clusterIt = clusters.begin(), endIt = clusters.end(); clusterIt != endIt; ++clusterIt) {
        std::cout << "Size:" << clusterIt->segmentIndices.size() << " Means:(" << clusterIt->center[0] << ','
                  << clusterIt->center[1] << ',' << clusterIt->center[2] << ")\n";
    }
#endif
}
                               ...
                               const Image& image)
{
    ...
    for (auto clusterIt = clusterIndices.begin(), endIt = clusterIndices.end(); clusterIt != endIt; ++clusterIt) {
        ...
        for (auto adjacentIt = clusterIt + 1; adjacentIt != endIt; ++adjacentIt) {
            if (!segments[*adjacentIt].pixels.empty()
                && components.findSet(segments[*adjacentIt].segmentId) == activeId) {
                                          ...
                                          const Image& image)
{
    ...
    for (auto borderIt = segment.boundaryPixels.begin(), endIt = segment.boundaryPixels.end(); borderIt != endIt;
         ++borderIt) {
                            ...
                            const Image& image)
{
    ...
    // Only merge the cluster if the IDs are different, the segment is in the cluster, and the segment isn't already
    // going to be merged
