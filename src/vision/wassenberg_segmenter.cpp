...
void WassenbergSegmenter::calculateComponentsCredit(void)
{
    ...
    #ifdef DEBUG_SEGMENTER
    std::cout<<"INFO:Wassenberg: Valid components:"<<validComponents<<" Average credit:"<<(creditTotal/validComponents)<<'\n';
    #endif
}
...
float WassenbergSegmenter::findPotentialMergeCredit(int componentA, int componentB)
{
    ...
    if((components[componentA].size > 1) &&
        (components[componentB].size > 1))
    {
