...
void RGBColorHistogram::calculateHistogram(const Image&                     image,
                                           const std::vector<Point<int16_t>>& pixels)
{
    ...
    for(auto pixelIt = pixels.begin(), endIt = pixels.end(); pixelIt != endIt; ++pixelIt)
    {
        ...
        {
            std::cerr<<"ERROR:RGBHistogram:Pixel out of bounds:RGB("<<(int)rValue<<','<<(int)gValue<<','<<(int)bValue<<")->("<<row<<','<<column<<','<<slice<<")\n";
            assert(false);
        }
...
OpponentColorHistogram::OpponentColorHistogram(const std::vector<float>& values, const std::vector<uint16_t>& dimensions) :
                                            PixelHistogram(dimensions, values)
{
}
...
void OpponentColorHistogram::calculateHistogram(const Image&                     image,
                                                const std::vector<Point<int16_t>>& pixels)
{
    int rowStride   = dimensions[0];
    int sliceStride = dimensions[0] * dimensions[1];
    ...
    for(auto pixelIt = pixels.begin(), endIt = pixels.end(); pixelIt != endIt; ++pixelIt)
    {
        ...
        if(row+column+slice > values.size())
        {
            std::cerr<<"ERROR:OpponentColorHistogram:Pixel out of bounds:RGB("<<(int)rValue<<','<<(int)gValue<<','<<(int)bValue<<")->("<<row<<','<<column<<','<<slice<<")\n";
            assert(false);
        }
...
SimpleColorConstancyHistogram::SimpleColorConstancyHistogram(const std::vector<float>& values, const std::vector<uint16_t>& dimensions) :
                                                        PixelHistogram(dimensions, values)
{
}
...
void SimpleColorConstancyHistogram::calculateHistogram(const Image&                     image,
                                                       const std::vector<Point<int16_t>>& pixels)
{
    int rowStride = dimensions[0];
