...
void display_help_if_needed(const utils::CommandLine& commandLine)
{
    bool needHelp = commandLine.argumentExists(HELP_SHORT) || commandLine.argumentExists(HELP_LONG)
      || !commandLine.argumentExists(POINTS_FILE) || !commandLine.argumentExists(CALIBRATION_FILE)
...
void calculate_world_to_camera_homography(const homography_points_t& points, vision::homography_matrix_t& matrix)
{
    ...
    for (size_t n = 0; n < points.numPoints; ++n) {
        ...
        std::cout << "(" << imagePoints[n][0] << ',' << imagePoints[n][1] << ")->(" << worldPoints[n][0] << ','
                  << worldPoints[n][1] << ")\n";
    }
...
void show_test_data_conversions(const homography_points_t& points, const vision::homography_matrix_t& matrix)
{
    // Idea is to do a conversion of the test points to the other frame to get a feel for how bad or good the conversion
    // is
