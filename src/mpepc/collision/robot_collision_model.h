...
public:
    ...
    /**
     * collisionArea calculates overlapping area between the robot body and static obstacles. 0 area means no collision,
     * with increasing value indicating more serious collision.
     *
     * \param    obstacleDistanceGrid    Map of distance to the nearest staic obstacles
     * \param    robotPose          Pose of the robot (optional)
     * \return   The distance to the closest obstacle. 0 means there is a collision.
...
/**
 * ConvexPolygonCollisionModel is a collision model that represents the robot boundary as a vector of vertices. The
 * shape is assumed to be symmetric and convex.
 */
...
/**
 * RectangleCollisionModel is a collision model that represents the robot as a rectangle. The closest distance is
 * calcuated by finding the closest distance between the object pose and each of the boundary lines for the rectangle.
 * If the object pose is inside the rectangle, that'll be a collision too (obviously!).
 */
