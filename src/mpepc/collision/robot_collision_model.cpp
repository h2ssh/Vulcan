...
float ConvexPolygonCollisionModel::distanceToObject(const pose_t& objectPose, float objectRadius) const
{
    ...
    return (minDistance - objectRadius < 0.0f) ? 0.0f
                                               : minDistance - objectRadius;   // object is assumed to be circular
}
...
float ConvexPolygonCollisionModel::closestObstacle(const ObstacleDistanceGrid& obstacleDistanceGrid) const
{
    ...
    if (distanceToObstacleFromRobotCenter <= shortestRadius_) {
        return 0.0f;   // if the distance to obstacles from the robot center is less than the shortest radius of the
                       // robot, then it is in colliision.
    }

    //     // check body cells for collision
    //     if(obstacleDistanceGrid.getObstacleDistance(robotPosition) <= longestRadius_) // no need to do this if
    //     obstacles are far away from the robot center
    //     {
    ...
    //             if(obstacleDistanceGrid.getObstacleDistance(bodyPointCell) < 0.001f) // querying cell location is
    //             fast since it does not interpolate.
    //             {
    //                 return 0.0f; // if any of the cell containing body points is an obstacle cell, then the robot is
    //                 in collision.
    //             }
    //         }
    //     }
    ...
    for (auto vertexIt = transformed_.begin(); vertexIt != transformed_.end() - 1; vertexIt++) {
               ...
               > stepSize)   // move onto next vertex if the distance left to vertex is less than step size.
        {
            ...
            if (distanceToObstacle < 0.001f) {
                return 0.0f;   // if any point on boundary is in collision then the robot is in collision so we can
                               // break early.
            }
...
float ConvexPolygonCollisionModel::collisionArea(const ObstacleDistanceGrid& obstacleDistanceGrid) const
{
    ...
    // NOTE: this is a quick and brute force solution that returns an approximate result. check how this works first and
    // elaborate later.
    for (std::size_t n = 0; n < robotBodyPoints_.size(); ++n) {
        ...
        if (obstacleDistanceGrid.getObstacleDistance(robotBodyCell) < obstacleDistanceGrid.metersPerCell()) {
            area += bodyCellSize_
              * bodyCellSize_;   // for each point that falls within an occupied cell add its area to collision area.
        }
    }

    // TODO: If this turns out to be slow then create robotBodyCore, which is a smaller boundary within a robot, and
...
RectangleCollisionModel::RectangleCollisionModel(const math::Rectangle<float>& boundary) : boundary_(boundary)
{
}
...
float RectangleCollisionModel::distanceToObject(const pose_t& objectPose, float objectRadius) const
{
    ...
    if (minDistance - objectRadius < 0.0f) {
        std::cout << "DEBUG:RectCollision:Min:" << minDistance << " Boundary:" << transformed << " Obj:" << object
                  << " ObjRad:" << objectRadius << '\n';
    }
#endif
...
{
    return closest_obstacle(boundary_,
                            robotPose_,
                            obstacleDistanceGrid);   // TODO: This should also be changed to use transformed points...
}
...
CircleCollisionModel::CircleCollisionModel(float radius) : radius_(radius)
{
}
...
float CircleCollisionModel::collisionArea(const ObstacleDistanceGrid& obstacleDistanceGrid) const
{
    ...
    if (distanceToObstacleFromRobotCenter < radius_) {
        return radius_
          - distanceToObstacleFromRobotCenter;   // TODO: this is just a measure of how bad the collision is, and not
                                                 // the actual area. need to check if this works.
    }
