...
template <typename T>
minkowskiSupport gjk_minkowski_support_2D(const std::vector<Point<T>>& shapeA, const std::vector<Point<T>>& shapeB, const Point<T>& directionAB)
{
    ...
    support.pointA = gjk_support_point_2D(shapeA,  directionAB); // point on A that is farthest along the current directionAB
    support.pointB = gjk_support_point_2D(shapeB, -directionAB); // point on B that is farthest along the current directionAB
...
template <typename T>
double distance_between_convex_polygons(const std::vector<Point<T>>& shapeA, const std::vector<Point<T>>& shapeB, gjk_info_t* gjkInfo)
{
    ...
    while(true)
    {
        ...
        T projectedProgress = -inner_product_between_points(directionAB, testPoint-simplex.back()); // positive means progress
