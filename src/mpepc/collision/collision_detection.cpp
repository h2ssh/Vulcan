...
std::vector<Point<double>>
  calculate_model_vertices(const math::Rectangle<float>& model, const pose_t& pose, const Grid& grid);
...
#ifdef DEBUG_EVENTS
    if (numCollisions) {
        std::cout << "DEBUG:detect_collision: Robot vertices:" << vertices[0] << ' ' << vertices[1] << ' '
                  << vertices[2] << ' ' << vertices[3] << '\n';
        std::cout << "Events:\n";
        for (auto eventIt = events.begin(); eventIt != events.end(); ++eventIt) {
            std::cout << "y:" << eventIt->yStart << "->" << eventIt->yEnd << "x:" << eventIt->xLeft << "->"
                      << eventIt->xRight << " xLeftStep:" << eventIt->xLeftStep << " xRightStep:" << eventIt->xRightStep
                      << '\n';
        }
    }
#endif
...
std::vector<vertex_event_t> create_vertex_events(const std::vector<Point<double>>& vertices)
{
    ...
    if (leftEvent.y < rightEvent.y) {
        ...
        events[2].xRight =
          rightEvent.x;   // events[1].xRight + events[1].xRightStep * (events[1].yEnd-events[1].yStart);
        ...
        events[1].xRight =
          rightEvent.x;   // events[0].xRight + events[0].xRightStep * (events[0].yEnd-events[0].yStart);
    ...
    if (events[0].xLeft > events[0].xRight || events[1].xLeft > events[1].xRight
        || events[2].xLeft > events[2].xRight) {
        std::cout << "DEBUG:detect_collision: Robot vertices:" << vertices[0] << ' ' << vertices[1] << ' '
                  << vertices[2] << ' ' << vertices[3] << '\n';
        std::cout << "Events:\n";
        for (auto eventIt = events.begin(); eventIt != events.end(); ++eventIt) {
...
bool event_has_collision(const vertex_event_t& event, const hssh::LocalPerceptualMap& grid)
{
    ...
    for (int y = event.yStart; y <= event.yEnd; ++y) {
        ...
        if (!(xLeft <= xRight || fabs(xLeft - xRight) < 0.001)) {
            std::cerr << "ERROR: collision_detection:" << xLeft << ' ' << xRight << " Step:" << event.xLeftStep << ','
                      << event.xRightStep << "y:" << y << " yEnd:" << event.yEnd << '\n';
