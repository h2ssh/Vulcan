 ...
 *   theta : angular difference between the line-of-sight line from robot to waypoint position and the waypoint target
 * orientation delta : angular difference between the line-of-sight line from robot to waypoint position and the robot
 * orientation
 */
struct graceful_motion_coordinates_t
{
    ...
    graceful_motion_coordinates_t(float r = 0, float theta = 0, float delta = 0) : r(r), theta(theta), delta(delta) { }
...
/**
 * graceful_motion_output_t defines the output of an application of the control law. The angular and linear velocity and
 * the kappa term relating the two quantities are available.
 *
 * If some other method for calculating the linear velocity is desired, the kappa value is independent of the actual
 * velocity, and the relation between linear and angular velocity is: angular = kappa * linear.
 */
...
/**
 * load_graceful_motion_params loads the parameters for the graceful motion control law from the provided configuration
 * file.
 *
...
public:
     ...
     */
    void
      setTargetPose(const pose_t& pose, graceful_motion_direction_t direction, const graceful_motion_params_t& params);
     ...
     * \return   Velocities to send to the robot
     */
    graceful_motion_output_t
      apply(const motion_state_t& currentState, float timestep, const graceful_motion_params_t& params);
