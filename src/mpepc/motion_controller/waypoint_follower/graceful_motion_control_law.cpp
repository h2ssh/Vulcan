...
void calculate_converged_radius_and_angle_velocities(graceful_motion_state_t& state,
                                                     const graceful_motion_params_t& params);
void calculate_converged_radius_velocities(graceful_motion_state_t& state,
                                           float deltaTheta,
                                           const graceful_motion_params_t& params);
...
void GracefulMotionControlLaw::setTargetPose(const pose_t& pose,
...
#ifdef DEBUG_TARGET
    std::cout << "DEBUG:GracefulMotionControlLaw: Setting control target:" << pose << " Dir:" << direction
              << " Gain:" << params.velocityGain << '\n';
#endif
}
...
graceful_motion_output_t GracefulMotionControlLaw::apply(const motion_state_t& currentState,
                                                         float timestep,
                                                         const graceful_motion_params_t& params)
{
...
#ifdef DEBUG_COMMAND
    std::cout << "DEBUG:GracefulMotionControlLaw:current:" << currentPose << " target:" << target << " command:("
              << output.linearVelocity << ',' << output.angularVelocity << ',' << output.kappa << ")\n";
#endif
...
bool GracefulMotionControlLaw::haveReachedTarget(const pose_t& currentPose)
{
    ...
    // numbers. To ensure the robot at least reaches the target, it'll overshoot a bit rather than not reach the target
    // at all.
    ...
    if (!hasAngleConverged) {
        hasAngleConverged = (std::abs(lastAngleError) < params.convergenceAngle)
          && (std::abs(deltaTheta) < params.convergenceAngle) && (lastAngleError * deltaTheta < 0);
    ...
    // the robot can't quite reach a given target for some reason, thereby avoiding chatter with the brakes turning
    // on/off
                                           ...
                                           const graceful_motion_params_t& params)
{
    ...
    // Use a proportional control where the maximum error is M_PI and decrease linearly until hitting the min velocity
    // at 0 error because the convergence doesn't happen at 0, the motion shouldn't creep to a halt as the error gets
    // smaller
    float thetaError = std::abs(deltaTheta) / M_PI;
    float proportionalAngularVelocity =
      thetaError * (params.angularVelocityAtTarget - params.minAngularVelocityAtTarget)
      + params.minAngularVelocityAtTarget;
...
void calculate_kappa(graceful_motion_state_t& state, const graceful_motion_params_t& params)
{
     ...
     *
     * where k1 and k2 are parameters that adjust the balance between position and orientation following in the
     * controller.
     */
...
void calculate_graceful_linear_velocity(graceful_motion_state_t& state, const graceful_motion_params_t& params)
{
     ...
     *
     * where v_max is a system parameter and beta and lambda adjust the slowdown of the controller as it approaches its
     * target.
     */
...
void adjust_velocities_to_limit_accelerations(graceful_motion_state_t& state, const graceful_motion_params_t& params)
{
    ...
    // Multiplying by the timestep yields the maximum change in velocity for the timestep over which this command will
    // be active
    float linearDecreaseRatio = calculate_velocity_decrease_ratio(state.linearVelocity,
                                                                  state.currentAngularVelocity,
                                                                  params.maxLinearAcceleration * state.timestep);
    float angularDecreaseRatio = calculate_velocity_decrease_ratio(state.angularVelocity,
                                                                   state.currentAngularVelocity,
                                                                   params.maxAngularAcceleration * state.timestep);
