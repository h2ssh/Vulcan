...
MotionControllerDirector::MotionControllerDirector(const utils::CommandLine& commandLine,
...
void MotionControllerDirector::handleData(const std::shared_ptr<MotionControllerTask>& task, const std::string& channel)
{
    ...
    if (isCancelled_) {
        std::cout << "Received task after being cancelled: Cancel time:" << cancelledTimestamp_
                  << " Task time:" << task->getTimestamp() << '\n';
    }
    ...
    if (task->getTimestamp()
        == 0)   // timestamp 0 only happens if the task was constructed with the default constructor.
    {
...
void MotionControllerDirector::handleData(const motion_controller_command_message_t& message,
...
void MotionControllerDirector::calculateNewCommand(void)
{
    ...
    // if the descrepancy between the timestamp in the data and the current system time is large use timestamp in the
    // data.
    ...
    if (shouldCalculateCommand()) {
        ...
        currentCommand_ =
          controllers_.calculateCommand(data);   // source and timestamp updated within the ControllerChain
    } else {
