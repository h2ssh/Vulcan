...
std::vector<dynamic_object_trajectory_t> NavigationTaskManifold::createTaskSpecificObjects(
                                                  ...
                                                  const std::vector<dynamic_object_trajectory_t>& trajectories)
{
    ...
    if(builderParams_.useLearnedNorm)
    {
        ...
        learned_norm_cost(topoRoute_,
                          builderParams_.learnedNormParams,
                          env,
                          trajectories,
                          costMap_,
                          learnedNormDebug);
...
void NavigationTaskManifold::updateStateMachine(const ObstacleDistanceGrid& map, const motion_state_t& state)
{
    ...
    if(state.timestamp != robotTrajectoryTimestamp_)
    {
        ...
        // use approach-to-goal mode only (1) if the robot is not already very close to the goal and (2) not too far away from the goal
        // and (3) if there is a clear line-of-sight between the inital point and the goal
        isApproachingGoal_ = !isVeryCloseToGoal_ && (radialDistanceToGoal < taskParams_.goalNeighborhoodRadius);
        if(isApproachingGoal_)
        {
            Line<float> lineToGoal(utils::global_point_to_grid_point(initialPose.toPoint(), map) , utils::global_point_to_grid_point(target_.toPoint(), map));
            ...
            {
                isApproachingGoal_ &= map.getObstacleDistance(cell) > builderParams_.navGridBuilderParams.robotShortRadius;;
            }
        }
        ...
        if(isUsingNavGrid_)
        {
            // use the gradient of the navigation function only if when the initial error is large. Avoid artifacts of grid computation.
            ...
            isUsingNavGradient_ = initialHeadingError_ > 0.5 && initialSpeed < 0.2 && initialDistanceToWalls > builderParams_.navGridBuilderParams.robotLongRadius + 0.02;
        }
                                                                    ...
                                                                    std::vector<float>& rewards) const
{
    ...
    for(size_t index = stride; index < trajectory.size(); index += stride)
    {
        ...
        if(isEndPointInMap)
        {
            piecewiseProgress  = piecewiseNegativeReward(trajectory[index-stride], trajectory[index], timeBetweenSamples);
        }
...
double NavigationTaskManifold::piecewiseNegativeReward(const motion_state_t& startState, const motion_state_t& endState, float timestep) const
{
    // stuff to be computed
    double negativeProgress  = 0.0; // this measures difference in the cost-to-go from the start pose to the end pose. negative if progressing.
    ...
    {
        bool isConverged = distance_between_points(endState.pose.toPoint(), target_.toPoint()) < 0.5*taskParams_.convergenceRadius;

        if(isConverged && isPoseTarget_)
        {
            isConverged = isConverged && (angle_diff_abs(endState.pose.theta, target_.theta) < 0.5*taskParams_.convergenceAngle);
        }
...
double NavigationTaskManifold::getCostToGo(const motion_state_t& state) const
{
    ...
    if(isVeryCloseToGoal_)
    {
        costToGo = distance_between_points(state.pose.toPoint(), target_.toPoint());    // ignore orientation when very close

        if(isPoseTarget_)
        {
            headingError = taskParams_.orientationHeuristicWeight * angle_diff_abs(target_.theta, state.pose.theta); // use goal orientation
        }
        else
        {
            headingError = 0.0;
...
//     bool   isGoalWithinSafeRadius = distanceToGoalAtPose <= fmin(taskParams_.goalNeighborhoodRadius, minDistanceToWallsFromTarget_);
//     bool   isUsingNonholonomicDistance = taskParams_.shouldUseNonholonomicDistNearGoal && isPoseTarget_ && isGoalWithinSafeRadius;
//
...
//     }
//
//     // progress made toward aligning to the vector field (finalError - initialError) // the more negative the better
//     float  finalHeadingError = angle_diff_abs(computeNavGradient(endPose), endPose.theta);
//     double progressToVectorField = finalHeadingError - initialHeadingError_;
//
...
double NavigationTaskManifold::computeNavGradient(const pose_t& pose) const
{
    ...
    float dy = ( navGrid_.getCostToGo(Point<int>(cell.x,   cell.y-1)) -
                    navGrid_.getCostToGo(Point<int>(cell.x,   cell.y+1)) );
    float dx = ( navGrid_.getCostToGo(Point<int>(cell.x-1, cell.y  )) -
                    navGrid_.getCostToGo(Point<int>(cell.x+1, cell.y  )) );
