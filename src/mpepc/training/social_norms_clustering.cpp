...
void cluster_eig_ratios(const std::vector<MotionVisitor>& motions)
{
    ...
    std::transform(motions.begin(), motions.end(), ratios.begin(), [](auto& m) {
        return m.eigRatio();
    });
...
MotionVisitor::MotionVisitor(const MotionObs& motions) : motions_(motions)
{
...
void MotionVisitor::visitPathSegment(const LocalPathSegment& path)
{
    ...
    std::cout << "Path vec:\n" << pathDir << " Vel vec:\n" << eigvecs_.col(1);
    std::cout << "Dot product path axis: " << arma::dot(pathDir, eigvecs_.col(1)) << '\n';
