...
struct cell_node_t
{
    ...
    cell_node_t(Point<int> cell, int32_t gridDist) : cell(cell), gridDist(gridDist) { }
        ...
        for (cell.y = 0; cell.y < static_cast<int>(lpm.getHeightInCells()); ++cell.y) {
            for (cell.x = 0; cell.x < static_cast<int>(lpm.getWidthInCells()); ++cell.x) {
                ...
                if (type & hssh::kUnsafeOccGridCell) {
                    distGrid.setGridDistNoCheck(cell,
                                                0);   // all unsafe (i.e. occupied) cells gets marked with distance 0
                    ...
                    distGrid.setGridDistNoCheck(cell, defaultGridDist_);   // all safe cells gets initialized with the
                                                                           // predetermined (large) distance value
                }
            }
        }
    }
