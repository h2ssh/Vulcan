...
struct cell_node_t
{
    ...
    cell_node_t(Point<int> cell, int32_t cost) : cell(cell), cost(cost) { }

    bool operator<(const cell_node_t& rhs) const { return cost < rhs.cost; }
...
struct NavigationGridBuilder::Impl
{
    ...
    void initializeGrid(const pose_t& goalPose, const CostMap& costMap, NavigationGrid& navGrid);
    ...
    void expandCellNode(const cell_node_t& node, const CostMap& costMap, NavigationGrid& navGrid);

    bool processCellNode(const cell_node_t& node, NavigationGrid& navGrid);
};
...
bool NavigationGridBuilder::Impl::buildGrid(const pose_t& start,
                                            ...
                                            NavigationGrid& navGrid)
{
    // No need to rebuild the grid if the costmap and target pose are the same (this is to be used in static
    // environment)
                                                 ...
                                                 NavigationGrid& navGrid)
{
    ...
    int64_t mostRecentTime =
      (goalPose.timestamp > costMap.getTimestamp()) ? goalPose.timestamp : costMap.getTimestamp();
...
void NavigationGridBuilder::Impl::setGoalPose(const pose_t& goalPose, NavigationGrid& navGrid)
{
    ...
    navGrid.setGridCostNoCheck(goalCell,
                               0);   // goal cell always has zero cost, and is the only cell that has zero cost.
}
...
bool NavigationGridBuilder::Impl::processCellNode(const cell_node_t& node, NavigationGrid& navGrid)
{
    ...
    if (navGrid.isCellInGrid(node.cell)) {
        if (node.cost < navGrid.getGridCostNoCheck(node.cell)) {
            if (node.cost < 0) {
                std::cerr << "ERROR: Cost below zero at " << node.cell << " cost:" << node.cost
                          << " Old cost:" << navGrid.getGridCostNoCheck(node.cell) << '\n';
                assert(node.cost >= 0);
            }
