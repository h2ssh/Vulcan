...
public:
    ...
    float
      getObstacleDistance(const Point<float>& position) const;   // returns obstacle distance at some metric position
...
private:
    ...
    {
        ar(cereal::base_class<utils::CellGrid<int32_t>>(this), timestamp_, id_, maxGridDist_);
    }
