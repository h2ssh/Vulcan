...
void TrajectoryEvaluator::setTaskEnvironment(const TaskManifold&                             taskManifold,
...
double TrajectoryEvaluator::evaluateTrajectory(robot_trajectory_info_t& robotTrajectory)
{
    ...
    for(size_t index = increment; index < robotTrajectory.states.size(); index += increment)
    {
        ...
        {
            // normal operation
            piecewiseStaticSurvivability = 1.0 - piecewiseCollisionProbability(robotTrajectory.states[index].pose,
        ...
        else
        {
            ...
            for(auto objectTrajectoryIt = objects_.begin(); objectTrajectoryIt != objects_.end(); ++ objectTrajectoryIt)
            {
                double objectRadius = params_.useFixedObjectRadius ? params_.fixedObjectRadius : objectTrajectoryIt->laserObject->radius();
                objectRadius = fmax(params_.fixedObjectRadius, objectRadius);

                double distanceToIndividualDynamicObj = 100.0;
                double objectCollisionProb = piecewiseCollisionProbability(robotTrajectory.states[index].pose,
                                                                            robotTrajectory.states[index].velocity,
                                                                            staticObjectUncertaintySigma_,
        ...
        if(params_.shouldConsiderOnlyTheClosestObject)
        {
            // This version means we only care about the most threatening object at each trajectory segment, either static or dynamic
        ...
        if((piecewiseStaticSurvivability < 0.001) && (robotTrajectory.staticCollisionTime < 0.0f) && (!robotTrajectory.hasStaticCollision))
        {
            robotTrajectory.hasStaticCollision  = true;
            robotTrajectory.staticCollisionTime = timeElapsed;
        }

        if((piecewiseDynamicSurvivability < 0.001) && (robotTrajectory.dynamicCollisionTime < 0.0f) && (!robotTrajectory.hasDynamicCollision))
        {
        ...
        robotTrajectory.distanceToDynamicObj.push_back(proximityToDynamicObj); // distance to the nearest dynamic object
        robotTrajectory.piecewiseSurvivability.push_back(survivability);       // survivability

#ifdef DEBUG_TRAJECTORY_EVALUATOR
        if(index == increment)
        {
            pose_t objectPose = pose_t(objectTrajectoryIt->states[index].x, objectTrajectoryIt->states[index].y, atan2(objectTrajectoryIt->states[index].yVel, objectTrajectoryIt->states[index].xVel));
            std::cerr<<"Start pose:  "      << robotTrajectory.states[index].pose
                        <<" Object pose: "     << objectPose
                        <<" Object velocity: " << objectTrajectoryIt->states[index].xVel << " , " << objectTrajectoryIt->states[index].yVel
                        <<" Object speed: "    << sqrt(objectTrajectoryIt->states[index].xVel*objectTrajectoryIt->states[index].xVel + objectTrajectoryIt->states[index].yVel*objectTrajectoryIt->states[index].yVel)
        ...
        {
            pose_t objectPose = pose_t(objectTrajectoryIt->states[index].x, objectTrajectoryIt->states[index].y, atan2(objectTrajectoryIt->states[index].yVel, objectTrajectoryIt->states[index].xVel));
            std::cerr<<"End pose:  "        << robotTrajectory.states[index].pose
                        <<" Object pose: "     << objectPose
                        <<" Object velocity: " << objectTrajectoryIt->states[index].xVel << " , " << objectTrajectoryIt->states[index].yVel
                        <<" Object speed: "    << sqrt(objectTrajectoryIt->states[index].xVel*objectTrajectoryIt->states[index].xVel + objectTrajectoryIt->states[index].yVel*objectTrajectoryIt->states[index].yVel)
                        <<" Distance-0: "      << robot_->distanceToObject(objectPose, 0)
    ...
    robotTrajectory.heuristicCost = task_->calculateHeuristicCost(robotTrajectory) * survivability; // convert to expected cost
    ...
    {
        // action cost
        actionCost = piecewiseActionCost(robotTrajectory.states[index].velocity, robotTrajectory.states[index].acceleration, params_.timeBetweenSamples);
    ...
    double totalProgress      = 0.0; // total expected progress over the entire trajectory (with additive heuristic cost, if one exists.)
    ...
    for(size_t i = 0; i < robotTrajectory.piecewiseRawProgress.size(); i++)
    {
        ...
        if(params_.useChanceConstraint) // using chance constraint ignores the survivability and the expected cost of constraint violation
        {
        ...
        {
            totalProgress      += timeDecayFactor * robotTrajectory.piecewiseSurvivability[i] * robotTrajectory.piecewiseRawProgress[i];
            totalCollisionCost += timeDecayFactor * robotTrajectory.piecewiseCollisionCost[i]; // already in expected cost form (1-p_s)*collision
            totalActionCost    += timeDecayFactor * robotTrajectory.piecewiseActionCost[i];
...
//     }
    ...
    std::cout<<"DEBUG: TrajectoryEvaluator: ExpectedProgress: "<<totalProgress<<
               ", ExpectedActionCost: "<<totalActionCost<<
               ", ExpectedCollisionCost: "<<totalCollisionCost<<'\n';
#endif
                                                          ...
                                                          double*                  distance)
{
    ...
    *distance = static_cast<double>(robot_->closestObstacle(*map_));
    ...
    double preferredClearance = params_.minStaticPassingRadius + fmax(fabs(velocities.linear)-0.2, 0.0) * kClearancePerMeter + fmax(fabs(velocities.angular)-0.3, 0.0) * kClearancePerRad;
    ...
    *distance = std::max(0.0, *distance - preferredClearance);
    ...
    double motionBasedUncertaintyStd = sqrt( (params_.linearVelocityUncertaintyStdWeight  * params_.linearVelocityUncertaintyStdWeight  * static_cast<double>(velocities.linear  * velocities.linear) ) +
                                             (params_.angularVelocityUncertaintyStdWeight * params_.angularVelocityUncertaintyStdWeight * static_cast<double>(velocities.angular * velocities.angular)) );
    ...
    *robotUncertaintySigma += motionBasedUncertaintyStd * timestep;
    ...
    double adaptiveUncertaintyStd       = (*robotUncertaintySigma > kSigmaCap) ? kSigmaCap : *robotUncertaintySigma; // capping
    double adaptiveCollisionProbability = exp( -(*distance/adaptiveUncertaintyStd)*(*distance/adaptiveUncertaintyStd) );
                                                          ...
                                                          double*                       distance)
{
    ...
    if(fabs(objectState.x - endPose.x) > kObjectIsTooFarThreshold || fabs(objectState.y - endPose.y) > kObjectIsTooFarThreshold)
    {
    ...
    *distance = robot_->distanceToObject(objectPose, objectRadius);
    ...
    double preferredClearance = params_.minDynamicPassingRadius + fmax(fabs(velocities.linear)-0.2, 0.0) * kClearancePerMeter + fmax(fabs(velocities.angular)-0.3, 0.0) * kClearancePerRad;
    ...
    *objectUncertaintySigma += motionBasedUncertaintyStd * timestep;
    ...
    // half-elliptical approximation (uncertainty bubble is an ellipse if the object is in front of the robot; otherwise it is a circle)
    ...
    else
    {
        ...
        pose_t robotPoseInObjectFrame = endPose.transformToNewFrame(objectPose); // positive x-axis lies along the direction of object linear speed
        float lineOfSightAngleInObjectFrame  = atan2(robotPoseInObjectFrame.y, robotPoseInObjectFrame.x); // is zero or +-M_PI when the robot is on the direction of object motion
...
double TrajectoryEvaluator::piecewiseActionCost(const velocity_t& velocity, const acceleration_t& acceleration, float timestep)
{
    ...
    double actionCost = ((params_.linearVelocityActionWeight  * velocity.linear  * velocity.linear)
                       ...
                       + (params_.angularAccelerationActionWeight * acceleration.angular * acceleration.angular)) * timestep;
...
double TrajectoryEvaluator::piecewiseCollisionCost(const motion_state_t& endState, float timestep)
{
    ...
    double collisionCost = (params_.baseCostOfCollision + params_.velocityWeightOfCollision * totalSpeed) * static_cast<double>(timestep);
...
double TrajectoryEvaluator::socialDistanceCost(const pose_t& endPose, const dynamic_object_state_t& objectState, float objectRadius, float timestep)
{
...
//
//     // modified object position in robot frame. This does not exactly correspond to relative position due to distanceToObject function.
//     double relativePositionXModified = distance*cos(relativeOrientation); // positive x is pointing to robot forward
//     double relativePositionYModified = distance*sin(relativeOrientation);

//     if(fabs(relativePositionYModified) < kSocialDistanceThresholdY)
//     {
//         proximityCost = std::max((kSocialDistanceThresholdX - relativePositionXModified), 0.0) / kSocialDistanceThresholdX * kMaxSocialDistanceCost * timestep;
//     }
...
void TrajectoryEvaluator::integratePathCost  (double& pathCost, const velocity_t& velocity, float timestep)
{
    ...
    pathCost += sqrt((velocity.linear * velocity.linear) + params_.angularTravelWeightForPathCost*(velocity.angular * velocity.angular))*timestep;
}
