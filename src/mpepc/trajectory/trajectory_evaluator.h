 ...
 *
 * So the expected progress is the simple progress shown above multiplied by the time-varying probability weights at
 * each segment. What does this mean? Usually, a cost function in an optimization problem is constructed as a
 * constant-weighted sum of (often conflicting) sub-objectives. Then almost invariably determination of those weights
 * becomes the real problem, as ill-chosen weights could lead to multiple local minima, unsmooth cost surface, etc,
 * which results in undesirable behaviors. But when I have tried something like that, it was very difficult to find a
 * set of constant weights that gave good results across wide range of situations.
 *
 * One of the key contribution of this formulation is that now we can compute those weights on-line, as a function of
 * states along the simulated trajectory, in such a way that it is very easy to combine different objectives and create
 * a nice, smooth cost surface that is easy to optimize over. Experimentally, the difference in robot behaviors
 * generated from the simple progress with constan weights to the expected progress with probabilistic weights was very
 * readily observable.
 *
 *
 ...
 *
 *
 * ADDED: Now we also add negative dwell time at the goal to the progress and change the name of the variable from
 * 'progress' to 'reward'. Functionally, this motivates the robot to move to the goal quickly and stay there. So what we
 * have now, looks like:
 *
...
public:
     ...
     */
    TrajectoryEvaluator(const trajectory_evaluator_params_t& params,
...
private:
    ...
    /**
     * piecewiseActionCost evaluates a trajectory segment using its velocity and acceleration, and returns the estimated
     * cost of action along the trajectory segment.
     *
     ...
     */
    double piecewiseActionCost(const velocity_t& velocity, const acceleration_t& acceleration, float timestep);

    /**
     * piecewiseCollisionCost evaluates a trajectory segment, and returns the estimated cost of collision assuming that
     * the trajectory segment is indeed in collision.
     *
    ...
    /**
     * socialDistanceCost evaluates a trajectory segment, and returns the estimated cost of staying too close to a
     * pedestrian.
     *
     * \param    endPose         Final Pose of the robot in the trajectory segment
     * \param    objectState     Position (and velocity) of the object of interest
     ...
     */
    double socialDistanceCost(const pose_t& endPose,
                              const dynamic_object_state_t& objectState,
                              float objectRadius,
                              float timestep);


    // NOTE: paths and trajectories are different! The key difference is that for the path, time is not a relevant
    // quantity,
    //       while for a trajectory it is a defining quantity. In discrete setting, a trajectory can be described as a
    //       time-stamped series of poses, but a path is a series of poses that does not require the time stamp. This
    //       distinction is important as path-related quantities are invariant under velocity transformation, e.g. path
    //       length remains constant regardless of the velocity of the travel, but the action cost explicitly depend on
    //       them.
    /**
    ...
    /**
     * integratePathCost integrates the distance-related cost of travel along the path resulting from the evaluated
     * trajectory.
     */
