...
/**
 * robot_trajectory_info_t stores an explicit trajectory of robot states, its parameterization and full evaluation
 * results.
 */
struct robot_trajectory_info_t
{
    ...
    // NOTE: The length of the commands vector will be smaller than that of the states vector by one, as the commands
    // from the final state will not be calculated.

    // trajectory parameterization via graceful kinematic control law (Park and Kuipers, ICRA 2011, Park, Johnson and
    // Kuipers, IROS 2012)
    ...
    std::vector<float> piecewiseRawProgress;     ///< Raw progress (not multiplied by the survivability) at each
                                                 ///< trajectory segement evaluated
    ...
    float totalSurvivability;   ///< Total survivability of the trajectory, i.e. the final survivability at the end pose
                                ///< of the trajectory
    ...
    float dynamicCollisionTime;   ///< Time at which a dynamic collision first occurred relative to the start time of
                                  ///< the trajectory
