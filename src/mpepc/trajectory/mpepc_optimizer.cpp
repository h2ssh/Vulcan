...
void MPEPCOptimizer::setInitialGuesses(const TaskManifold& task, const motion_target_t& previousMotionTarget)
{
    ...
    if(goalPoseInControlLawCoords.r < params_.maxRadius)
    {
        ...
        while(nominalVelocityGain < params_.maxVelocityGain)
        {
            ...
            goalPoseInOptimizerCoords.theta = 0.0; // this makes the orientation of the target pose aligne with the line of sight
            newGuesses_.push_back(goalPoseInOptimizerCoords);
        }
    }
...
motion_target_t MPEPCOptimizer::runOptimizer(trajectory_planner_debug_info_t& debugInfo)
{
    ...
    if(globalIterations > 0)
    {
        ...
        control_law_coordinates_t globalOptimizerOptimizedCoords(globalOptimizerOutputCoords.r, globalOptimizerOutputCoords.theta, globalOptimizerOutputCoords.delta);
        motion_target_t           globalOptimizerOptimizedTarget(globalOptimizerOptimizedCoords, robotPose_, globalOptimizerOutputCoords.velocityGain);
    ...
    {
        // compare the result of the global optimizer to a set of some hand-picked list of targets, which should include previous optimum and the goal,
        // and use the best as the initial contidition for the local optimizer.
        // NOTE: This step is the ONLY step that can help enforce temporal consistency between consecutive solutions, so it is crtical to have this.
        //       Also, it is important that this step happens here, as the seed to global optimizer usually does not do anything (see releavant references in NLOPT webpage.)
    ...
    if(optimizerInfo_->iteration - globalIterations > 0)
    {
        ...
        control_law_coordinates_t localOptimizerOptimizedCoords(localOptimizerOutputCoords.r, localOptimizerOutputCoords.theta, localOptimizerOutputCoords.delta);
        motion_target_t           localOptimizerOptimizedTarget(localOptimizerOptimizedCoords, robotPose_, localOptimizerOutputCoords.velocityGain);
...
double MPEPCOptimizer::getChanceConstraint(const optimizer_coords_t& coords)
{
    ...
    {
        std::cerr << "WARNING: getChanceConstraint: Detected wrong order of cost and constraint check. Assuming failed.\n";
        return -1;
    }

    double minAcceptableChance = params_.useChanceConstraint ? params_.minAcceptableChance : 0.001;

    return -(optimizerInfo_->back().totalSurvivability - minAcceptableChance); // note the negative sign. the constraint is in <= 0 form
}



void MPEPCOptimizer::evaluateGradient(const optimizer_coords_t& coords, double cost, double grad[kOptimizationDimension])
{
                                            ...
                                            double x[kOptimizationDimension])   // output
{
    ...
    if(bestIdx < optimizerInfo_->numTrajectories)
    {
        ...
        control_law_coordinates_t bestTrajCoords(robotPose_, bestTraj.motionTarget.pose, bestTraj.motionTarget.direction);
    ...
    std::cout<<"    Selected initial point for the local optimizer at ("<<x[kIdxR]<<','<<x[kIdxTheta]<<','<<x[kIdxDelta]<<','<<x[kIdxVGain]<<")\n"
             <<"    with cost "<<minCost<<" after "<< (initialGuesses.size() + defaultGuesses_.size()) <<" comparisons.\n";
...
{
    return (target.r            >= params_.minRadius)       && (target.r            <= params_.maxRadius) &&
           (target.theta        >= params_.minTheta)        && (target.theta        <= params_.maxTheta) &&
           (target.delta        >= params_.minDelta)        && (target.delta        <= params_.maxDelta) &&
