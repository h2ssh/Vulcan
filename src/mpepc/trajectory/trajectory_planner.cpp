                                                   ...
                                                   trajectory_planner_debug_info_t& debugInfo)
{
    ...
    debugInfo.trajectoryToInitialState =
      simulator_.setupForOptimization(robotState, previousMotionTarget, planReleaseTimeUs);
    ...
    // if in any case the optimizer returns an output that involves collision, manually set velocity gain to zero so the
    // robot stops.
    ...
    std::cout << "INFO: TrajectoryPlanner: \n"
              ...
              << "    Optimized motion target (x,y,theta,gain): (" << optimizedTarget.pose.x << ','
              << optimizedTarget.pose.y << ',' << optimizedTarget.pose.theta << ',' << optimizedTarget.velocityGain
              << ")\n";
    ...
    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Initial pose           : "
              << debugInfo.trajectoryToInitialState.back().pose << "\n";

    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Expected cost          : "
              << debugInfo.plannedTrajectory.expectedCost << "\n";
    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Total survivability    : "
              << debugInfo.plannedTrajectory.totalSurvivability << "\n";
    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Expected progress      : "
              << debugInfo.plannedTrajectory.expectedProgress << "\n";
    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Expected collision cost: "
              << debugInfo.plannedTrajectory.expectedCollisionCost << "\n";
    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Expected action cost   : "
              << debugInfo.plannedTrajectory.expectedActionCost << "\n";

    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Piecewise survivability values: (";
    for (auto sampleIt = debugInfo.plannedTrajectory.piecewiseSurvivability.begin(),
              sampleEnd = debugInfo.plannedTrajectory.piecewiseSurvivability.end();
         sampleIt != sampleEnd;
         sampleIt++) {
        std::cout << *sampleIt << ',';
    }
    ...
    for (auto sampleIt = debugInfo.plannedTrajectory.piecewiseRawProgress.begin(),
              sampleEnd = debugInfo.plannedTrajectory.piecewiseRawProgress.end();
         sampleIt != sampleEnd;
         sampleIt++) {
        std::cout << *sampleIt << ',';
    }
    ...
    for (auto sampleIt = debugInfo.plannedTrajectory.piecewiseCollisionCost.begin(),
              sampleEnd = debugInfo.plannedTrajectory.piecewiseCollisionCost.end();
         sampleIt != sampleEnd;
         sampleIt++) {
        std::cout << *sampleIt << ',';
    }
    ...
    for (auto sampleIt = debugInfo.plannedTrajectory.piecewiseActionCost.begin(),
              sampleEnd = debugInfo.plannedTrajectory.piecewiseActionCost.end();
         sampleIt != sampleEnd;
         sampleIt++) {
        std::cout << *sampleIt << ',';
    }
    ...
    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Clearance to static obstacles :"
              << debugInfo.clearanceToStaticObs << "\n";
    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Clearance to dynamic obstacles:" << debugInfo.clearanceToDynObs
              << "\n";

    std::cout << "DEBUG MPEPC INFO: Optimial Trajectory: Distance to static obstacles from robot center:"
              << map.getObstacleDistance(robotState.pose.toPoint()) << "\n";
#endif
    ...
    for (auto vertexIt = boundaryInGlobalFrame.begin(); vertexIt != boundaryInGlobalFrame.end() - 1; vertexIt++) {
        ...
        while (lengthToWalk
               > stepSize)   // move onto next vertex if the distance left to vertex is less than step size.
        {
            float closestObstacleFromCurrentPoint = map.getObstacleDistance(currentPoint);

            std::cout << "DEBUG: CollisionModelInTrajectoryPlanner: at point: (" << currentPoint.x << ", "
                      << currentPoint.y << "), clearance: " << closestObstacleFromCurrentPoint << "\n";
