...
/**
 * metric_planner_command_t and metric_planner_command_message_t is used to pause, resume, or cancel the planning
 * process.
 */
enum metric_planner_command_t
{
    PAUSE,    // pause the current trajectory planning process, run path planner if applicable, keep all data, stop the
              // robot.
    ...
    SET_MODE_SIMPLE_POSE_FOLLOWING,   // activate simple pose following mode, where the optimizer only optimizes the
                                      // velocity gain. The pose of the motion target is fixed at the next waypoint.
};
...
struct metric_planner_command_message_t
{
    ...
    metric_planner_command_message_t(void) : timestamp(0), command(CANCEL), intValue(0) { }
};


/**
 * metric_planner_status_t and metric_planner_status_message_t is used to report the status of the planner (e.g.
 * failure/success). useful for display purposes.
 */
...
{
    ar(message.timestamp, message.command, message.intValue);
}

template <class Archive>
void serialize(Archive& ar, metric_planner_status_message_t& message)
{
    ar(message.timestamp, message.status, message.taskId);
}
...
DEFINE_DEBUG_MESSAGE(mpepc::metric_planner_status_message_t,
                     ("MPEPC_METRIC_PLANNER_STATUS_MESSAGE"))   // this will needs to change to a system message once
                                                                // there are other modules using this status
