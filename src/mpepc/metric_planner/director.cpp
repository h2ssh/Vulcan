...
//     frontConfig.range = utils::ray_trace_range_t((-135. + 11.25) * M_PI / 180.0, std::size_t(955 / 4), M_PI / 180.0);
//
...
//     backConfig.range = utils::ray_trace_range_t((-135. + 5.0) * M_PI / 180.0, std::size_t(1040 / 4), M_PI / 180.0);
//
//     laserConfigs_.push_back(frontConfig);
...
void MetricPlannerDirector::handleData(const tracker::DynamicObjectCollection& trackedObjects, const std::string& channel)
{
...
void MetricPlannerDirector::setMetricPlannerTiming(void)
{
    ...
    std::cout<<"INFO: MetricPlanner: time since the last planner execution (ms): "<<timeSinceLastPlanReleaseUs/1000<<"\n";
    ...
    std::cout<<"DEBUG: Director: LPM timestamp     (ms): "<<lpmTimestampUs/1000    <<", relative (ms): "<<(lpmTimestampUs   - planReleaseTimeUs_)/1000 <<'\n';
    std::cout<<"DEBUG: Director: state timestamp   (ms): "<<stateTimestampUs/1000  <<", relative (ms): "<<(stateTimestampUs - planReleaseTimeUs_)/1000 <<'\n';

    if(!trackedObjectsBuffer_.read().empty())
    {
        int64_t objectTimestampUs = (*trackedObjectsBuffer_.read().begin())->timeLastSeen();
        std::cout<<"DEBUG: Director: object timestamp  (ms): "<<objectTimestampUs/1000<<", relative (ms): "<<(objectTimestampUs - planReleaseTimeUs_)/1000<<'\n';
    }
...
void MetricPlannerDirector::enforceMetricPlannerTiming(void)
{
    ...
    if(abs(timeSincePreviousOutputUs) > utils::sec_to_usec(10)) // detect if data is coming from a log by looking at the time differnce
    {
        timeSincePreviousOutputUs = timeElapsedUs; // if data is coming from a log then just use computation time for update
    }
    ...
    averagePlanningTimeUs_ = (averagePlanningTimeUs_ == 0) ? timeElapsedUs : 0.2*timeElapsedUs + 0.8*averagePlanningTimeUs_;
...
void MetricPlannerDirector::updateTaskEnvironment(void)
{
    ...
    // The grids are rebuilt at roughly at regular interval if the robot is stationary, and update more often as robot moves faster
    // (e.g. with coefficient = 1.0 and robot linear velocity at 1 m/s, the grids are updated twice as fast), and only if there has been a change in the original lpm.
    int64_t adaptiveGridRebuildIntervalUs = params_.gridRebuildIntervalUs/(1.0 + 1.0 * robotStateBuffer_.read().velocity.linear * robotStateBuffer_.read().velocity.linear);

    // attempt updating costMap if a stored lpm is loaded (lpm timestamp equals zero), or the grid has never been updated before (lastGridUpdateTimeUs equals zero),
    // or enough time has passed since the last update.
    if(lpmBuffer_.read().getTimestamp() == 0 || lastGridUpdateTimeUs_ == 0 || (utils::system_time_us() - lastGridUpdateTimeUs_) > adaptiveGridRebuildIntervalUs)
    {

#ifdef DEBUG_GRID_BUILDING
        int64_t ticUs = utils::system_time_us();
#endif
        // buildCostMap will look at the ID's and decide if the costMap should be built. If we already have a costMap generated from the given lpm (indicated by same ID)
        // then the map will not be updated.
        haveNewObstacleDistanceGrid_ = obstacleDistanceGridBuilder_.buildGrid(lpmBuffer_.read(), obstacleDistanceGrid_); // builder will not update the map if the lpm did not change
        ...
        if(haveNewObstacleDistanceGrid_)
        {
            std::cout<<"INFO: SensorDataProcessor: Costmap updated:\n"
                     <<"    LPM ID: "<<lpmBuffer_.read().getId()<<"  Costmap ID: "<<obstacleDistanceGrid_.getId()<<"  Processing time (ms): "<<(tocUs - ticUs)/1000<<'\n';
        }
#endif
...
void MetricPlannerDirector::processCommandMessages(void)
{
    ...
    std::sort(messages.begin(), messages.end(), [](const metric_planner_command_message_t& lhs,
                                                   const metric_planner_command_message_t& rhs)
             {
...
void MetricPlannerDirector::handleCommandMessage(const metric_planner_command_message_t& message)
{
    ...
    case UPDATE_WAYPOINTS:
        // TODO: Fill this in. Doing this properly will require substantial change to how the script and tasks are handled currently.
    ...
    case SET_MODE_USE_RRT_STAR:
        ...
        // params_.taskManifoldBuilderParams.navigationTaskManifoldParams.shouldBuildRRT = static_cast<bool>(message.intValue);
...
bool MetricPlannerDirector::isSafeToPlan(void)
{
    ...
    if(planReleaseTimeUs_ - robotStateBuffer_.read().timestamp > params_.plannerUpdatePeriodUs + 100000)
    {
        ...
        std::cout << "\nWARNING!: MetricPlanner: Robot state too old! : State timestamp (ms): "<< robotStateBuffer_.read().timestamp/1000 << "\n";
    }
    ...
    {
        isSafe = false;
        int64_t overTime = (planReleaseTimeUs_ - trackedObjectsBuffer_.read().timestamp() - 2 * params_.plannerUpdatePeriodUs);
        std::cout << "\nWARNING!: MetricPlanner: Tracked object state too old!: Object timestamp (ms): "<< trackedObjectsBuffer_.read().timestamp()/1000
            << " Over time by " << (overTime / 1000) << "ms\n";
    }
...
void MetricPlannerDirector::handleScript(void)
{
    ...
    if(haveNewScript_)
    {
        ...
        if(receivedScript_->isSafeToExecute(obstacleDistanceGrid_)) // the script is valid only if it is compatible to the current map
        {
            activeScript_ .reset();
    ...
    if(activeScript_ && usingScript_)
    {
        ...
        else if(activeScript_->shouldWaitForPrecondition())
        {
            ...
            std::cout<<"INFO: MetricPlanner: Script is waiting until the precondition for the next task is satisfied.\n";
        }
...
void MetricPlannerDirector::runPlanners(void)
{
    ...
    {
        plannerStatus_.status = metric_planner_status_t::SUCCESS_REACHED_POSE; // this is the only task type that can be completed-for now.
    ...
    else
    {
        ...
        {
            visibility_ = VisibilityAnalysis(env.robotState.pose, laserConfigs_, *env.lpm, trackedObjectsBuffer_.read());
        }
        else if(!laserConfigs_.empty())
        {
            visibility_ = VisibilityAnalysis(env.robotState.pose,
                                             laserConfigs_.front(),
                                             *env.lpm,
                                             trackedObjectsBuffer_.read());
        }
        ...
        {
            plannerStatus_.status = metric_planner_status_t::ACTIVE_NORMAL; // this should account for (ideally) ALL normal situation.
        }
        else
        {
            plannerStatus_.status = metric_planner_status_t::FAILURE_CANNOT_FIND_SOLUTION; // indicate failure case otherwise.
        }
    }
...
void MetricPlannerDirector::transmitCalculatedOutput(system::ModuleCommunicator& transmitter)
{
    ...
    if(haveMotionControllerTask_) // handle controller tasks first! Other large pieces of data might take more time to communicate
    {
        // package the result for output comsumer
        int64_t motionTargetTaskTimeoutUs = 10*params_.plannerUpdatePeriodUs; // set timeout length for graceful slowdown.
        // the motion controller will send stop commands if the motion target task has timed out.
        motionControllerTask_ = std::shared_ptr<MotionControllerTask>(new MotionTargetTask(nextMotionTarget_, motionTargetTaskTimeoutUs, planReleaseTimeUs_));
...
void MetricPlannerDirector::openDataLog(void)
{
    if(!dataLog.is_open())
    {
        ...
        filename<<"planner_data_"<<timeInfo->tm_mon<<timeInfo->tm_mday<<timeInfo->tm_hour<<timeInfo->tm_min<<".log";
...
void MetricPlannerDirector::logData(void)
{
    ...
    //     <<robotStateBuffer_.read().pose.theta<<' '
        <<robotStateBuffer_.read().velocity.linear<<' '
        <<robotStateBuffer_.read().velocity.angular<<' '
    ...
    //     <<robotStateBuffer_.read().acceleration.angular<<' '
        ...
        <<robotStateBuffer_.read().differentialWheels.leftWheel.speed<<' '
        <<nextMotionTarget_.velocityGain<<std::endl;
    }
}
