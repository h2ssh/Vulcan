...
{
    std::string referenceHeadingType =
      config.getValueAsString(kNavigationTaskParamsHeading, kNonholonomicHeadingTypeKey);
...
// , rrtBuilderParams(config)
{
    ...
    useLearnedNorm =
      utils::load_serializable_from_file(config.getValueAsString(kNavigationTaskParamsHeading, kLearnedNormFileKey),
                                         learnedNormParams);
