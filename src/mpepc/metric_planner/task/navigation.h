 ...
 *
 *
 * The id for the task is assigned to the system timestamp at the time of creation. Thus, to ensure the id stays the
 * same only copies of a NavigationTask be made, rather than new tasks with the same target type.
 */
...
private:
    ...
    {
        ar(id_, target_, waypointPoses_, candidateTargets_, isPositionTask_);
    }
};
