...
bool UnicycleRRTStar::improvePath(size_t maxAdditionalSamples, bool canTerminateEarly)
{
    ...
    while(numNewSamples <= maxAdditionalSamples)
    {
        ...
        std::cout << "NumNewSamples:" << numNewSamples << "    maxAdditionalSamples:" << maxAdditionalSamples << "    Nearest Node: " << nearestNode->pose << '\n';
    ...
    std::cout << "UnicycleRRTStarInfo: NumSamples : " << numSamples_ << "    TotalTimeSpent (ms): " << totalMilliSecondsSpent_ <<"\n";
...
// Steering
unicycle_path_segment_t UnicycleRRTStar::extendTo(const unicycle_rrt_node_t& rrtNode, const pose_t& sampledPose, double maxExtension, bool isMovingBackward) const
{
...
double UnicycleRRTStar::rrtCostFromNodeToPose(const unicycle_rrt_node_t& fromNode, const pose_t& toPose, bool isMovingBackward)
{
...
{
    UnicycleLyapunovDistance lyap(toNode.shouldBeApproachedBackward ? toNode.pose.flip() : toNode.pose, params_.distanceParams);
...
unicycle_rrt_node_t* UnicycleRRTStar::rrtInsertNode(unicycle_rrt_node_t* parentNode, const pose_t& childPose, bool isGoal, bool isMovingBackward)
{
    double costFromParent = rrtCostFromNodeToPose(*parentNode, childPose, isMovingBackward);
    unicycle_rrt_node_t* insertedNode = RRT_.insertNode(childPose, costFromParent, isGoal, isMovingBackward, parentNode);
...
void UnicycleRRTStar::rrtReconnect(unicycle_rrt_node_t* newParent, unicycle_rrt_node_t* childToRewire, double rewiredEdgeCost)
{
...
void UnicycleRRTStar::rrtPropagateCostToChildren(unicycle_rrt_node_t* parentNode)
{
    for(auto childIt = parentNode->children.begin(), childEnd = parentNode->children.end(); childIt != childEnd; childIt++)
    {
        ...
        double costToChild = rrtCostFromNodeToPose(*parentNode, (*childIt)->pose, (*childIt)->shouldBeApproachedBackward);
...
bool UnicycleRRTStar::retrievePathFromLeaf(unicycle_rrt_node_t* leafNode)
{
    ...
    else
    {
        ...
        if(!pathNodes_.empty())
        {
            ...
            for(auto pathNodeIt = pathNodes_.begin(), pathNodeEnd = pathNodes_.end(); pathNodeIt != pathNodeEnd; pathNodeIt++)
            {
                // path info
                if(pathNodeIt != pathNodes_.begin())
                {
                    ...
                    plannedPath_.insert(plannedPath_.begin(), pathSegment.path.begin(), pathSegment.path.end()); // NOTE: Is steps sufficient?
                }
