...
//     void resetGoalPose (const pose_t& goalPose);
    ...
    // getters for retrieving various description of the planned path. Will return empty vector if no path has been found.
...
private:
    ...
    unicycle_path_segment_t extendTo(const unicycle_rrt_node_t& rrtNode, const pose_t& sampledPose, double maxExtension, bool isMovingBackward = false) const;
...
//     bool obstacleFree(const std::vector<pose_t>& steps, const ObstacleDistanceGrid& grid, const RobotCollisionModel& robotShape); // to be implemented
    ...
    // Cost definitions
    double rrtCostFromNodeToPose(const unicycle_rrt_node_t& fromNode, const pose_t& toPose, bool isMovingBackward = false);
    double rrtCostFromPoseToNode(const pose_t& fromPose, const unicycle_rrt_node_t& toNode);

    double rrtCostToGoViaNode(const pose_t& robotPose, const unicycle_rrt_node_t& node); // works only if a path has been found
...
//     double rrtCostFromPoseToNode(const unicycle_path_segment_t& directedEdge);
    ...
    unicycle_rrt_node_t* rrtInsertNode(unicycle_rrt_node_t* parentNode, const pose_t& childPose, bool isGoal, bool isMovingBackward = false);
    ...
    pose_t        startPose_;
    pose_t        goalPose_;
    ObstacleDistanceGrid grid_; // currently assume fixed map. will implement efficient reuse of information in case of partial map change later.
    ...
    std::vector<std::pair<pose_t, bool>>            plannedWaypoints_; // a series of waypoints and approach direction. i.e. <pose, isBackward> from start to goal
//     std::vector<std::pair<UnicycleLyapunovDistance, bool>> plannedManifold_;  // a series of lyapunov functions anchored around each waypoint on the path.
