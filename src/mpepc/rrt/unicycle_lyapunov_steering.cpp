...
UnicycleLyapunovSteering::UnicycleLyapunovSteering(const unicycle_lyapunov_steering_params_t& params)
: params_(params)
{
}

    
unicycle_path_segment_t UnicycleLyapunovSteering::extend(const pose_t& robotPose, const UnicycleLyapunovDistance& lyap, double maxExtension) const
{
...
//         std::cout<< "is rotating at: "<< rotatedPose << "    with increments: " << params_.incrementalRotation <<'\n';
    ...
    while(!isConverged)
    {
        ...
        pose_t translatedPose = incrementalTranslation(steps.back(), lyap, params_.incrementalTranslation, &path, &isConverged);
//         std::cout<< "is translating at: "<< translatedPose << "    with increments: " << params_.incrementalTranslation <<'\n';
...
unicycle_path_segment_t UnicycleLyapunovSteering::steer(const pose_t& robotPose, const UnicycleLyapunovDistance& lyap) const
{
    return this->extend(robotPose, lyap, -1); // extend without length constraint
}


std::vector<Point<float>> UnicycleLyapunovSteering::steerAway(const Point<float>& point, const UnicycleLyapunovDistance& lyap, double maxExtension) const
{
    // TODO: Implement this!
...
pose_t UnicycleLyapunovSteering::incrementalRotation(const pose_t& robotPose, const UnicycleLyapunovDistance& lyap, double maxRotation, bool* isAligned) const
{
...
pose_t UnicycleLyapunovSteering::incrementalTranslation(const pose_t& robotPose, const UnicycleLyapunovDistance& lyap, double maxTranslation, std::vector< Point< float > >* points, bool* isConverged) const
{
    ...
    while(true)
    {
        ...
        *isConverged = distance_on_manifold(coords.r, coords.phi, lyap.params_.kPhi) < kConvergedRadius;
        ...
        float deltaStar = stabilizing_delta_star(coords.r, coords.phi, lyap.params_.kPhi, lyap.params_.vectorFieldType, lyap.params_.smallRadius);
    ...
    // final pose
    double deltaAtEndPoint = stabilizing_delta_star(coords.r, coords.phi, lyap.params_.kPhi, lyap.params_.vectorFieldType, lyap.params_.smallRadius);
...
//                                                                             double kPhi,
//                                                                             stabilizing_vector_field_type_t vectorFieldType,
//                                                                             const RobotCollisionModel&  robotShape,
...
//     {
//         // rotation onto vector field (isAligned is updated here)
//         pose_t rotatedPose = rotate_onto_vector_field(robotPose, targetPose, maxRotation, kPhi, vectorFieldType, &isAligned, smallRadius);
//         
//         // collision check
...
//     {
//         // translation along vector field (path and isConverged are updated here)
//         pose_t translatedPose = steer_along_vector_field(robotPose, targetPose, maxTranslation, kPhi, vectorFieldType, &path, &isConverged, smallRadius);
//         
//         // collision check
