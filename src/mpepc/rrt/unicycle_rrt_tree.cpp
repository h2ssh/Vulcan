...
unicycle_rrt_node_t* UnicycleRRTTree::insertNode(const pose_t& newSample,
                                                 float costFromParent,
                                                 bool isGoal,
                                                 bool isMovingBackward,
                                                 unicycle_rrt_node_t* parentNodePtr)
{
    assert(parentNodePtr != nullptr);

    // create a new node
    int newNodeID = tree_.back().nodeID
      + 1;   // this depends on the monotonically increasing IDs in the tree, which is guaranteed by construction.
...
std::vector<unicycle_rrt_node_t*>
  UnicycleRRTTree::findNearbyNodesFromPose(const pose_t& pose, float distanceThreshold, bool isMovingBackward)
{
...
std::vector<unicycle_rrt_node_t*>
  UnicycleRRTTree::findNearbyNodesToPose(const pose_t& pose, float distanceThreshold, bool isMovingBackward)
{
    std::vector<unicycle_rrt_node_t*> neighbors;
...
unicycle_rrt_node_t*
  UnicycleRRTTree::findNearestNodeFromPose(const pose_t& pose, double* nonHolonomicDistance, bool isMovingBackward)
{
...
unicycle_rrt_node_t*
  UnicycleRRTTree::findNearestNodeToPose(const pose_t& pose, double* nonHolonomicDistance, bool isMovingBackward)
{
    unicycle_rrt_node_t* nearestNode = nullptr;
    double minDistance = 1000000.0;   // some large number;
