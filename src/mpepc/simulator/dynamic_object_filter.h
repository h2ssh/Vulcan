...
private:
     ...
     * \param    map             obstacle cost map, which stores information about freesapce and minimum distance to the
     * nearest static obstacle in the map \param    startTimeUs     time at which to start the simulation in
     * microseconds. \param    timeStep        time interval between simulated points.
     */
    ...
    // NOTE: This is potentially dangerous method that lets the robot completely ignore object directly behind the robot
    // at close range,
    //       but at least it allows us to perform demos with people closely following the robot from behind. We probably
    //       want to have more sophisticated method for handling object-robot interactions so that we do better
    ...
    Point<float> estimatePreferredVelocity(const dynamic_object_state_t& objectState, const pose_t& objectGoal);
};
