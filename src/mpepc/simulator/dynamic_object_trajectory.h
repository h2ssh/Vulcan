...
struct dynamic_object_trajectory_t
{
    ...
    tracker::DynamicObject::Ptr
      laserObject;   // original data from which the object trajectory were estimated. Rich set of data exists in here.
};
...
{
    ar(state.x, state.y, state.xVel, state.yVel, state.xAccel, state.yAccel);
}
...
DEFINE_DEBUG_MESSAGE(std::vector<mpepc::dynamic_object_trajectory_debug_info_t>,
                     ("MPEPC_DYNAMIC_OBJECT_TRAJECTORIES_DEBUG_INFO"))
