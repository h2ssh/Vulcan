...
, simulatorJoystickControl_(params.joystickControlLawParams,  params.robotPlantModelParams.differentialMotorsPlantParams) // dynamic joystick controller
, simulatorRobotPlantModel_(robot::create_robot_plant_model(params.robotPlantModelParams.type, params.robotPlantModelParams)) // a dynamic robot model of the specified type
                                                                        ...
                                                                        int64_t                      startTimeUs)
{
    ...
    if(std::abs(startTimeUs - currentTimeUs) < utils::sec_to_usec(1)) // only do this when the system time makes sense
    {
        std::cout<<"DEBUG: RobotSimulator: relative timestamp (ms): Pose      : "<< (robotState.pose.timestamp - currentTimeUs)/1000 <<'\n';
        std::cout<<"DEBUG: RobotSimulator: relative timestamp (ms): Start Time: "<< (startTimeUs - currentTimeUs)/1000 <<'\n';
    }
    ...
    std::cout<<"DEBUG: RobotSimulator: timestamp (ms): PoseDistribution : "<< robotState.poseDistribution.timestamp/1000 <<'\n';
    std::cout<<"DEBUG: RobotSimulator: timestamp (ms): Velocity         : "<< robotState.velocity.timestamp/1000 <<'\n';
    std::cout<<"DEBUG: RobotSimulator: timestamp (ms): RightDriveWheel  : "<< robotState.differentialWheels.rightWheel.timestamp/1000 <<'\n';
    std::cout<<"DEBUG: RobotSimulator: timestamp (ms): LeftDriveWheel   : "<< robotState.differentialWheels.leftWheel.timestamp/1000 <<'\n';
#endif
    ...
    control_law_output_t controlLawOutput = simulatorKinematicControlLaw_.computeOutput(simulatorState.pose, motionTarget, 0);
    simulatorKinematicControlLaw_.resetInternalStates(controlLawOutput.angularVelocity, -1); // assume unconverged state at initialization
    ...
    std::cout<<"    ("<<simulatorState.velocity.linear<<' '
                      <<simulatorState.velocity.angular<<' '
    ...
    {
        // compute command
        control_law_output_t      controlLawOutput = simulatorKinematicControlLaw_.computeOutput(simulatorState.pose, motionTarget, simulatorTimeUs);
        robot::velocity_command_t velocityCommand(controlLawOutput.linearVelocity, controlLawOutput.angularVelocity);
        robot::joystick_command_t joystickCommand  = simulatorJoystickControl_.computeOutput(simulatorState, controlLawOutput);
    ...
    isInitialized_ = true;
    initializedConvergenceStartTimeUs_         = simulatorKinematicControlLaw_.getConvergenceStartTimeUs(); // controller remembers when the robot was within the convergence zone.
...
{
    return estimateRobotTrajectory(candidateMotionTarget,
                                   defaultTimeStep_,
                                   defaultTimeLength_,
                                   trajectory);
}
                                                            ...
                                                            int64_t convergenceStartTimeUs)
{
    ...
    std::cout<<"DEBUG: RobotSimulator: Running the simulator for: (timestep, duration, numSteps) = : ("<<timeStep<<','<<timeLength<<','<<timeLength/timeStep_<<")\n";
    if(timeLength < 0.0)
    {
    ...
    simulatorKinematicControlLaw_.resetInternalStates(prevAngularVelocityCommand,
                                                      convergenceStartTimeUs);
    ...
    // do a default long timeLength is the timeLength is not properly specified. We're gonna exit when the target is reached.
    ...
    while(!hasReachedstopCondition)
    {
        ...
        control_law_output_t      controlLawOutput = simulatorKinematicControlLaw_.computeOutput(simulatorState.pose, candidateMotionTarget, simulatorTimeUs);
        robot::velocity_command_t velocityCommand(controlLawOutput.linearVelocity, controlLawOutput.angularVelocity);
        robot::joystick_command_t joystickCommand  = simulatorJoystickControl_.computeOutput(simulatorState, controlLawOutput);
        robot::motion_command_t   simulatorCommand(robot::AUTONOMOUS_CONTROLLER, velocityCommand, joystickCommand);
        ...
        simulatorState = simulatorRobotPlantModel_->nextState(simulatorState, simulatorCommand, timeStep); // planner interval is in float (seconds)

#ifdef DEBUG_ROBOT_SIMULATOR
        std::cout<<"DEBUG: RobotSimulator: Command: ("<<simulatorCommand.velocityCommand.linear <<','<<simulatorCommand.velocityCommand.angular<<','
                                                      <<simulatorCommand.joystickCommand.forward<<','<<simulatorCommand.joystickCommand.left<<")\n";
        std::cout<<"DEBUG: RobotSimulator: Pose: ("<<simulatorState.pose.x<<','<<simulatorState.pose.y<<','<<simulatorState.pose.theta<<")\n";
#endif
...
//     {
//         // compute command
//         controlLawOutput = simulatorKinematicControlLaw_.computeOutput(simulatedMotionState.pose, controlLawTarget, simulatorTimeUs);
//
...
//         // propagate simulated state and time
//         simulatorTimeUs += timeIncrementUs;
//         simulatedMotionState = simulatorKinematicRobotPlantModel_.nextState(simulatedMotionState, motionCommand, timeStep);
//
//         // record updated state
...
//
//         // check to see if the robot has reached the target
//         trajectory.hasArrivedAtTarget = simulatorKinematicControlLaw_.haveReachedTarget(simulatedMotionState.pose, controlLawTarget.pose, simulatorTimeUs);
//     }
//
//     return trajectory;
// }
