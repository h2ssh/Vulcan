...
const float SMALL_RADIUS_M = 0.05; // constant determining when the robot is too close to the target to compute the line of sight.
...
*
*     r     : radial distance from current pose to the target. negative when the desired direction of approach is backward.
...
struct control_law_coordinates_t
{
    ...
    control_law_coordinates_t(float r = 0, float theta = 0, float delta = 0)
    : r(r)
    , theta(theta)
    , delta(delta)
    {
    ...
    control_law_coordinates_t(const pose_t& robotPose, const pose_t& targetPose, const approach_direction_t direction = FORWARD)
    {
        ...
        else if(direction == BACKWARD) // flip the target and the robot pose when the direction of approach is backward.
        {
            float lineOfSightOrientation = (r > SMALL_RADIUS_M) ? atan2(deltaY, deltaX) : angle_sum(targetPose.theta, M_PI);
    ...
    {
        float lineOfSightOrientation = (r > 0.0) ? angle_diff(targetPose.theta, theta) : angle_diff(angle_sum(targetPose.theta, M_PI), theta);
        float robotOrientation = (r > 0.0) ? angle_sum(lineOfSightOrientation, delta) : angle_sum(lineOfSightOrientation + M_PI, delta);
    ...
    {
        float lineOfSightOrientation = (r > 0.0) ? angle_diff(robotPose.theta, delta) : angle_diff(angle_sum(robotPose.theta, M_PI), delta);
        float targetOrientation = (r > 0.0) ? angle_sum(lineOfSightOrientation, theta) : angle_sum(lineOfSightOrientation + M_PI, theta);
...
* (4) (optional) shape parameters (also can be interpreted as positional and steering gain parameters) for the KinematicControlLaw
* 
...
struct motion_target_t
{
    ...
    motion_target_t(const pose_t& targetPose, const approach_direction_t direction = FORWARD, float velocityGain = DEFAULT_VELOCITY_GAIN)
    : pose(targetPose)
    , direction(direction)
...
/**
* control_law_output_t encodes the output of the KinematicControlLaw, which includes reference heading, curvature and velocities
* as well as a indicator for reaching the target and the instance of control_law_coordinates_t used to compute those values.
*/
