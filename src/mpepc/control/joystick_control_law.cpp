...
JoystickControlLaw::JoystickControlLaw(const joystick_control_law_params_t& params,
...
robot::joystick_command_t JoystickControlLaw::computeOutput(const motion_state_t& robotState,
                                                            const control_law_output_t& reference)
{
    ...
    if (params_.shouldOutputDebugMessage) {
        std::cout << "DEBUG JOYSTICK CONTROL: reference velocity : (" << reference.linearVelocity << ','
                  << reference.angularVelocity << ")\n";
        std::cout << "DEBUG JOYSTICK CONTROL: current velocity   : (" << robotState.velocity.linear << ','
                  << robotState.velocity.angular << ")\n";
        std::cout << "DEBUG JOYSTICK CONTROL: controller status  : (" << isTryingToMove_ << ',' << isNotMoving_ << ','
                  << integralError_ << ',' << shouldTryHarderToMove_ << ")\n";
        std::cout << "DEBUG JOYSTICK CONTROL: Feedforward command: (" << feedforwardCommand.forward << ','
                  << feedforwardCommand.left << ")\n";
        std::cout << "DEBUG JOYSTICK CONTROL: Feedback command   : (" << feedbackCompensation.forward << ','
                  << feedbackCompensation.left << ")\n";
...
robot::joystick_command_t JoystickControlLaw::computeFeedbackCommand(const motion_state_t& currentState,
                                                                     const control_law_output_t& reference)
{
    robot::joystick_command_t output;
    ...
    if (currentState.haveWheels) {
        currentVelocity.linear =
          (currentState.differentialWheels.rightWheel.speed + currentState.differentialWheels.leftWheel.speed) / 2.0f;
        currentVelocity.angular =
          (currentState.differentialWheels.rightWheel.speed - currentState.differentialWheels.leftWheel.speed)
          / robotParams_.wheelbase;
    } else {
    ...
    if (params_.useAdaptiveParams) {
        ...
        linearVelocityPGain =
          fmax(1.0 + (kGainMultiplier - 1.0) / kGainIntercept * (kGainIntercept - fabs(reference.linearVelocity)), 1.0)
          * linearVelocityPGain;
        angularVelocityPGain =
          fmax(1.0 + (kGainMultiplier - 1.0) / kGainIntercept * (kGainIntercept - fabs(reference.angularVelocity)), 1.0)
          * angularVelocityPGain;

        // vary maximum control effort based on current speed. Low speed (closer to being stationary) means higher max
        // control effort.
        float currentSpeed =
          sqrt(currentVelocity.linear * currentVelocity.linear + currentVelocity.angular * currentVelocity.angular);
        maxControlEffort =
          fmax(1.0 + (kEffortMultiplier - 1.0) / kEffortIntercept * (kEffortIntercept - currentSpeed), 1.0)
          * maxControlEffort;
    }
...
robot::joystick_command_t JoystickControlLaw::computeFeedforwardCommand(const control_law_output_t& reference)
{
    ...
    // TODO: ideally, have steady_state_joystick_positions = inverse_model(reference_velocity) and have the inverse
    // model live within the planner. i.e. if(useLearedModel) { ouput = inverseModel->steadyStateCommand(const
    // velocity_t& referenceVelocity) }
    ...
    // This preserves the ratio between the linear and the angular velocity, which is important for maintaining
    // curvature.
    ...
    if (fabs(desiredRightWheelSpeed) < 0.005)   // if desired speed steady state is very small
    {
        ...
        rightMotorCommand = robotParams_.motorBeta / robotParams_.motorAlpha * desiredRightWheelSpeed
          + copysign(robotParams_.motorMu * robotParams_.motorGamma / robotParams_.motorAlpha, desiredRightWheelSpeed);
    ...
    if (fabs(desiredLeftWheelSpeed) < 0.005)   // if desired speed steady state is very small
    {
        ...
        leftMotorCommand = robotParams_.motorBeta / robotParams_.motorAlpha * desiredLeftWheelSpeed
          + copysign(robotParams_.motorMu * robotParams_.motorGamma / robotParams_.motorAlpha, desiredLeftWheelSpeed);
    ...
    float turnRate = (joystickForward == 0)
      ? robotParams_.turnRateInPlace
      : robotParams_.turnRateBase * (1 - robotParams_.turnReductionRate * (fabs(static_cast<float>(joystickForward))));
    ...
    if (params_.shouldModifyJoystickAxis) {
        int16_t modifiedJoystickForward =
          -sin(params_.axisRotation) * joystickLeft + cos(params_.axisRotation) * joystickForward;
        int16_t modifiedJoystickLeft =
          cos(params_.axisRotation) * joystickLeft + sin(params_.axisRotation) * joystickForward;
...
void JoystickControlLaw::updateStatus(const motion_state_t& currentState, const control_law_output_t& reference)
{
    ...
    {
        integralError_ +=
          fabs(reference.angularVelocity - currentState.velocity.angular);   // just using angular component for error.
        shouldTryHarderToMove_ = isNotMoving_
          && integralError_ > 1.0;   // if is trying to move and is not moving for some time then it should try harder!
    }
