...
public:
    ...
    //       KinematicControlLaw, requires current time information and also previous angular velocity at
    //       initialization. This is to deal with the sensor uncertainties and to better cope with the singularity
    //       introduced by the hard thresholding near the target.
    /**
     * computeOutput applies the kinematic control law (Park and Kuipers, ICRA-11) to the current pose and time of the
     * robot givan a motion target. \param    robotPose                      Cuurent pose of the robot. \param
     * motionTarget                   The motion target (pose, direction and velocity gain) to converge to. \param
     * currentTimeUs                  Current time (in microseconds). If <=0, this will reset internal states that it
     * uses to check convergence. \param    previousAngularVelocityCommand If specified, this will override the previous
     * angular velocity command remembered by the class. \return   Desired velocities, heading and curvature that
     * guarantees convergence to the motion target.
     */
    control_law_output_t
      computeOutput(const pose_t& robotPose, const motion_target_t& motionTarget, int64_t currentTimeUs = 0);
    ...
    /**
     * haveReachedTarget chekcs if the robot has reached a target pose by checking if the robot has been within a
     * sufficiently small neighborhood of the target for sufficient amount of time. \param    robotPose       Current
     * pose of the robot. \param    targetPose      Target pose of interest. \param    currentTimeUs   Current time (in
     * microseconds). \return   Indicator for reaching the target
     */
    ...
    /**
     * resetInternalStates initializes previous angular velocity command and convergence start time, which is needed in
     * simulation routines. \param    previousAngularVelocityCommand  Previous angular velocity command. \param
     * convergenceStartTimeUs          Convergence start time.
     */
...
private:
    /**
     * in normalUpdate, the following (abbreviated) equations extends the graceful controller from ICRA-11 to admit
     * negative velocity for back up motion.
     *
     ...
     *
     * The velocity computed by normalUpdate is automatically bounded in velocities. It is also bounded and smooth in
     * accelerations and jerks if the target is stationary.
     */
    control_law_output_t normalUpdate(const control_law_coordinates_t& coords, float velocityGain, float targetTheta);

    // smallnote: It makes sense to keep previous velocity command and convergence history as an internal state. Those
    // are the stuff produced by this class. Other variables like pose and time are not.
