...
void calculate_volume_statistics(const search_volume_scores_t& scores,
                                 const search_volume_t& volume,
                                 search_statistics_t& stats);
...
void calculate_volume_scores(const search_volume_t& volume,
                             const std::vector<Point<float>>& scan,
...
: fineResolutionGrid(params.fineResolutionGridWidth,
                     params.fineResolutionGridHeight,
                     params.fineResolutionMetersPerCell)
, coarseResolutionGrid(params.fineResolutionGridWidth,
                       params.fineResolutionGridHeight,
                       params.fineResolutionMetersPerCell)
, params(params)
{
...
void CorrelativeScanMatcher::setReferenceScan(const std::vector<Point<float>>& reference, int64_t referenceTime)
{
    ...
    coarseResolutionMetersPerCell =
      fineResolutionGrid.createCoarseGrid(params.coarseResolutionMetersPerCell, coarseResolutionGrid);

    referenceScanTimestamp = referenceTime;
}


MultivariateGaussian CorrelativeScanMatcher::findMostLikelyTransform(const std::vector<Point<float>>& scan,
                                                                     int64_t scanTime) const
{
    ...
    search_voxel_t bestVoxel = search_fine_resolution_grid_for_best_match(coarseScores,
                                                                          coarseVolume,
                                                                          scan,
                                                                          fineResolutionGrid,
                                                                          params,
                                                          ...
                                                          search_statistics_t& statistics)
{
    ...
    search_volume_t voxelVolume = create_search_volume_from_coarse_voxel(voxelToSearch,
                                                                         params.fineResolutionMetersPerCell,
                                                                         params.fineAngularSearchResolution);
    ...
    while (--numCoarseVoxels >= 0) {
        ...
        voxelVolume = create_search_volume_from_coarse_voxel(voxelToSearch,
                                                             params.fineResolutionMetersPerCell,
                                                             params.fineAngularSearchResolution);
                             ...
                             search_volume_scores_t& scores)
{
    ...
    for (uint8_t thetaIndex = 0; thetaIndex < volume.thetaRange.numSteps;
         ++thetaIndex, theta += volume.thetaRange.increment) {
...
Matrix calculate_transform_covariance(const search_statistics_t& stats)
{
    // The numbers here are VERY BIG, so divide out the sum of likelihoods for each weightedVoxelSum otherwise, well
    // it's just ugly.
    Matrix covarianceDouble = (stats.weightedVoxelSquareSum / stats.sumOfVoxelLikelihoods)
      - ((stats.weightedVoxelSum / stats.sumOfVoxelLikelihoods)
         * (trans(stats.weightedVoxelSum) / stats.sumOfVoxelLikelihoods));
    ...
    std::cout << "INFO: CorrelativeScanMatcher: K:(" << stats.weightedVoxelSquareSum(0, 0) << ' '
              << stats.weightedVoxelSquareSum(0, 1) << ' ' << stats.weightedVoxelSquareSum(0, 2) << '\n'
              << "                                 " << stats.weightedVoxelSquareSum(1, 0) << ' '
              << stats.weightedVoxelSquareSum(1, 1) << ' ' << stats.weightedVoxelSquareSum(1, 2) << '\n'
              << "                                 " << stats.weightedVoxelSquareSum(2, 0) << ' '
              << stats.weightedVoxelSquareSum(2, 1) << ' ' << stats.weightedVoxelSquareSum(2, 2) << ")\n";

    std::cout << "INFO: CorrelativeScanMatcher: u:(" << stats.weightedVoxelSum(0) << ',' << stats.weightedVoxelSum(1)
              << ',' << stats.weightedVoxelSum(2) << ")\n";

    std::cout << "INFO: CorrelativeScanMatcher: s:" << stats.sumOfVoxelLikelihoods << '\n';

    std::cout << "INFO: CorrelativeScanMatcher: cov: (" << covariance(0, 0) << ' ' << covariance(0, 1) << ' '
              << covariance(0, 2) << '\n'
...
search_volume_t create_initial_search_volume(const scan_matcher_params_t& params)
{
    ...
    volume.xRange = create_search_range(params.minXSearchArea,
                                        params.maxXSearchArea - params.minXSearchArea,
                                        params.coarseResolutionMetersPerCell);
    volume.yRange = create_search_range(params.minYSearchArea,
                                        params.maxYSearchArea - params.minYSearchArea,
                                        params.coarseResolutionMetersPerCell);
    volume.thetaRange = create_search_range(params.minThetaSearchArea,
                                            params.maxThetaSearchArea - params.minThetaSearchArea,
                                            params.coarseAngularSearchResolution);
                                                       ...
                                                       float fineAngularDiscretization)
{
    ...
    std::cout << "DEBUG: volume_from_coarse: xRange=" << volume.xRange << " yRange=" << volume.yRange
              << " thetaRange=" << volume.thetaRange << '\n';
#endif
