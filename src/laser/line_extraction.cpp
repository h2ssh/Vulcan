...
struct PointSet
{
    ...
    PointSet(std::pair<size_t, size_t> points) :
        set(points)
    {
    }

    /** Constructor for PointSet. */
    PointSet(std::pair<size_t, size_t> points, const Line<float>& line) :
        set(points),
        line(line)
    {
    }
...
int  max_two_above(const std::vector<Point<float>>& points, int begin, int end, const Line<float>& line, double threshold);
void merge_sets(const std::vector<PointSet>& sets, const std::vector<Point<float>>& coords, double slopeThresh, double distThresh, std::vector<PointSet>& merged);
void filter_sets(const std::vector<PointSet>& sets, const std::vector<Point<float>>& coords, uint16_t minPoints, double minLength, std::vector<Line<float>>& lines);

/** lines_from_segments creates Lines from segments in raw lidar data. */
extracted_lines_t lines_from_segments(const std::vector<Point<float>>& points, const std::vector<int>& segments, int minPoints);

/** Helper to convert a set of PointSets into a set of lines. */
void build_lines(std::vector<PointSet>& sets, const std::vector<Point<float>>& coords, uint16_t minPoints, vulcan::laser::extracted_lines_t& extracted);
...
extracted_lines_t vulcan::laser::split_and_merge(const polar_laser_scan_t& scan, const cartesian_laser_scan_t& cartesian, const split_and_merge_params_t& params)
{
    ...
    std::vector<PointSet> pointSets;  // for the initial clustering, then can be reused after splitting because the splits are the new official spots
...
void split_set(PointSet& set, const std::vector<Point<float>>& points, double distThresh, std::vector<PointSet>& split)
{
    ...
    * 2) Recursively call split_set on each new set. The first called should be the set with the lower start index to ensure
    *    that the ordering of the sets remains monotonic
    * 3) Base case: set can't be split, so add it and return.
    */
...
int max_two_above(const std::vector<Point<float>>& points, int begin, int end, const Line<float>& line, double threshold)
{
    ...
    */
    ...
    for(int x = begin; x < end - 1; ++x)
    {
        ...
        // Now if nextDist > threshold and cur and next have the same sign -- in this case, there's a new max value in town
        if((fabs(curDist) > threshold) && (fabs(nextDist) > threshold) && (nextDist * curDist > 0) && (curDist > maxDist))
        {
            maxDist = curDist;
            maxPos  = x;
        }
...
void merge_sets(const std::vector<PointSet>& sets, const std::vector<Point<float>>& coords, double slopeThresh, double distThresh, std::vector<PointSet>& merged)
{
    ...
    * 1) Distance between the lines is close -- for this, take point on one line and find projection to other line, then look at the distance of projection vector
    *
    * Continue merging the same line until no longer can be merged -- duh! The lines here are in monotonic ascending order as received by the laser scanner which
...
extracted_lines_t vulcan::laser::quick_split(const polar_laser_scan_t& scan, const cartesian_laser_scan_t& cartesian, const quick_split_params_t& params)
{
    /*
    ...
    */
    ...
    for(std::vector<PointSet>::const_iterator lineIt = points.begin(), endIt = points.end(); lineIt != endIt; ++lineIt)
    {
...
extracted_lines_t vulcan::laser::incremental(const polar_laser_scan_t& scan, const cartesian_laser_scan_t& cartesian, const incremental_params_t& params)
{
    /*
    ...
    */
    ...
    while(currentSet.set.second < coords.size())
    {
        ...
        {
            // Correlation defines whether the line is valid or not, if points are well-correlated, then all is well with the fit
            r = math::correlation(coords.begin() + currentSet.set.first, coords.begin() + tempEnd);

            #ifdef DEBUG_INCR
            std::cout<<"incremental: Correlation among the points: "<<r<<" Number of points: "<<(tempEnd - currentSet.set.first)<<'\n';
            #endif
        ...
        // If tempEnd < INCREMENT, then reached the end of the line, so create a new line and carry on, otherwise, loop again
        if(tempEnd < currentSet.set.second + params.increment)
        {
            ...
            #ifdef DEBUG_INCR
            std::cout<<"incremental: Added line:"<<currentSet.set.first<<"->"<<currentSet.set.second<<" r:"<<r<<'\n';
            #endif
        }
...
extracted_lines_t vulcan::laser::angle_segmentation(const polar_laser_scan_t& scan, const cartesian_laser_scan_t& cartesian, const angle_segmentation_params_t& params)
{
    /*
    * The strategy here is to look at the angle between vectors in the coords. If the angle is less than the
    * provided threshold angle, then a split point will be added. However, it is likely that for any given corner, there
    * will be multiple locations that could be validly split, so the location with the greatest angle will be used so
    * that the decision will hopefully be the best possible location. To do this, track when a violation has occurred, save
    * that point, then keep going, once the angles start decreasing, then you have moved beyond the split point, so
    */

    const std::vector<Point<float>>& coords = cartesian.scanPoints;

    // Need to check the distance from PI / 2 for the angles as the angle found is [0, PI], but I want only the curvy spots
    ...
    for(size_t x = params.windowSize, end = coords.size() - params.windowSize; x < end; ++x)
    {
        ...
        // If the current is less than max distance and the next point is greater than max dist, then a break should happen
        // Or if the current is greater than max distance and the next is less than max dist, then a break should also happen
        ...
        else if(((curAngle < threshAng) && (nextAngle < threshAng)) || needSplit) // need to make a split here -- a split is made by adding the current position to the list of segments
        {
            if(curAngle > minAngle) // need to split at the previous position
            {
                ...
                // Increment x beyond the window size from the previous split because otherwise you'll get some double splits to
                // happen where the max angle was already encountered, but you haven't finished splitting the rest of the data
...
extracted_lines_t lines_from_segments(const std::vector<Point<float>>& points, const std::vector<int>& segments, int minPoints)
{
    // lines_from_segments creates Lines from segments in raw lidar data. The provided segments come from the divisions created by angle_segmentation().
    ...
    for(size_t x = 0; x < segments.size(); ++x)
    {
        ...
        {
            extracted.lines.push_back(vulcan::math::total_least_squares(points.begin() + last, points.begin() + segments[x] + 1));

            std::fill(extracted.indices.begin() + last, extracted.indices.begin() + segments[x]+1, extracted.lines.size()-1);
        }
        else
        {
...
void vulcan::laser::filter_lines(extracted_lines_t& lines, double minLength)
{
    ...
    //     {
        //         #ifdef DEBUG_FILTER
        //         std::cout<<"filter_lines: Line: "<<lines[x].line<<" Length: "<<sqrt(pow(lines[x].line.a.x - lines[x].line.b.x, 2) + pow(lines[x].line.a.y - lines[x].line.b.y, 2))<<" Num points: "<<lines[x].numPoints
        //                 <<" Density: "<<(sqrt(pow(lines[x].line.a.x - lines[x].line.b.x, 2) + pow(lines[x].line.a.y - lines[x].line.b.y, 2)) / lines[x].numPoints)<<'\n';
        //         #endif
        //
        //         if(sqrt(pow(lines[x].line.a.x - lines[x].line.b.x, 2) + pow(lines[x].line.a.y - lines[x].line.b.y, 2)) < minLength)
        //         {
...
void initial_clustering(const std::vector<Point<float>>& points, double distance, std::vector<PointSet>& sets)
{
    ...
    */
    ...
    for(int x = 1, end = points.size(); x < end; ++x)
    {
        ...
        if(distance < (sqrt(pow(points[curSet.set.second].x - points[x].x, 2) + pow(points[curSet.set.second].y - points[x].y, 2)) / (sqrt(points[x].x * points[x].x + points[x].y * points[x].y))))
        {
            curSet.set.second = x;
...
/**
* filter_sets goes through the sets to see which contain valid data and which should be axed. A valid set is both long enough and contains enough points.
*
...
*/
void filter_sets(const std::vector<PointSet>& sets, const std::vector<Point<float>>& coords, size_t minPoints, double minLength, std::vector<Line<float>>& lines)
{
    // To filter the sets, go through and see that they have enough points and are longer enough -- if not, then make them into point clusters
...
void build_lines(std::vector<PointSet>& sets, const std::vector<Point<float>>& coords, uint16_t minPoints, vulcan::laser::extracted_lines_t& extracted)
{
    ...
    for(std::vector<PointSet>::const_iterator setIt = sets.begin(), endIt = sets.end(); setIt != endIt; ++setIt)
    {
        ...
        {
            extracted.lines.push_back(vulcan::math::total_least_squares(coords.begin() + setIt->set.first, coords.begin() + setIt->set.second));
//             extracted.lines.push_back(Line<float>(coords[setIt->set.first], coords[setIt->set.second-1]));

            std::fill(extracted.indices.begin() + setIt->set.first, extracted.indices.begin() + setIt->set.second, lineIndex++);
        }
        else
        {
