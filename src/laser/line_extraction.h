...
{
    extracted_lines_t(size_t numPoints = 0) : indices(numPoints) { }
 ...
 */
extracted_lines_t split_and_merge(const polar_laser_scan_t& scan,
                                  const cartesian_laser_scan_t& cartesian,
                                  const split_and_merge_params_t& params);

/**
 * quick_split uses a faster version of the line split that only calculates line parameters after the lines have been
 * extracted. quick_split is very similar to split-and-merge. The key difference is that a regression is not calculated
 * for each set of points, but rather the line is assumed to run from the first to last point in the set, thus removing
 * a large chunk of the calculation time.
 *
 ...
 */
extracted_lines_t quick_split(const polar_laser_scan_t& scan,
                              const cartesian_laser_scan_t& cartesian,
 ...
 * \param    lines           Lines found in the data (output)
 */
extracted_lines_t incremental(const polar_laser_scan_t& scan,
                              const cartesian_laser_scan_t& cartesian,
 ...
 *
 * The approach used is to consider three consecutive data points. The points will be considered part of the same
 * segment if the angle that they form is within some tolerance of a perfectly straight line.
 *
 ...
 */
extracted_lines_t angle_segmentation(const polar_laser_scan_t& scan,
                                     const cartesian_laser_scan_t& cartesian,
