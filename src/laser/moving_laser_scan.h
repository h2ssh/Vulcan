...
{
    ar(ray.position, ray.endpoint, ray.range, ray.angle, ray.intensity);
}
...
public:
     ...
     */
    MovingLaserScan(void) : id_(-1), timestamp_(0) { }
    ...
    , timestamp_(scan.timestamp)
    {
        ...
        if ((deltaTime == 0.0) || (previousPose.timestamp > scan.timestamp)
            || (scan.timestamp > currentPose.timestamp)) {
            deltaTime = 1.0;
        ...
        if (thetaStepRay * numSteps > M_PI / 4.0) {
            std::cout << "(dX,dY,dT) (sX,sY,sT): (" << deltaX << ',' << deltaY << ',' << deltaTheta << ") (" << xStepRay
                      << ',' << yStepRay << ',' << thetaStepRay << ") Time:" << deltaTime << " Pose time: ("
                      << scanStartTime << ',' << scanDuration << ") Start:" << rayPose << "\n"
                      << "\n\n\n\n\n\n\n\n";
        }
        ...
        for (int n = 0; n < scan.numRanges; n += stride) {
            ...
            adjusted.position = homogeneous_transform(scan.offset.to2DPosition(), rayPose.x, rayPose.y, rayPose.theta);
            ...
            if (scan.ranges[n] > 0) {
                ...
                adjusted.endpoint.x =
                  (std::cos(globalTheta) * endpoint.x) - (std::sin(globalTheta) * endpoint.y) + adjusted.position.x;
                adjusted.endpoint.y =
                  (std::sin(globalTheta) * endpoint.x) + (std::cos(globalTheta) * endpoint.y) + adjusted.position.y;
