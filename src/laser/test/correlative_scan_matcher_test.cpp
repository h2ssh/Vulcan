...
public:
    ...
    : laserName(laserChannel)
    , odometryName(odometryChannel)
    , poseName(poseChannel)
    , usingStaticData(false)
    {
        communicator.subscribeTo<polar_laser_scan_t>(this);
    }

    void useStaticData(void) { usingStaticData = true; }

    void useDynamicData(void) { usingStaticData = false; }
    ...
    void sendPose(const pose_t& pose) { communicator.sendMessage(pose, poseName); }
...
void run_scan_matcher(DataHandler& handler,
                      const vulcan::laser::scan_matcher_params_t& params,
                      int iterations,
                      bool shouldPerturb);
...
void run_scan_matcher(DataHandler& handler, const scan_matcher_params_t& params, int iterations, bool shouldPerturb)
{
    ...
    while (--iterations >= 0) {
        ...
        vulcan::MultivariateGaussian distribution =
          matcher.findMostLikelyTransform(cartesian.scanPoints, cartesian.timestamp);
        ...
        if ((fabs(transform(X_INDEX)) < 0.0001 && fabs(transform(Y_INDEX)) < 0.0001
             && fabs(transform(THETA_INDEX)) < M_PI / 36.0f)
        ...
        if (shouldPerturb) {
            std::cout << "Difference: (" << (transform(X_INDEX) + perturb.x) << ',' << (transform(Y_INDEX) + perturb.y)
                      << ',' << (transform(THETA_INDEX) + perturb.theta) << ')' << std::endl;
        }
