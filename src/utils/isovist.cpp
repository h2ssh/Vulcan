...
void Isovist::calculateDistanceScalars(PointIter begin, PointIter end, std::vector<double>& scalars)
{
    ...
    for (auto pIt = begin, endIt = end - 1; pIt != endIt; ++pIt) {
        ...
        if ((curDist < secondShortestDist) && (curDist < nextDist) && (curDist < prevDist)) {
    ...
    scalars[kShapeDistVariation] =
      (scalars[kShapeDistAvg] > 0.0) ? scalars[kShapeDistStd] / scalars[kShapeDistAvg] : 0.0;
    scalars[kShapeDistCompactness] =
      (max(distFromCenterAcc) > 0.0) ? scalars[kShapeDistAvg] / max(distFromCenterAcc) : 1e5;

    scalars[kRayCompactness] =
      (max(distFromCenterAcc) > 0.0) ? (mean(distFromCenterAcc) / max(distFromCenterAcc)) : 1.0;
...
void Isovist::calculateMomentsScalars(PointIter begin, PointIter end, std::vector<double>& scalars)
{
    if (scalars[kArea] > 0.0) {
        auto zernikeFeatures = zernike.moments<float>(begin, end, position_, scalars[kDmax] * 3);
...
void Isovist::calculateDerivs(std::vector<Point<float>>& endpoints)
{
    ...
    auto halfIt =
      std::copy(endpoints.begin() + minIdx, endpoints.begin() + minIdx + otherSide, endpoints.begin() + numPoints);
    *halfIt++ = *(endpoints.begin() + numPoints);
    ...
    scalarDerivs_[kWeightedOrientation] =
      angle_diff_abs_pi_2(halfScalars[kWeightedOrientation], otherHalfScalars[kWeightedOrientation]);
    scalarDerivs_[kMinDistOrientation] =
      angle_diff_abs(halfScalars[kMinDistOrientation], otherHalfScalars[kMinDistOrientation]);
    scalarDerivs_[kMaxThroughDistOrientation] =
      angle_diff_abs(halfScalars[kMaxThroughDistOrientation], otherHalfScalars[kMaxThroughDistOrientation]);
