...
int64_t SensorTime::timestamp(int64_t sensorTime)
{
    ...
    int64_t currentOffsetEstimate =
      clock_offset_estimate(sensorTime, systemTime, drift_estimate(0, slowRate_, fastRate_));
    int64_t previousOffsetEstimate =
      clock_offset_estimate(sensorTimeForOffset_,
                            systemTimeForOffset_,
                            drift_estimate(sensorTime - sensorTimeForOffset_, slowRate_, fastRate_));
...
#ifdef DEBUG_TIME
        std::cout << "DEBUG:SensorTime: Changed offset estimate: Old:" << previousOffsetEstimate
                  << "New:" << currentOffsetEstimate << '\n';
#endif
    }
...
#ifdef DEBUG_TIME
    std::cout << "DEBUG:SensorTime: Timestamp:" << correctedTime << " System time:" << systemTime
              << " Delta:" << (systemTime - correctedTime) << '\n';
#endif
    ...
    if ((totalSystemTimeElapsed_ > 0) && (numUpdates_ % 100 == 0)) {
        std::cout << "INFO:SensorTime: Drift estimate:"
                  << static_cast<long double>(totalSensorTimeElapsed_) / totalSystemTimeElapsed_ << '\n';
    }
