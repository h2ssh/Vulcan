...
private:
    ...
    struct heap_bucket_t
    {
        ...
        heap_bucket_t(const HeapNode& firstNode)
            : priority(firstNode.getPriority())
        {
            values.push_back(firstNode);
        }
    };
    ...
    std::unordered_map<priority_t, int> priorityIndices;        ///< Keep track of which priority is at which index to avoid a linear search when inserting a new node
