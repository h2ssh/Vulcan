...
int establish_serial_connection(const std::string& terminal, Baud baudRate, SerialMode mode, bool useCanonical, char eol, struct termios& oldSettings);
...
SerialConnection::SerialConnection(const std::string& device, Baud baudRate, SerialMode mode, bool useCanonical, char eol)
                                : dev(device)
                                , baud(baudRate)
                                , parityMode(mode)
...
bool SerialConnection::setBaudRate(Baud newBaud)
{
    ...
    struct serial_struct reset;  // before doing any changes, reset the high-speed settings in case not going to that mode
...
int establish_serial_connection(const std::string& terminal, Baud baudRate, SerialMode mode, bool useCanonical, char eol, struct termios& oldSettings)
{
    ...
    */
    ...
    // NOTE: Trying something slightly different here, s.t. parameters are set only a single time rather than with multiple calls
    ...
    //CREAD - enable receiver, meaning it's time for us to start getting some data by saying that the driver should start reading data
    ...
    termOptions.c_cflag |= HUPCL;  // just give this a try to see what happens with regards to the death of a connection if terminated unexpectedly
