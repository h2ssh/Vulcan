 ...
 *
 *   - SharedToRawComp : functor to use with std::find_if() to find the raw pointer associated with a collection of
 * shared_ptr.
 *   - UniqueToRawComp : functor to use with std::find_if() to find the raw pointer associated with a collection of
 * unique_ptr.
 *
...
{
    SmartToRawComp(const T* raw) : rawPtr(raw) { }

    bool operator()(const SmartPtr& ptr) const { return ptr.get() == rawPtr; }
...
std::vector<T*> smart_to_raw_vector(SmartIter begin, SmartIter end)
{
    ...
    std::transform(begin, end, raw.begin(), [](const Value& ptr) {
        return ptr.get();
    });
    return raw;
}

template <typename T, class SmartPtr = std::unique_ptr<T>>
struct SmartGetFunc
{
    T* operator()(const SmartPtr& ptr) const { return ptr.get(); }
...
{
    const T* operator()(const SmartPtr& ptr) const { return ptr.get(); }
 ...
 *   Type of *it = int*
 */
template <typename T, class SmartPtr = std::unique_ptr<T>, class Container = std::vector<SmartPtr>>
 ...
 */
template <typename T, class SmartPtr = std::unique_ptr<T>, class Container = std::vector<SmartPtr>>
using SmartToRawConstIter =
  boost::transform_iterator<SmartGetConstFunc<T, SmartPtr>, typename Container::const_iterator>;
