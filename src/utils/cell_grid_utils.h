 ...
 *   - copy_boundary_region : copies the boundary region into the appropriate part of the grid using the provided
 * coordinates
 *
 * Grid shifting:
 *   - grid_shift_params_t  : struct that calculates the indices by which a shift will occur, for use in the shift
 * functions
...
/**
 * global_point_to_grid_point converts a point in the global coordinate system to a point in the grid coordinate system.
 * The point can be fractions of a grid cell so no information is lost converting back and forth between global and grid
 * points.
 *
 ...
 *
 * NOTE: This function only returns the correct value when the grid cell is positive. All cells actually in the grid
 * have a positive coordinate, so for all practical purposes this function works correctly.
 *
 * \param    globalPoint     Point to be converted
 * \param    grid            CellGrid in which globalPoint will be converted
 ...
 * system. The round version of the function helps deal with truncation problems associated with not-quite exact
 * floating point math. Any values within 1e-5 of the next higher integer will be rounded to that integer.
 *
...
struct boundary_intersection_t
{
    ...
    , updateHeight(0)
    {
        ...
        if (boundary.area() > 0) {
            ...
            updateStartCell.x =
              round_in_tolerance((boundary.bottomLeft.x - updateBoundary.bottomLeft.x) * cellsPerMeter);
            updateStartCell.y =
              round_in_tolerance((boundary.bottomLeft.y - updateBoundary.bottomLeft.y) * cellsPerMeter);
            ...
            // After the calculations, go through and confirm that floating point errors haven't created an untenable
            // and memory-crashing situation by adjusting the boundary dimensions as needed based on the input sizes.
            // Explicitly supplying these dimensions because other floating-point errors could result in incorrect
            // calculations anyhow
...
#ifdef DEBUG_BOUNDARY_COORDS
            std::cout << "DEBUG:boundary_coords:Intersection:" << boundary << " Grid:" << gridStartCell
                      << " Boundary:" << updateStartCell << " Width:" << updateWidth << " Height:" << updateHeight
                      << '\n'
                      << "Dimensions:Grid:" << gridDimensions.first << ',' << gridDimensions.second
                      << " Update:" << updateDimensions.first << ',' << updateDimensions.second << '\n';
#endif
        }
    }
};
...
template <typename T>
void copy_boundary_region(const boundary_intersection_t& coords,
                          T* grid,
                          T* update,
...
#ifdef DEBUG_BOUNDARY_COORDS
    std::cout << "DEBUG:copy_boundary: boundary:" << coords.updateWidth << " update:" << updateWidth
              << " grid:" << gridWidth << " height:" << coords.updateHeight << " grid start:" << coords.gridStartCell
              << " update start:" << coords.updateStartCell << '\n';
#endif
...
struct grid_shift_params_t
{
    ...
    grid_shift_params_t(int gridShift, int gridMaxValue) : shift(gridShift), maxValue(gridMaxValue)
    {
...
inline std::pair<int, int>
  calc_cells_to_shift(const Point<float>& bottomLeft, const Point<float>& newBottomLeft, double cellsPerMeter)
{
...
template <typename T>
void shift_grid_cells(const grid_shift_params_t& xShift,
                      const grid_shift_params_t& yShift,
                      T* gridValues,
                      std::size_t width)
{
    for (int y = yShift.shiftStart; y != yShift.shiftEnd; y += yShift.shiftIncrement) {
        for (int x = xShift.shiftStart; x != xShift.shiftEnd; x += xShift.shiftIncrement) {
            gridValues[cell_to_index(x, y, width)] =
              gridValues[cell_to_index(x - xShift.shift, y - yShift.shift, width)];
        }
    }
}

template <typename T>
void reset_new_grid_cells(const grid_shift_params_t& xShift,
                          const grid_shift_params_t& yShift,
                          T* gridValues,
                          T initialValue,
                          uint16_t width)
