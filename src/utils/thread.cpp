...
Thread::~Thread(void)
{
    ...
    */
    ...
    if(!executing && persist)   // give the thread a chance to finish up on its own -- if here, then waiting on cond, broadcast and it'll bail
    {
        persist = false;
        objTask = 0;
...
int Thread::attachTask(Threadable* task)
{
    ...
    */
    ...
    if(retVal)
    {
        std::cerr<<"Thread already executing a task wait until the task completes before attaching a new task."<<std::endl;
    }
