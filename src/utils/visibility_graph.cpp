...
struct VertexSetVertex
{
    ...
    explicit VertexSetVertex(const VertexSet* idSet = nullptr, const VisGraphType* graph = nullptr)
    : idSet(idSet)
    , graph(graph)
    {
    }
...
struct VertexSetEdge
{
    ...
    explicit VertexSetEdge(const VertexSet* idSet = nullptr, const VisGraphType* graph = nullptr)
    : idSet(idSet)
    , graph(graph)
    {
    }
...
VisibilityGraphFeature
  feature_map_to_graph_feature(const FeatureMap& featMap, const VisGraphType& graph, VisibilityGraphFeatureType type);
...
VisibilityGraph::VisibilityGraph(const std::vector<VisGraphVertex>& points,
...
VisibilityGraph VisibilityGraph::createSubgraph(VisVertexIter begin, VisVertexIter end) const
{
    ...
    for (auto e : make_iterator_range(boost::edges(filtered))) {
        ...
        subgraph.edges_.push_back(
          std::make_pair(graph_[source(e, graph_)].position, graph_[target(e, graph_)].position));
    }
