...
public:
     ...
     * \param    rayTerminationFunc      Function that specifies when a cell in the grid should terminate a ray in the
     * isovist \param    options                 Options for the calculation of the isovists (optional, default =
     * default constructed options)
     */
    template <class Grid, class TerminationFunc>
    Isovist(Point<int16_t> position, const Grid& grid, TerminationFunc rayTerminationFunc, isovist_options_t& options)
    ...
    , cell_(position)
    {
        ...
        trace_range_until_condition(position,
                                    isovistRays,
                                    options.maxDistance,
                                    grid,
                                    rayTerminationFunc,
                                    options.endpointStorage_.begin());
        ...
        scalars_[kMinNormalDiff] = angle_diff_abs_pi_2(scalars_[kMinLineNormal], scalars_[kMinDistOrientation]);
    }
 ...
 * isovists in the field can be iterated over and a particular field can be generated by calling the appropriate
 * property observer for the individual Isovists.
 */
class IsovistField
{
public:
     ...
     * \param    options                 Options for the calculation of the isovists (optional, default = default
     * constructed options)
     */
    template <class Grid, class TerminationFunc>
                 ...
                 isovist_options_t options = isovist_options_t())
    {
        ...
        if (!positions.empty()) {
            ...
            std::size_t positionsPerThread = positions.size() / kNumThreads
     ...
     */
    IsovistField(const std::vector<Isovist>& isovists) : isovists(isovists) { }
...
private:
    ...
    struct IsovistThreadFunc
    {
        ...
        std::vector<Isovist> operator()(PointIter begin,
                                        PointIter end,
                                        const Grid& grid,
                                        TerminationFunc rayTerminationFunc,
