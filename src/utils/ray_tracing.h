...
/**
 * trace_ray_until_condition traces a ray from the start cell until the termination condition is true or the maximum
 * distance is reached.
 *
 ...
 * \param    terminationCond     Condition to use for deciding if the ray has reached its end  signature: bool(const
 * Grid&, Point<int>) \return   Endpoint of the ray in grid coordinates.
 */
                                        ...
                                        TerminationCondition terminationCond)
{
    ...
    if (dx == 0) {
        xIncr = 0;
        error = std::numeric_limits<double>::infinity();   // positive error means increment y. pos infinity to only
                                                           // increment y
    ...
    if (dy == 0) {
        yIncr = 0;
        error -= std::numeric_limits<double>::infinity();   // negative error means increment x. neg infinity to only
                                                            // increment x
    ...
    // If less than 2 increments were made, then the termination condition triggered immediately, or on the adjacent
    // cell which means we should clamp to the starting position
    if (numSteps < 2) {
        return start;
    }
    ...
    if (wasTerminated) {
        const double kCellDist =
          0.99999;   // keep the dist slightly inside the cell, so if truncation is used, the same
        ...
        if (lastCrossedHorizontal) {
            ...
            // Else came from the top of the cell
                ...
                cellBoundary =
                  Line<double>(rayCell.x, rayCell.y + kCellDist, rayCell.x + kCellDist, rayCell.y + kCellDist);
            }
        } else   // if(!lastCrossHorizontal)
        {
            ...
            // Otherwise, we enter from the right side of the cell
                ...
                cellBoundary =
                  Line<double>(rayCell.x + kCellDist, rayCell.y, rayCell.x + kCellDist, rayCell.y + kCellDist);
            }
        }
 ...
 * \param    grid                Grid in which to trace
 * \param    terminationCond     Condition to use for deciding if the ray has reached its end  signature: bool(const
 * Grid&, Point<int>) \param    endpointBegin       Location to being storing the endpoints. StorageIterator must
 * dereference to a Point<double> reference.
 */
                                 ...
                                 StorageIterator endpointBegin)
{
    ...
    for (std::size_t n = 0; n < range.numIncrements; ++n) {
        *endpointBegin++ =
          trace_ray_until_condition(start, range.startAngle + n * increment, maxDistance, grid, terminationCond);
    }
}

/**
 * is_cell_visible_from_position checks if a particular cell has a line of visibility from the provided position. Both
 * position and cell are in grid coordinates, not global coordinates.
 */
template <class Grid, class TerminationCondition>
                                   ...
                                   TerminationCondition terminationCond)
{
    ...
    auto cacheTerminationResultFunc = [&terminationFired, terminationCond](const Grid& g, Point<int> c) {
        return terminationFired |= terminationCond(g, c);
    };
...
/**
 * find_cells_along_line finds the cells in the grid along the specified line. The line is in grid coordinates, not
 * global coordinates.
 */
