...
public:
     ...
     */
    GridIterator(Grid& grid) : GridIterator(0, 0, grid.getWidthInCells(), grid) { }
 ...
 */
template <class T, class Grid>
std::pair<GridIterator<T, Grid>, GridIterator<T, Grid>> make_grid_iterators(Grid& grid)
{
    // The iteration goes in row-major order through the cells, so one-past-the-end is the first cell of the row
    // above the desired region
    using iterator = GridIterator<T, Grid>;
...
public:
     ...
     * Create an iterator across the entire grid.
     */
    TiledGridIterator(TiledGrid& grid) : TiledGridIterator(0, 0, grid.getWidthInCells(), grid) { }
    ...
    {
        return cell_iterator(x_ * grid_.getTileSize(), y_ * grid_.getTileSize(), grid_.getTileSize(), grid_);
    }

    cell_iterator end(void)
    {
        return cell_iterator(x_ * grid_.getTileSize(), (y_ + 1) * grid_.getTileSize(), grid_.getTileSize(), grid_);
    }
