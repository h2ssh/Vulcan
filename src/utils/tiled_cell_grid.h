...
public:
    ...
    {
        return (x >= 0) && (y >= 0) && (static_cast<std::size_t>(x) < width * N)
          && (static_cast<std::size_t>(y) < height * N);
    }
...
private:
    ...
    {
        ar(timestamp, id, gridValues, width, height, metersPerCell_, cellsPerMeter_, xIndices, yIndices, boundary);
    }
};
  ...
  transform_grid(const TiledCellGrid<T, N>& grid, T initialValue, double xRef, double yRef, double thetaRef)
{
    ...
    // Use the cell dimensions/2 in order to make sure the bo            ttom left is an integral number of cells from
...
void TiledCellGrid<T, N>::changeBoundary(const math::Rectangle<double>& newBoundary, T initialValue)
{
    ...
    math::Rectangle<double> adjustedBoundary(
...
{
    boundary = math::Rectangle<double>(
      newBottomLeft,
