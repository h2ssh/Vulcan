 ...
 *
 * In all cases, a maximum number of poses can be set for the trace_. When the maximum number of poses is hit, each
 * addition will erase the pose at the start of the trace. Poses can also be removed via the clearAll(), clearBefore(),
 * or clearAfter() methods that erase all poses occurring before or after a certain time.
 *
...
public:
    ...
    /**
     * poseAt finds the pose at the specified time. The pose at the specified time is determined via linear
     * interpolation between the time immediately before and immediately after the requested pose time.
     *
