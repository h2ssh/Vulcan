 ...
 * of space. TODO
 *
 * Currently, all goals are assumed reachable given the current robot state. However, a topological area can't be
 * reached if only a global metric map is available. Eventually, the interface should filter to only show the available
 * goals.
 */
class GoalInterface
{
