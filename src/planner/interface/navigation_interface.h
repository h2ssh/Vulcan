...
public:
     ...
     */
    void goToDecision(const Decision& decision, const pose_t& robotPose, system::SystemCommunicator& communicator);
     ...
     * \return   Number of new decisionsfound for the current location, which will be 0 if the location is unchanged
     * from the previous update.
     */
    int determineDecisions(const hssh::LocalTopoMap& map, const hssh::LocalLocation& location, const pose_t& pose);
     ...
     */
    void goToGoal(const Goal& goal, const hssh::LocalTopoMap* topoMap, system::SystemCommunicator& communicator);
