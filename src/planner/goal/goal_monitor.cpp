...
void GoalMonitor::updatePathSegmentProgress(const hssh::GlobalLocation& state)
{
    ...
    */
    if(!state.onPath)
    {
        ...
        // The route MUST, at this time, be an alternating sequence of Places and PathSegments, so enforce the new element to be a place
        assert(active.isPlace);

        // Check that the new route place matches the place in the map
        if(active.place.getId() != state.placeId)
        {
            std::cerr<<"ERROR:GoalMonitor: Moved to a place not on the planned route! Expected:"<<active.place.getId()<<" Actual:"<<state.placeId<<'\n';
...
void GoalMonitor::updatePlaceProgress(const hssh::GlobalLocation& state)
{
    if(state.onPath)
    {
        ...
        // The route MUST, at this time, be an alternating sequence of Places and PathSegments, so enforce the new element to be a path segment
        ...
        if(((active.segment.getPlusTransition().placeId  != state.pathState.entryPlaceId) && (active.segment.getMinusTransition().placeId != state.pathState.targetPlaceId)) &&
           ((active.segment.getMinusTransition().placeId != state.pathState.entryPlaceId) && (active.segment.getPlusTransition().placeId  != state.pathState.targetPlaceId)))
        {
            std::cerr<<"ERROR:GoalMonitor: Moved onto a path segment with an incorrect place at the other end!\n"
                     <<"Expected (P->M):"<<active.segment.getPlusTransition().placeId<<"->"<<active.segment.getMinusTransition().placeId
                     <<" Actual:"<<state.pathState.targetPlaceId<<"->"<<state.pathState.entryPlaceId<<'\n';
