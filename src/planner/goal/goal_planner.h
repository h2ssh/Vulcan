...
namespace vulcan
{
namespace hssh { class TopologicalMap; }
...
private:
    ...
    // Fragment info provides the path and direction of a path to be navigated. From the fragment info the associated global_path_fragment_t can be
    // found in a GlobalPlace for creating PlaceNeighborhoodTargets.
    ...
    std::shared_ptr<DecisionTarget> createInitialPathTarget(const hssh::GlobalPathSegment& segment, int nextPlaceId, const hssh::GlobalLocation& location);
    std::shared_ptr<DecisionTarget> createPlaceTarget(const hssh::LargeScaleStar& star, fragment_info_t entry, fragment_info_t exit);
