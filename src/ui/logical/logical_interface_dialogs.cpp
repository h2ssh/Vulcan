...
LogicalDecisionDialog::LogicalDecisionDialog(wxWindow* parent) : DecisionDialog(parent), activeButton(0)
{
...
void LogicalDecisionDialog::createTarget(void)
{
    ...
    if (activeButton == forwardDecisionButton) {
        target =
          std::shared_ptr<planner::DecisionTarget>(new planner::RelativePlaceTarget(planner::DECISION_GO_STRAIGHT));
        ...
        target =
          std::shared_ptr<planner::DecisionTarget>(new planner::RelativePlaceTarget(planner::DECISION_TURN_LEFT));
    } else if (activeButton == rightDecisionButton) {
        target =
          std::shared_ptr<planner::DecisionTarget>(new planner::RelativePlaceTarget(planner::DECISION_TURN_RIGHT));
    } else {
        assert(activeButton);
    }
}
...
LogicalGoalDialog::LogicalGoalDialog(wxWindow* parent,
...
void LogicalGoalDialog::createTarget(void)
{
    ...
    target = std::shared_ptr<planner::GoalTarget>(
      new planner::GoalTarget(0, map.getId(), map.getPlace(descriptions.find(description)->second)));
}
...
LogicalTaskCompleteDialog::LogicalTaskCompleteDialog(wxWindow* parent) : TaskCompleteDialog(parent)
{
}
...
LogicalExperimentCompleteDialog::LogicalExperimentCompleteDialog(wxWindow* parent) : ExperimentCompleteDialog(parent)
{
}
