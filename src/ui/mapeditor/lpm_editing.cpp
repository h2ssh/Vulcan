...
std::size_t                   flood_fill_lpm   (hssh::LocalPerceptualMap& lpm, const Point<int>& startCell, hssh::cell_type_t newType, uint8_t newCost);
...
{
    flood_fill_lpm(*lpm, edit.cell, (undo ? edit.originalType : edit.newType), (undo ? edit.originalCost : edit.newCost));
}
...
void enqueue_and_change_type_if_needed(hssh::LocalPerceptualMap& lpm, 
                                       ...
                                       CellQueue&                queue)
{
    // The check to enqueue is simplified because all visited cells will have their types changed immediately, so the visited
    // set is automatically handled
