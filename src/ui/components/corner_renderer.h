...
public:
    ...
    void setShapeWidths(float cornerLineWidth, float rectifiedCornerLineWidth, float circleRadius);
...
private:
    void renderFilledCircle(const Point<float>& center,
                            float radius,
                            const GLColor& color);   // NOTE: There should be a Scatter function which include this.
    void renderLine(const Point<float>& pointA,
                    const Point<float>& pointB,
                    float lineWidth,
                    const GLColor& color);   // NOTE: This should get merged with LinesRenderer
