...
struct grid_cell_t
{
    ...
    explicit grid_cell_t(int row = 0, int column = 0) : row(row), column(column) { }

    bool operator<(const grid_cell_t& rhs) const { return (row < rhs.row) || (row == rhs.row && column < rhs.column); }
};
...
struct preview_cell_contents_t
{
    ...
    explicit preview_cell_contents_t(GLColor color = GLColor()) : borderColor(color), mapId(-1) { }
};
...
public:
     ...
     * \return   Grid cell to which the map was set. If no cell is open, then the grid cell will be (-1,-1) to indicate
     * failure.
     */
