...
void VoronoiSkeletonGridRenderer::renderGrid(void)
{
    if(initialized)
    {
        ...
        {
            draw_two_textures_on_rectangle(gridOrigin.x, gridOrigin.y, metricWidth, metricHeight, textureXMax, textureYMax);
        }
        else
        {
            draw_one_texture_on_rectangle(gridOrigin.x, gridOrigin.y, metricWidth, metricHeight, textureXMax, textureYMax);
        }
...
void VoronoiSkeletonGridRenderer::initializeGridTextures(void)
{
    ...
    initialize_texture(textureNames[CLASSIFICATION_INDEX], classificationTexture, textureWidth, textureHeight, GL_RGB, GL_RGB);
    initialize_texture_16(textureNames[DISTANCE_INDEX], distanceTexture, textureWidth, textureHeight, GL_ALPHA, GL_ALPHA);
}
