                                               ...
                                               path_segment_attribute_t attributes)
{
    ...
    if (plusPlace.topo->cycle().contains(plusTransition) && minusPlace.topo->cycle().contains(minusTransition)
        && plusPlace.metric && minusPlace.metric)
    //         && (plusTransition.otherArea(segment.toArea()) == minusTransition.otherArea(segment.toArea())))
    {
        hssh::local_path_fragment_t plusFragment =
          plusPlace.metric->star().getFragmentWithId(plusPlace.topo->cycle().transitionIndex(plusTransition));
        hssh::local_path_fragment_t minusFragment =
          minusPlace.metric->star().getFragmentWithId(minusPlace.topo->cycle().transitionIndex(minusTransition));
...
GLColor TopologicalMapRenderer::getSegmentColor(const hssh::GlobalPathSegment& segment,
                                                path_segment_attribute_t attributes)
{
...
void TopologicalMapRenderer::drawLocation(const hssh::TopologicalState& topoMap, const hssh::MetricMapCache& places)
{
    ...
    // Is there actually a place we came from or are going to? If not, then the mapping is still bootstrapping and thus
    // there is no location to draw
...
{
    // If the mapBoundary has zero area, then this is the initial place being added, so the map boundary is the same as
    // the place boundary
