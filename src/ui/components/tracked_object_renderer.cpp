...
void TrackedObjectRenderer::renderObjectMotion(const tracker::DynamicObject& object, float time, size_t steps)
{
    ...
    for(std::size_t n = 1; n <= steps; ++n)
    {
        ...
        drawBoundary(object.boundary(),
                     GLColor(objectColor_.red(),
                             objectColor_.green(),
                             objectColor_.blue(),
                             alpha*std::pow(alphaStep, n)),
                     false);
    }
    glPopMatrix();
...
void TrackedObjectRenderer::renderEstimatedObjectTrajectory(const mpepc::dynamic_object_trajectory_debug_info_t& estimatedObjectTrajectory)
{
...
//     double objectRadius = std::max(0.2, estimatedObjectTrajectory.radius);
    ...
    {
        Point<float> estimatedObjectCenter(estimatedObjectTrajectory.states[n].x, estimatedObjectTrajectory.states[n].y);

        float estimatedObjectSpeed = sqrt(estimatedObjectTrajectory.states[n].xVel * estimatedObjectTrajectory.states[n].xVel +
        ...
        {
            float estimatedObjectHeading = atan2(estimatedObjectTrajectory.states[n].yVel, estimatedObjectTrajectory.states[n].xVel);
            GLColor black;
            black.set();
            glBegin(GL_LINES);
...
{
    GLColor objectColor(color.red(),
                        color.green(),
                        color.blue(),
                        calculateObjectAlpha(object.totalTimeSeen(), 3000));
