...
HypothesisTreeRenderer::HypothesisTreeRenderer(void) : tree_(std::make_unique<hssh::HypothesisTree>())
{
}
...
#ifdef DEBUG_SLOTS
    std::cout << "DEBUG:HypothesisTree:Node " << treeNode->id << " Breadth:" << treeNode->breadth
              << " Children:" << hypNode.children.size() << '\n';
#endif
}
...
#ifdef DEBUG_SLOTS
    std::cout << "DEBUG:HypothesisTree:Node " << node->id << " Breadth:" << node->breadth
              << " Slot:" << node->pixelCenter << " Children:" << node->children.size() << '\n';
#endif
