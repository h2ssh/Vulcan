...
* position is drawn as a filled in circle. The orientation is drawn as an arrow pointing in the direction of the target.
*
...
* drawn for the indicating the target. The orientation arrow will extend from waypoint position to orientation position,
* thus the length of the arrow will not be fixed. The other method simply accepts a pose. In this case, the orientation
* arrow will be a fixed length. The arrow will not scale as the view is zoomed-in/out, so it can disappear if the scene
* is drawn from a sufficiently far distance.
*/
...
public:
    ...
    */
    ...
    /**
    * setTargetSize sets the size, which is used as a radius of a circle, a base length of a triangle, and a base length of a rectable.
    ...
    * renderTargetCircle renders the specified target as a circle.
    *
    * \param    target          Pose to render. The orientation is a fixed length as specified by setOrientationArrowLength
    */
    ...
    * \param    isFilled        Optional parameter to set filled/unfilled triangle to be drwan. Default is unfilled rectangle.
    */
