...
public:
    ...
    /**
     * setMinViewDimensions sets the minimum dimensions of the viewing area displayed by the camera in the widget's
     * coordinate system, not the pixel coordinates. The minimum dimensions puts a limit on how close the camera will
     * zoom.
     */
    void setMinViewDimensions(float minWidth, float minHeight);

    /**
     * setMaxViewDimensions sets the maximum dimensions of the viewing area displayed by the camera in the widget's
     * coordinate system, not the pixel coordinates. The maximum dimensions puts a limit on how far out the camera will
     * zoom.
     */
    void setMaxViewDimensions(float maxWidth, float maxHeight);

    /**
     * setViewRegion sets the region to be rendered in the widget's coordinate system, not the pixel coordinates. The
     * camera will be zoomed accordingly to show the desired area.
     */
    void setViewRegion(float width, float height);

    /**
     * setCameraFocalPoint sets the position on which the camera should focus. This method can be used to set an initial
     * camera position and then the built-in camera control can handle the rest. If tracking some object, each time the
     * position of the object is encountered, the new focal point should be set.
     */
    ...
    /**
     * getCameraPosition retrieves the position of the camera in the coordinate frame. The position is where the camera
     * is placed. The focal point is where it is looking.
     */
    ...
    /**
     * removeKeyboardHandler removes the associated handler from the stack of event handlers. If the requested handler
     * to removed isn't found, then nothing happens.
     */
