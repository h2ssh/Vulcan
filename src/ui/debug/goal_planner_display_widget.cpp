...
{
    mapRenderer->setRenderColors(params.placeVertexColor,
...
void GoalPlannerDisplayWidget::setSequenceColors(const std::vector<planner::goal_route_element_t>& sequence,
...
void GoalPlannerDisplayWidget::handlePlaceSelectionEvent(int mouseX, int mouseY)
{
    ...
    Point<float> cursorWorldCoordinate =
      convert_screen_to_world_coordinates(Point<int>(mouseX, size.GetHeight() - mouseY), getCameraPosition());
...
bool GoalPlannerDisplayWidget::cursorIsInsideBoundary(const Point<float>& cursor,
...
void GoalPlannerDisplayWidget::setSegmentSelectionColor(int32_t oldId, int32_t selectedId, float alpha)
{
    ...
    if (selectedId != -1) {
        ...
        mapRenderer->setPathSegmentColor(selectedId,
                                         GLColor(segmentColor.red(), segmentColor.green(), segmentColor.blue(), alpha));
    }
}
...
void GoalPlannerDisplayWidget::populatePathSegmentLines(void)
{
    ...
    //             {
    //                 const hssh::GlobalPlace& plusPlace  =
    //                 places.find(segmentIt->getPlusTransition().placeId)->second; const hssh::GlobalPlace& minusPlace
    //                 = places.find(segmentIt->getMinusTransition().placeId)->second;
    //
...
void GoalPlannerDisplayWidget::convertSelectedLocationsToGlobalLocation(void)
{
    ...
    //     }
    //
    //     const hssh::GlobalDecisionPoint* place   = map.getDecisionPoint(firstSelectedWasPlace ? firstSelectedLocation
    //     : secondSelectedLocation); const hssh::GlobalPathSegment* segment = map.getPathSegment(firstSelectedWasPlace
    //     ? secondSelectedLocation : firstSelectedLocation);
    //
    //     if(firstSelectedWasPlace)
    //     {
    ...
    //         selectedLocation.pathState.segmentIndex =
    //         path->getSegmentIndex(path->getSegmentWithId(segment->getUniqueId()));
    //     }
    //
