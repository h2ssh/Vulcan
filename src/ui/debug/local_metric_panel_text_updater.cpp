...
void LocalMetricPanelTextUpdater::handleData(const imu_data_t& imuData, const std::string& channel)
{
    ...
    accelOut << std::fixed << std::showpos << std::setprecision(2) << '(' << imuData.acceleration[IMU_X_INDEX] << ','
             << imuData.acceleration[IMU_Y_INDEX] << ',' << std::setw(6) << imuData.acceleration[IMU_Z_INDEX] << ')';
    imuAccelString_ = wxString(accelOut.str().c_str(), wxConvUTF8);

    std::ostringstream velocityOut;
    velocityOut << std::fixed << std::showpos << std::setprecision(2) << '('
                << imuData.rotationalVelocity[IMU_ROLL_INDEX] << ',' << imuData.rotationalVelocity[IMU_PITCH_INDEX]
    ...
    orientationOut << std::fixed << std::showpos << std::setprecision(2) << '(' << imuData.orientation[IMU_ROLL_INDEX]
                   << ',' << imuData.orientation[IMU_PITCH_INDEX] << ',' << imuData.orientation[IMU_YAW_INDEX] << ')';
...
void LocalMetricPanelTextUpdater::handleData(const motion_state_t& motion, const std::string& channel)
{
    ...
    poseOut << std::fixed << std::showpos << std::setprecision(2) << '(' << set_to_zero_if_needed(motion.pose.x) << ','
            << set_to_zero_if_needed(motion.pose.y) << ',' << set_to_zero_if_needed(motion.pose.theta) << ')';
    poseString_ = wxString(poseOut.str().c_str(), wxConvUTF8);

    std::ostringstream velocityOut;
    velocityOut << std::fixed << std::showpos << std::setprecision(2) << '('
                << set_to_zero_if_needed(motion.velocity.linear) << ','
    ...
    leftOut << std::fixed << std::showpos << std::setprecision(2) << '('
            << set_to_zero_if_needed(motion.differentialWheels.leftWheel.speed) << ','
    ...
    rightOut << std::fixed << std::showpos << std::setprecision(2) << '('
             << set_to_zero_if_needed(motion.differentialWheels.rightWheel.speed) << ','
...
void LocalMetricPanelTextUpdater::handleData(const robot::commanded_velocity_t& command, const std::string& channel)
{
    ...
    out << std::fixed << std::showpos << std::setprecision(2) << '(' << command.linearVelocity << ','
