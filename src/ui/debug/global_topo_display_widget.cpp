...
void GlobalTopoDisplayWidget::setWidgetParams(const global_topo_display_params_t& params,
...
void GlobalTopoDisplayWidget::getViewportBoundary(Point<int>& bottomLeft, int& width, int& height)
{
    ...
    mapPreviewer->setBoundary(Point<int>(bottomLeft.x + width, bottomLeft.y),
                              size.GetWidth() * (1.0 - HYP_TREE_RATIO),
                              height);
}
...
void GlobalTopoDisplayWidget::renderWidget(void)
{
    ...
    auto mapToRender =
      (shouldShowBestMap || !treeOfMaps || treeOfMaps->empty()) ? &bestTopoMap : treeOfMaps->stateWithId(mapToShowId);
...
void GlobalTopoDisplayWidget::drawHypothesisTree(void)
{
    ...
    for (auto previewIt = previewContents.begin(), previewEnd = previewContents.end(); previewIt != previewEnd;
         ++previewIt) {
...
void GlobalTopoDisplayWidget::mouseRightUp(wxMouseEvent& event)
{
    ...
    if ((viewToShow == HYPOTHESIS_TREE) && (event.GetX() > size.GetWidth() * HYP_TREE_RATIO)) {
        grid_cell_t clickCell =
          mapPreviewer->getCellUnderCursor(Point<int>(event.GetX(), size.GetHeight() - event.GetY()));
