...
void MetricPlannerPanel::update(void)
{
    ...
    widgets_.numTrajectoriesLabel->SetLabel(wxString::Format(wxT("Trajectory Count (current): %u"), static_cast<uint32_t>(numTrjEvaluatedToShow)));
    ...
    widgets_.motionTargetCostText->SetLabel(wxString::Format(wxT("Evaluated Expected Cost: %7.4f"), motionTargetCostToShow));
...
{
    wxFileDialog loadDialog(widgets_.displayWidget, wxT("Select planner script file..."), wxT(""), wxT(""), wxT("*.spt"), wxFD_OPEN);
