...
void LocalMetricPanel::modeChanged(wxCommandEvent& event)
{
    ...
    std::shared_ptr<hssh::LocalMetricCommand> message =
      std::make_shared<hssh::SetSlamModeCommand>("DebugUI", mode, hssh::MappingMode::unchanged, res);
    consumer->sendMessage(message);

    std::shared_ptr<hssh::LocalMetricCommand> glassMessage =
      std::make_shared<hssh::ToggleGlassMapping>(useGlass, "DebugUI");
...
{
    std::shared_ptr<hssh::LocalMetricCommand> command =
      hssh::GlassEvaluationCommand::CreateSaveMapMessage("DebugUI", kGlassMapFilename);
...
void LocalMetricPanel::savePosesPressed(wxCommandEvent& event)
{
    ...
    {
        std::shared_ptr<hssh::LocalMetricCommand> command =
          hssh::GlassEvaluationCommand::CreateSavePosesMessage("DebugUI", saveDialog.GetPath().ToStdString());
        consumer->sendMessage(command);
    }
}
...
void LocalMetricPanel::saveScansPressed(wxCommandEvent& event)
{
    ...
    {
        std::shared_ptr<hssh::LocalMetricCommand> command =
          hssh::GlassEvaluationCommand::CreateSaveScansMessage("DebugUI", saveDialog.GetPath().ToStdString());
        consumer->sendMessage(command);
    }
}
