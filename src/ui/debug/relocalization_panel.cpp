...
void RelocalizationPanel::handleData(const hssh::local_metric_relocalization_debug_info_t& info, const std::string& channel)
{
    if(mode_ == kLocal)
    {
...
void RelocalizationPanel::sendLocalRelocalizationMessage (const std::shared_ptr<hssh::FilterInitializer>& initializer)
{
    ...
    std::shared_ptr<hssh::LocalMetricCommand> message = std::make_shared<hssh::RelocalizeInLpmCommand>("DebugUI", 
                                                                                                       *loadedLPM_, 
                                                                                                       initializer);
...
void RelocalizationPanel::sendRegionMessagePressed(wxCommandEvent& event)
{
    ...
    sendRelocalizationMessage(std::make_shared<hssh::RegionFilterInitializer>(initialRegion_, numSamples, posesPerPosition));
}
