...
public:
    ...
    */
    void setHandler(ObjectSelectorEventHandler<Shape>* handler)
    {
        handler_ = handler;
    }
...
private:
    ...
    {
        // The area of a shape is usually much larger than a pixel, which means that most of the time, the mouse will remain
        // inside the same shape as the mouse is dragged across the screen. Short-circuit the full search by opportunistically
        // checking that the mouse hasn't changed shapes
