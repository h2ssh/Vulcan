...
GLColor FIXED_COLORS[NUM_DEFINED_COLORS] = {GLColor(135, 206, 235, 255),
...
std::vector<GLColor> generate_colors(int numColors, float alpha, bool randomAlpha)
{
    ...
    for (int n = 0; n < numColors; ++n) {
        if (n < NUM_DEFINED_COLORS) {
            colors.emplace_back(FIXED_COLORS[n].red(),
                                FIXED_COLORS[n].green(),
                                FIXED_COLORS[n].blue(),
                                randomAlpha ? drand48() : FIXED_COLORS[n].alpha());
        } else {
