...
void gl_draw_line_ellipse  (const Point<float>& center, float xAxisRadius, float yAxisRadius, float width = 2.0f, int numSegments = 36);
...
void gl_draw_line_triangle  (const Point<float>& center, float width, float height, float orientation, float lineWidth = 2.0f);
...
void gl_draw_medium_arrow_polygon_line(const Point<float>& start, float length, float direction, float lineWidth = 2.0f);
...
void gl_draw_gaussian_distribution(const MultivariateGaussian& gaussian, float numSigma, const GLColor& color, float width = 2.0f);
