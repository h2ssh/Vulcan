...
LogplayerUI::LogplayerUI( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style ) : wxFrame( parent, id, title, pos, size, style )
{
	...
	logFileText = new wxStaticText( this, ID_LOGFILE_TEXT, wxEmptyString, wxDefaultPosition, wxDefaultSize, wxALIGN_CENTRE|wxST_NO_AUTORESIZE );
	logFileText->Wrap( -1 );
	...
	playbackSpeedText = new wxTextCtrl( this, ID_PLAYBACK_SPEED_TEXT, wxT("1.0"), wxDefaultPosition, wxDefaultSize, wxTE_CENTRE|wxTE_NO_VSCROLL|wxTE_PROCESS_ENTER );
