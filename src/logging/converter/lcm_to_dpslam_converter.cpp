...
bool save_dpslam_log(const std::string& filename, const log_contents_t& contents);
...
bool convert_lcm_to_dpslam(const std::string& filename, bool useOldLogFormat)
{
    ...
    std::cout << "INFO: Read " << logContents.laserScans.size() << " scans and " << logContents.odometry.size()
              << " odometry readings." << std::endl;
...
log_contents_t read_lcm_new_log(::lcm::LogFile& log)
{
    ...
    while ((event = log.readNextEvent())) {
        try {
            ...
            std::cerr << "EXCEPTION: Failed to unserialize message on channel: " << event->channel
...
bool save_dpslam_log(const std::string& filename, const log_contents_t& contents)
{
    ...
    for (std::size_t n = 0; n < contents.laserScans.size(); ++n) {
        ...
        for (std::size_t i = contents.firstLaserIndex; i < contents.lastLaserIndex - kRayIncrement;
             i += kRayIncrement) {
            out << ' ' << std::abs(scan.ranges[i]) << ' '
                << wrap_to_pi(scan.offset.theta + scan.startAngle + scan.angularResolution * i);
        }
...
void print_log_stats(log_contents_t& log)
{
    ...
    for (std::size_t n = 1; n < poseDiffs.size(); ++n) {
        ...
        if (poseDiffs[n].theta > M_PI / 4) {
            std::cout << "Big pose diff found! Erasing value at N = " << n << " Prev: " << log.scanPoses[n - 1]
                      << " Curr: " << log.scanPoses[n] << '\n';
...
void prune_scans_without_odometry(log_contents_t& contents)
{
    ...
    std::cout << "Initial time: " << contents.laserScans.front().timestamp
              << " Odom: " << contents.odometry.front().timestamp << '\n';
    std::cout << "Final time: " << contents.laserScans.back().timestamp
              << " Odom: " << contents.odometry.back().timestamp << '\n';
...
pose_t pose_at_time(int64_t timestamp, const std::vector<odometry_t>& odometry)
{
    ...
    for (std::size_t n = 1; n < odometry.size(); ++n) {
        if (odometry[n - 1].timestamp <= timestamp && odometry[n].timestamp >= timestamp) {
            return interpolate_pose(
              pose_t(odometry[n - 1].timestamp, odometry[n - 1].x, odometry[n - 1].y, odometry[n - 1].theta),
              pose_t(odometry[n].timestamp, odometry[n].x, odometry[n].y, odometry[n].theta),
              timestamp);
        }
    }
    ...
    assert(false);
    return pose_t(odometry.back().timestamp, odometry.back().x, odometry.back().y, odometry.back().theta);
}


pose_t pose_diff(const pose_t& lhs, const pose_t& rhs)
{
    return pose_t(std::abs(lhs.x - rhs.x), std::abs(lhs.y - rhs.y), std::abs(angle_diff(lhs.theta, rhs.theta)));
}
