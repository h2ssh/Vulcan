...
void display_help_if_needed(const utils::CommandLine& commandLine)
{
    bool needHelp = commandLine.argumentExists(HELP_SHORT) || commandLine.argumentExists(HELP_LONG)
      || !commandLine.argumentExists(LOG) || !commandLine.argumentExists(FORMAT) || !commandLine.argumentExists(CONFIG);
    ...
    if (needHelp) {
        std::cout
          ...
          << "  --config-file 'filename'    Filename with configuration for logplayer -- data channels and the like\n"
          << "   --speed 'value'             Playback speed for the log (>1 = faster, <1 = slower, default = 1.0) "
             "(optional)\n";
