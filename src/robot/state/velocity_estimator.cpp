...
VelocityEstimator::VelocityEstimator(const velocity_estimator_params_t& params) : params_(params), initialized_(false)
{
}


velocity_t VelocityEstimator::updateEstimate(const motion_state_input_t& input,
                                             const pose_distribution_t& estimatedPose)
{
    updatePoseWindow(estimatedPose);

    auto estimate = input.odometry.empty() ? slidingWindowVelocity() : odometryVelocity(input.odometry);

    // If there is IMU data, then use the most recent angular velocity measurement directly. It will be a better
    // estimate than either the pose differentiation or the odometry.
    if (!input.imu.empty()) {
...
{
    // The sliding window simply averages the linear and angular motion over a small span of time to determine the
...
velocity_t VelocityEstimator::odometryVelocity(const std::vector<odometry_t>& odometry)
{
    ...
    if (totalTime > 0.0) {
        ...
        velocity.linear = previousVelocityEstimate_.linear
          + params_.odometryFilterGain * (linearVelocity - previousVelocityEstimate_.linear);
    }
