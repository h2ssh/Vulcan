...
struct motor_command_t
{
    ...
    motor_command_t(void) : leftMotor(0.0), rightMotor(0.0) { }

    motor_command_t(const commanded_velocity_t& commandedVelocity,
                    const differential_motors_plant_params_t& robotParams)
    {
        wheel_speeds_t desiredSpeeds(commandedVelocity.linearVelocity,
                                     commandedVelocity.angularVelocity,
                                     robotParams.wheelbase);
    ...
    motor_command_t(const commanded_joystick_t& commandedJoystick,
                    const differential_motors_plant_params_t& robotParams)
    {
    ...
    double convert_speed_to_command(double speed, const differential_motors_plant_params_t& robotParams)
    {
        ...
        if (std::abs(speed) > 0.005) {
            command = robotParams.motorBeta / robotParams.motorAlpha * speed
              + std::copysign(robotParams.motorMu * robotParams.motorGamma / robotParams.motorAlpha, speed);
        }
    ...
    {
        return (forwardJoystickPosition == 0)
          ? robotParams.turnRateInPlace
          : robotParams.turnRateBase * (1.0 - robotParams.turnReductionRate * abs(forwardJoystickPosition));
    }
};
...
DriveWheelEstimator::DriveWheelEstimator(const drive_wheel_estimator_params_t& params,
                                         const differential_motors_plant_params_t& robotParams)
: params_(params)
...
differential_drive_wheels_t DriveWheelEstimator::updateEstimate(const motion_state_input_t& input,
                                                                const velocity_t& estimatedVelocity)
{
    // observation
    ...
    if (!input.commandedJoysticks.empty()) {
        ...
        auto fakeCommand =
          commanded_velocity_t(estimatedVelocity.linear, estimatedVelocity.angular, estimatedVelocity.timestamp);
    ...
    previousLeftWheel_ = applyFilterToWheelSpeed(previousLeftWheel_,
                                                 observation.leftWheel,
                                                 motorCommands.leftMotor,
                                                 observation.time,
    ...
    previousRightWheel_ = applyFilterToWheelSpeed(previousRightWheel_,
                                                  observation.rightWheel,
                                                  motorCommands.rightMotor,
                                                  observation.time,
                                                  observation.timestamp,
                                                  &rightWheelFriction);
...
DriveWheelEstimator::wheel_speeds_observation_t
  DriveWheelEstimator::calculateWheelObservation(const motion_state_input_t& input, const velocity_t& estimatedVelocity)
{
    ...
    if (!input.encoders.empty()) {
        ...
        observation.timestamp =
          previousVelocity_.timestamp + (estimatedVelocity.timestamp - previousVelocity_.timestamp) / 2;
    }
    ...
    if (!input.imu.empty()) {
        ...
        if (fabs(imuAngularVelocity - encoderAngularVelocity) > 0.1) {
            std::cout << "Warning!!: DriveWheelEstimator: Possible wheel slip detected: RotationalVelocity (rad/s): "
                      << "IMU: " << imuAngularVelocity << ", Encoder: " << encoderAngularVelocity << '\n';
...
drive_wheel_t DriveWheelEstimator::applyFilterToWheelSpeed(const drive_wheel_t& previousState,
                                                           ...
                                                           double* friction)
{
    ...
    auto predictedWheelState = motor_model(previousState,
                                           motorCommand,
                                           timeDelta,
                                           robotParams_.motorMu,
                                           robotParams_.motorBeta,
                                           robotParams_.motorGamma,
                                           robotParams_.motorAlpha,
                                           friction);
