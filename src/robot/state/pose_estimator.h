...
private:
    ...
    // INVARIANT: predictedPoses_[0] is the result of applying the motion model using predictionInput_[0] to
    // filteredPose_ INVARIANT: predictedPoses_[n] is the result of applying the motion model using predictionInput_[n]
    // to predictedPoses_[n-1]
    ...
    pose_distribution_t
      predictNextPose(const pose_distribution_t& previous, const odometry_t& previousOdom, const odometry_t& nextOdom);
