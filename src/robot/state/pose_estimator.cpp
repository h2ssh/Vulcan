...
pose_estimator_params_t::pose_estimator_params_t(const utils::ConfigFile& config)
: odomParams(config)
{
}
...
void PoseEstimator::addNewOdometryMeasurements(const motion_state_input_t& measurements)
{
    ...
    for(auto& odom : measurements.odometry)
    {
        ...
        {
            std::cout << "WARNING! PoseEstimator: Odometry must come after previous prediction. Odom time:" 
                << odom.timestamp << " Prediction time:" << predictionInput_.back().timestamp << " Diff:" 
                << (odom.timestamp - predictionInput_.back().timestamp) 
...
void PoseEstimator::predictPosesFromOdometry(void)
{
    ...
    {
        predictedPoses_.push_back(predictNextPose(predictedPoses_[n-1], predictionInput_[n-1], predictionInput_[n]));
    }
}
...
void PoseEstimator::adjustPosesUsingLocalizedPoses(void)
{
    ...
    while(!localizedPoses_.empty() && !predictionInput_.empty())
    {
        ...
        {
            std::cout << "No data before:" << localizedPoses_.front().timestamp << " Earliest:"
                << predictionInput_.front().timestamp << " Delta: "
                << (localizedPoses_.front().timestamp - predictionInput_.front().timestamp) << '\n';
...
bool PoseEstimator::incorporateLocalizedPose(const pose_distribution_t& localizedPose)
{
    ...
    {
        std::cout << "No odometry after the localized pose. Time:" << localizedPose.timestamp << " Input:"
            << predictionInput_.back().timestamp << '\n';
        return false;
    }
...
std::size_t PoseEstimator::findIndexAfterTime(int64_t observationTime)
{
    ...
    {
        if(predictionInput_[n-1].timestamp <= observationTime &&
           predictionInput_[n].timestamp >= observationTime)
        {
            observationIndex = n;
            break;
        }
                                                         ...
                                                         const pose_distribution_t& localizedPose)
{
    ...
    Matrix innovation  = predictedPose.uncertainty.getCovariance() * arma::inv(predictedPose.uncertainty.getCovariance() + localizedPose.uncertainty.getCovariance());
    ...
    filteredPose.uncertainty.setDistributionStatistics(filteredMean,
                                                        (arma::eye<Matrix>(3, 3) - innovation) * predictedPose.uncertainty.getCovariance());
    
    filteredPose.timestamp = localizedPose.timestamp;
    
#ifdef DEBUG_FILTER
    std::cout << "DEBUG::PoseEstimator: Kalman filter results:\n"
              << "Prediction:\n"  << predictedPose.uncertainty.getMean() << predictedPose.uncertainty.getCovariance()
              << "Observation:\n" << localizedPose.uncertainty.getMean() << localizedPose.uncertainty.getCovariance()
