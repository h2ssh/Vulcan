...
DifferentialTorquePlant::DifferentialTorquePlant(const differential_torque_plant_params_t& params) : params_(params)
{
}


motion_state_t
  DifferentialTorquePlant::nextState(const motion_state_t& state, const motion_command_t& command, float timestep)
{
    ...
    wheelAccelerations.right =
      fmin(fabs(desiredWheelSpeeds.right - currentWheelSpeeds.right) / timestep, params_.wheelAccelMax);
    wheelAccelerations.right = copysign(wheelAccelerations.right, desiredWheelSpeeds.right - currentWheelSpeeds.right);

    wheelAccelerations.left =
      fmin(fabs(desiredWheelSpeeds.left - currentWheelSpeeds.left) / timestep, params_.wheelAccelMax);
    ...
    updatedState.acceleration =
      acceleration_t(robotAccelerations.linear, robotAccelerations.angular);   // convert to proper type
...
{
    return velocity_t((right + left) / 2, (right - left) / wheelbase);
}
