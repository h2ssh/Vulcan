...
DifferentialMotorsPlant::DifferentialMotorsPlant(const differential_motors_plant_params_t& params)
: params_(params)
{
}


motion_state_t DifferentialMotorsPlant::nextState(const motion_state_t& currentState, const motion_command_t& command, float timestep)
{
    differential_drive_wheels_t updatedWheelState = nextWheelState(currentState.differentialWheels, command.joystickCommand, timestep);
    ...
    velocity_t updatedVelocity( (updatedWheelState.rightWheel.speed + updatedWheelState.leftWheel.speed)*0.5,
                                (updatedWheelState.rightWheel.speed - updatedWheelState.leftWheel.speed)/params_.wheelbase );
...
differential_drive_wheels_t DifferentialMotorsPlant::nextWheelState(const differential_drive_wheels_t& currentState, const joystick_command_t& joystickCommand, float timestep)
{
    differential_drive_wheels_t updatedState;

    // convert given joystick commands to right and left motor commands
    double turnRate = (joystickCommand.forward == 0) ? params_.turnRateInPlace : params_.turnRateBase*(1 - params_.turnReductionRate*fabs(joystickCommand.forward));
    ...
    updatedState.rightWheel = motor_model(currentState.rightWheel, rightMotorCommand, timestep, params_.motorMu, params_.motorBeta, params_.motorGamma, params_.motorAlpha, nullptr);
    updatedState.leftWheel  = motor_model(currentState.leftWheel,  leftMotorCommand,  timestep, params_.motorMu, params_.motorBeta, params_.motorGamma, params_.motorAlpha, nullptr);
