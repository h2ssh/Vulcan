...
InverseOdometryModel::InverseOdometryModel(const odometry_model_params_t& params)
: params_(params)
{
}
                                                      ...
                                                      const motion_model_data_t& currentData)
{
    ...
    double rot1  = angle_diff(std::atan2(currentOdom.y - previousOdom.y, currentOdom.x - previousOdom.x), previousOdom.theta);
    ...
    predictedPose.uncertainty.setDistributionStatistics(Vector({predictedPose.x, predictedPose.y, predictedPose.theta}),
                                                        gT*previousPose.uncertainty.getCovariance()*gT.t() + vT*mT*vT.t());  // Non-linear uncertainty propagation for EKF update
