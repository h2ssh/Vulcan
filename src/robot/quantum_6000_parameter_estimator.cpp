...
/**
 * quantum_6000_parameter_estimator is used to estimate the mapping between joystick position and angular velocity
 * command.
 *
 * To find the parameter, a big chunk of data is consumed. The test runs all commands between -100 and 100 for a given
 * amount of time. Angular velocity is measured from the IMU
 *
 * 10-26-2012 Ron Gaynier -- The controller within the Quantum 6000 is manufactured by Curtis Instruments. The Curtis
 * documentation on the controller reveals there are several selectable parameters that influence the behavior of X
 * (angular velocity) relative to Y (forward velocity). In order to better understand this relationship of X at various
 * values of Y, and additional test has been added, CIRCLE_XY. This test is only intended to be run while the wheelchair
 * drive wheels are off the ground and with an additional PC running the Curtis PC Programmer and logging X Y and Left
 * Right RPMs. The function that accomplishes this is run_circleXY. The function is a hack of run_circle and I hope I've
 * commented it sufficiently for anyone to understand how it is suppose to work.
 *
 ...
 *   --ramp-time        'ms'          Milliseconds to wait for the speed to stabilize before beginning to acquire
 * samples
...
void display_help_if_needed(const utils::CommandLine& commandLine)
{
    bool helpNeeded = !commandLine.argumentExists(JOYSTICK_PORT) || !commandLine.argumentExists(CONTROLLER_PORT)
      ...
      || !(commandLine.argumentExists(TURN_IN_PLACE) || commandLine.argumentExists(CIRCLE)
           || commandLine.argumentExists(CIRCLE_XY) || commandLine.argumentExists(CIRCLE_LINEAR));
    ...
    if (helpNeeded) {
        std::cout << "The command-line parameters for quantum_6000_parameter_estimation are:\n"
                  ...
                  << "--controller-port  'port'        CANBus port to which the controller is connected\n"
                  << "--imu-port         'port'        Port for the IMU\n"
                  << "--ramp-time        'ms'          Milliseconds to wait for the speed to stabilize before "
                     "beginning to acquire samples\n"
                  << "--samples          'number'      Number of samples to acquire for each command\n"
                  << "--output-file      'filename'    File in which the data should be written\n"
                  << "--turn-in-place                  Run the turn-in-place test\n"
                  << "--circle                         Run the drive in a circle test\n"
...
void run_test(test_configuration_t& config, std::vector<test_sample_t>& samples)
{
    ...
    for (int n = config.testSamples; --n >= 0;) {
        ...
        if (utils::system_time_us() < startTime) {
            ...
            // The IMU on the Quantum6000 is mounted so turning left shows a negative angular velocity, so the sign
            // needs to be flipped
...
{
    std::cout
      << "This is a special version of the circle test to evaluate the Curtis control algorithm implementation"
    ...
    // After setting the XY command to zero/zero intially, from MIN_FORWORD_VELOCITY conduct a set of tests for
    // increasing Y. Y value is increased by 2 point increments until MAX_FORWARD_VELOCITY is reached
    ...
    for (int y = MIN_FORWARD_VELOCITY; y <= MAX_FORWARD_VELOCITY; y += 2) {
        ...
        // For the value of Y established in the for loop above, ramp x from the Minimum Angular Velocity to the Max
        // Angular Velocity
        ...
        // Do the same thing as in the for loop just proceeding, but now in the opposite direction, to determine if
        // there a difference between left and right turns.
        for (int x = MIN_ANGULAR; x <= MAX_ANGULAR; x += 1) {
