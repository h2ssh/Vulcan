...
void run_square_wave(const square_wave_t& wave,
...
void display_help_if_needed(const vulcan::utils::CommandLine& commandLine)
{
    bool helpNeeded = commandLine.argumentExists(HELP_SHORT) || commandLine.argumentExists(HELP_LONG)
      || !commandLine.argumentExists(CHANNEL) || !commandLine.argumentExists(LINEAR)
      || !commandLine.argumentExists(ANGULAR) || !commandLine.argumentExists(DURATION)
...
void run_square_wave(const square_wave_t& wave,
                     ...
                     const std::string& channel)
{
    std::cout << "Running square wave with: (" << wave.linear << ',' << wave.angular
              << "), Duration=" << (wave.duration / 1000000) << " seconds, Iterations=" << wave.iterations << '\n';
