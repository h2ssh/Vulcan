...
void display_message_counts(const std::map<uint8_t, int>& joystickMessages,
                            const std::map<uint8_t, int>& controllerMessages);
...
Quantum6000::Quantum6000(const wheelchair_joystick_calibration_t& calibration,
...
joystick_command_t Quantum6000::convertVelocityCommandToJoystickCommand(const velocity_command_t& command) const
{
    ...
    if (fabs(desiredRightWheelSpeed) < 0.005)   // if desired speed steady state is very small
    {
        ...
        rightMotorCommand = MOTOR_BETA / MOTOR_ALPHA * desiredRightWheelSpeed
          + copysign(MOTOR_MU * MOTOR_GAMMA / MOTOR_ALPHA, desiredRightWheelSpeed);
    }
    ...
    if (fabs(desiredLeftWheelSpeed) < 0.005)   // if desired speed steady state is very small
    {
        ...
        leftMotorCommand = MOTOR_BETA / MOTOR_ALPHA * desiredLeftWheelSpeed
          + copysign(MOTOR_MU * MOTOR_GAMMA / MOTOR_ALPHA, desiredLeftWheelSpeed);
    }

    // motor commands to joystick commands
    int16_t joystickForward = static_cast<int16_t>((rightMotorCommand + leftMotorCommand) / 2.0);

    float turnRate = (joystickForward == 0) ? TURN_RATE_IN_PLACE
                                            : TURN_RATE_BASE - TURN_REDUCTION_RATE * (fabs(float(joystickForward)));
    int16_t joystickLeft = static_cast<int16_t>(rightMotorCommand - leftMotorCommand) / 2.0 / turnRate;

    //     std::cout<<"Motion:"<<command.linearVelocity<<','<<command.angularVelocity<<"
    //     Joystick:"<<joystick.x<<','<<joystick.y<<'\n';

    return joystick_command_t(clamp_joystick_position(joystickForward),
                              clamp_joystick_position(joystickLeft),
                              JOYSTICK_GAIN);
}
...
void Quantum6000::handleJoystickBus(bool canUseRobotCommand)
{
    ...
    if (is_joystick_position_message(internalJoystickMessage.Msg)) {
        std::cout << "Joystick: Human:(" << humanJoystickCommand.forward << ',' << humanJoystickCommand.left << ','
                  << humanJoystickCommand.gain << ")\n"
                  << "          Robot:(" << robotJoystickCommand.forward << ',' << robotJoystickCommand.left << ','
                  << robotJoystickCommand.gain << ")\n";
    }
#endif
}
                            ...
                            const std::map<uint8_t, int>& controllerMessages)
{
    ...
    for (auto controlIt = controllerMessages.begin(), controlEnd = controllerMessages.end(); controlIt != controlEnd;
         ++controlIt) {
...
void print_message(const TPCANMsg& message)
{
    std::cout << (int)message.ID << '\t' << std::dec << (short)message.MSGTYPE << '\t' << (short)message.LEN << '\t'
              << (short)message.DATA[0] << '\t' << (short)message.DATA[1] << '\t' << (short)message.DATA[2] << '\t'
              << (short)message.DATA[3] << '\t' << (short)message.DATA[4] << '\t' << (short)message.DATA[5] << '\t'
              << (short)message.DATA[6] << '\t' << (short)message.DATA[7] << '\n';
}
