...
std::unique_ptr<Wheelchair> create_wheelchair(const wheelchair_params_t& params)
{
    if (params.wheelchairType == robot::QUANTUM_6000_TYPE) {
        return std::unique_ptr<Wheelchair>(
          new robot::Quantum6000(params.calibration, params.controllerPort, params.joystickPort));
    } else if (params.wheelchairType == robot::INVACARE_ATM_TYPE) {
        return std::unique_ptr<Wheelchair>(
          new robot::InvacareATM(params.calibration, params.joystickPort, params.controllerPort));
    }
...
Wheelchair::Wheelchair(void) : humanIsDriver(true)
{
}
...
void Wheelchair::setMotorCommandTarget(const motion_command_t& command)
{
    ...
    //    std::cout<<"WHEELCHAIR: Setting robot joystick command:
    //    ("<<robotJoystickCommand.forward<<','<<robotJoystickCommand.left<<")\n";
}
...
commanded_joystick_t Wheelchair::updateCommandedJoystick(void)
{
    ...
    if (humanIsDriver) {
        commandedJoystick =
          commanded_joystick_t(humanJoystickCommand.forward, humanJoystickCommand.left, humanJoystickCommand.timestamp);
    } else {
        commandedJoystick =
          commanded_joystick_t(robotJoystickCommand.forward, robotJoystickCommand.left, robotJoystickCommand.timestamp);
    }
...
motion_command_t Wheelchair::getWheelchairMotionCommand(void) const
{
    ...
    //    std::cout<<"WHEELCHAIR: Human joystick command:
    //    ("<<humanJoystickCommand.x<<','<<humanJoystickCommand.y<<")\n";

    commanded_velocity_t commandedVelocity = convertJoystickCommandToCommandedVelocity(humanJoystickCommand);
    velocity_command_t velocityCommand(commandedVelocity.linearVelocity,
                                       commandedVelocity.angularVelocity,
                                       humanJoystickCommand.timestamp);
...
int Wheelchair::run(void)
{
    while (true) {
        ...
        bool canUseRobotCommand = isJoystickCentered(humanJoystickCommand)
          && (utils::system_time_us() - commandRecieved.timestamp < COMMAND_TIMEOUT_US);
