...
public:
    ...
    /**
     * getWheelchairMotionCommand retrieves the metric velocity and joystick command associated with the current
     * joystick position. A non-zero velocity indicates a manual command is currently being issued to the wheelchair.
     *
...
protected:
     ...
     */
    virtual commanded_velocity_t
      convertJoystickCommandToCommandedVelocity(const joystick_command_t& joystick) const = 0;
