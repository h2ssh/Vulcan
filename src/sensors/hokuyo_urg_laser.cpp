...
, lastLaserTimestamp(0)
{
    ...
    scanPeriod = (60.0 / laserParams.scan_rpm) * (laserParams.area_max - laserParams.area_min) / static_cast<float>(laserParams.area_total);
}
...
bool HokuyoURGLaser::getLaserScan(polar_laser_scan_t& scan)
{
    ...
    scan.angularResolution = 2.0*M_PI / laserParams.area_total; // area_total = # readings/360degrees. weird name for that
