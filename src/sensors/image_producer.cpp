 ...
 *   --publish-period-ms 'ms'    Number of milliseconds between publishing frames (optional, default = 0, i.e. all
 * frames)
 */
...
void display_help_if_needed(const utils::CommandLine& commandLine)
{
    ...
    if (helpNeeded) {
        std::cout << "The command-line arguments for the image_producer are:\n"
                  ...
                  << "   --output-basename 'name'    Base name of output images. Images will be written in format: "
                     "basenameXXXX.png"
                  << "   --publish-period-ms 'ms'    Number of milliseconds between publishing frames (optional, "
                     "default = 0, i.e. all frames)"
                  << '\n';
        exit(1);
    }
}
