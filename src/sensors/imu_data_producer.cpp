...
void produce_imu_data(boost::shared_ptr<IMU> imu,
                      boost::shared_ptr<ModuleCommunicator> transmitter,
...
bool display_help_message_if_needed(const CommandLine& commandLine)
{
    ...
    if (needHelp) {
        std::cout << "imu_data_producer: \n"
                  ...
                  << '\n'
                  << "    -h, --help                  Print this message\n"
                  << "    --port 'IMU port'           Port the device is connected to (supplied to driver) : Default = "
                  << DEFAULT_IMU_PORT << '\n'
                  << "    --imu-model 'IMU type'      IMU driver to use : Default = " << DEFAULT_IMU_MODEL << '\n'
                  << "            Currently supported IMUs:\n"
                  << "                3DM-GX2\n"
                  << "                3DM-GX3\n"
                  << "    --channel 'name'            Name of the output channel for the data : Default = "
                  << DEFAULT_CHANNEL << '\n'
                  << "    --bias-samples 'number'     Number of samples to take for estimating the acceleration vector "
                     ": Default = "
                  << DEFAULT_BIAS_SAMPLES << '\n'
                  << std::endl;
    }
...
{
    std::string imuModel =
      commandLine.argumentExists(IMU_MODEL) ? commandLine.argumentValue(IMU_MODEL) : DEFAULT_IMU_MODEL;
...
boost::shared_ptr<IMU> load_microstrain_3dmgx2_driver(const CommandLine& commandLine)
{
    ...
    return boost::shared_ptr<IMU>(new vulcan::sensors::Microstrain3DMGX2(imuPort, gx2TicksPerSecond));
}
...
void estimate_gravity(boost::shared_ptr<IMU> imu, const CommandLine& commandLine)
{
    std::string biasString =
      commandLine.argumentExists(BIAS_SAMPLES) ? commandLine.argumentValue(BIAS_SAMPLES) : DEFAULT_BIAS_SAMPLES;
