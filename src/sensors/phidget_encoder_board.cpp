...
encoder_data_t PhidgetEncoderBoard::getEncoders(void)
{
    ...
    // If the time hasn't changed between pulls of the encoder data, then update the timestamp manually. Otherwise, go
    // with the time that was set by the event handler, which will be closer to the time that the data actually arrived.
                            ...
                            int relativePosition)
{
    ...
    #ifdef DEBUG_TIME
    std::cout << "DEBUG:PhidgetEncoderBoard: Time:" << time << " Position change:" << relativePosition
              << " Left time:" << board->totalLeftEncoderTime_ << " Right time:" << board->totalRightEncoderTime_
              << " Delta:" << (board->totalRightEncoderTime_ - board->totalLeftEncoderTime_)
              << " Timestamp:" << board->encoderData.timestamp << '\n';
    #endif

    // Left and right wheel encoders are read in alternating order. Wait for both before updating by just triggering on
    // the left wheel.
    if (index == board->params.leftIndex) {
