...
void print_generic_error(
  tPvErr error);   // many errors are shared between functions, so shared messages are displayed with this function
...
bool AVTCamera::getNextImage(Image& image)
{
    ...
    // The frame has been processed, so add it to the back of the queue and increment the index at the front of the
    // queue
