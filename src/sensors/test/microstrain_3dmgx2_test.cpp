...
int main(int argc, char** argv)
{
    ...
    while (1) {
        ...
        if (deltaTime > vulcan::utils::sec_to_usec(1)) {
            ...
            std::cout << "Output rate: " << (static_cast<float>(numReadings) * vulcan::utils::usec_to_sec(deltaTime))
                      << " Hz"
...
void display_imu_data(const imu_data_t& imuData)
{
    std::cout << "IMU: Acceleration: (" << imuData.acceleration[IMU_X_INDEX] << ", "
              << imuData.acceleration[IMU_Y_INDEX] << ", " << imuData.acceleration[IMU_Z_INDEX] << ")\n"
              << "     Rotation vel: (" << imuData.rotationalVelocity[IMU_X_INDEX] << ", "
              << imuData.rotationalVelocity[IMU_Y_INDEX] << ", " << imuData.rotationalVelocity[IMU_Z_INDEX] << ")\n"
              << "     Orientation:  (" << imuData.orientation[IMU_ROLL_INDEX] << ", "
              << imuData.orientation[IMU_PITCH_INDEX] << ", " << imuData.orientation[IMU_YAW_INDEX] << ")" << std::endl;
}
