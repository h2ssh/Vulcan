...
public:
     ...
     */
    Microstrain3DMGX2(const std::string& device, int64_t ticksPerSecond = 19660800);
    ...
    /**
     * captureBias captures the current gyro-bias for the Microstrain3DMGX2. Capturing the bias for the
     * Microstrain3DMGX2 makes good things happen whenever the robot has decided to enter the magical, spin-in-place
     * like Thelonius Monk thing. At the same time, the yaw reading from the Microstrain3DMGX2 will jump to the
     * 'correct' value which could cause problems with odometry. At the least, the bias should be captured when the
     * robot is booting up. It should only be called when the robot is stationary though.
     *
