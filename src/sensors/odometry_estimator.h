...
* approach. The traditional method of estimating odometry is using wheel encoders, though scan matching or visual feature
* matching can also be used. This interface is minimal and simply provides the method for any of these odometry estimation
* approaches to be integrated with Vulcan.
*
* The initialization should subscribe to any necessary data and initialize internal state. The update should provide the most
* recently updated odometry. The method can either copy a previously calculated value, or do the calculation at that point.
* Finally, the send method will send out the odometry and any other calculated state using the normal data transmitter.
* At bare minimum, odometry data should be sent out.
*/
class OdometryEstimator
{
public:
    ...
    */
    ...
    /**
    * update provides an updated estimate of the robot's odometry. The provided odometry estimate needs to be new data. If
    * no new data has been calculated, the call to update should block until data is available.
    *
    ...
    */
    ...
    /**
    * send sends out the calculated odometry data, along with any other state being estimated, like wheel encoder information.
    *
