...
struct motion_state_t
{
    ...
    motion_state_t(void) : timestamp(0) { haveDistribution = haveWheels = false; }

    motion_state_t(const pose_t& pose, const velocity_t& velocity)
    ...
    motion_state_t(const pose_distribution_t& poseDistribution, const velocity_t& velocity)
    ...
    motion_state_t(const pose_t& pose, const velocity_t& velocity, const differential_drive_wheels_t& wheels)
    : timestamp(pose.timestamp)
    , pose(pose)
    , velocity(velocity)
