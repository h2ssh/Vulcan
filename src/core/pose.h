...
struct pose_t
{
    ...
    pose_t(void) : timestamp(0), x(0), y(0), theta(0) { }

    pose_t(float x, float y, float theta) : timestamp(0), x(x), y(y), theta(theta) { }

    explicit pose_t(position_t position, float theta = 0.0f) : timestamp(0), x(position.x), y(position.y), theta(theta)
    {
    }
...
struct pose_6dof_t
{
    ...
    pose_6dof_t(const pose_t& pose) : pose_6dof_t(pose.timestamp, pose.x, pose.y, 0.0f, 0.0f, 0.0f, pose.theta) { }
