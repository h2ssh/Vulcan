...
Image::Image(void) : width(0), height(0), cspace(RGB), pixels(0)
{
}


Image::Image(unsigned int w, unsigned int h, Colorspace cs) : width(w), height(h), cspace(cs)
{
    if (cspace != MONO) {
...
, cspace(cs)
{
    if (copy) {
        ...
        pixels = pix;   // NOTE: This is bad and dangerous because deleting pix causes a dangling reference in the image
                        // and instant segfault on next access
    }
}


Image::Image(const Image& i) : width(i.width), height(i.height), cspace(i.cspace)
{
...
{
    // Image uses a lazy evaluation scheme, so RGB values may not have been calculated, if not, do that and then get the
    // value
