...
struct Line
{
    ...
    Line(const Point<T>& pointA, const Point<T>& pointB) : a(pointA), b(pointB) { }

    Line(T pointAX, T pointAY, T pointBX, T pointBY) : a(pointAX, pointAY), b(pointBX, pointBY) { }

    // Constructor for creating a line from a line using a different type
    template <typename U>
    Line(const Line<U>& copy) : a(copy.a)
                              , b(copy.b)
    {
    }
...
Line<T> normal(const Line<T>& line)
{
    ...
    return Line<T>(center, Point<T>(center.x - (line.b.y - line.a.y), center.y + (line.b.x - line.a.x)));
}
...
bool right_of_line(const Line<T>& line, const Point<U>& p)
{
    ...
    return (m < 0) ? (p.y - line.a.y) > m * (p.x - line.a.x) : (p.y - line.a.y) < m * (p.x - line.a.x);
}
...
{
    double angle =
      angle_diff(atan2(left.b.y - left.a.y, left.b.x - left.a.x), atan2(right.b.y - right.a.y, right.b.x - right.a.x));
...
{
    line_intersection_data_t(const Line<T>& lineA, const Line<T>& lineB) : lineA(lineA), lineB(lineB)
    {
        /*
         * The following intersection algorithm is pulled from: http://www.faqs.org/faqs/graphics/algorithms-faq/
         * Section 1.03
         *
    ...
    bool linesIntersect(void) const { return den != 0; }

    bool segmentsIntersect(void) const
    {
        return linesIntersect() && ((rNum / den) >= -0.0000001) && ((rNum / den) <= 1.0000001)
          && ((sNum / den) >= -0.0000001) && ((sNum / den) <= 1.0000001);
    }
...
bool line_segments_intersect(const Line<T>& lineA, const Line<U>& lineB)
{
    ...
    // If den == 0, then see if the x-projection of the lines overlap at all, i.e. collinear lines -- yes, very henious
    // looking equation
    return data.segmentsIntersect()
      || ((data.den == 0)
          && ((lineA.a.x >= lineB.a.x && lineA.a.x <= lineB.b.x) || (lineA.b.x >= lineB.a.x && lineA.b.x <= lineB.b.x)
