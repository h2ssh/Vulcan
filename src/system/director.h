...
public:
     ...
     */
    TimeTriggeredDirector(int periodMs) : periodUs_(periodMs * 1000), updateStartTimeUs_(0) { }
    ...
    TriggerStatus waitForTrigger(void) override
    {
        ...
        if (timeSinceStartUs < periodUs_) {
            ...
            std::cerr << "WARNING::TimeTriggeredDirector: Missed expected period: Was: " << (timeSinceStartUs / 1000)
                      << "ms. Desired: " << (periodUs_ / 1000) << "ms.\n";
        }
