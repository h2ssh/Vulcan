...
/**
* message_traits defines the traits that control how a message is sent to other modules. The traits for a message can be divided
* into two categories: the type of message and the channel(s) on which it is sent.
* 
...
struct message_traits
{
    ...
    enum { num_channels = 0 };      ///< num_channels specifies the number of channels that exist for the type. The majority of messages
...
// For most messages, the only thing that changes is the channel name. These macros will define the message_traits structs
// using the specified class and channel name
...
{                                                                                           \
    using type = tag;                                                                       \
                                                                                            \
    enum { num_channels = BOOST_PP_TUPLE_SIZE(channels) };                                  \
                                                                                            \
    static std::string channelName(int index)                                               \
    {                                                                                       \
...
#define DEFINE_SYSTEM_MESSAGE(MessageType, channels) DEFINE_SERIALIZATION_MSG_TRAITS(MessageType, channels, system_message_tag)
#define DEFINE_DEBUG_MESSAGE(MessageType, channels) DEFINE_SERIALIZATION_MSG_TRAITS(MessageType, channels, debug_message_tag)
