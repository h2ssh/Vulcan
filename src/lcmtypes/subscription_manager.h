...
{
    channel_subscriber_t(const std::string& channel, void* userdata, void (*callback_func)(const vulcan_type&, const std::string&, void*)) :
            channel(channel), userdata(userdata), callback(callback_func)
    {

    }

    std::string channel;
    void*       userdata;//this is the custom pointer, prob. to a DataConsumer, that will be given back to callaback's last argument
...
template <typename lcm_type, typename vulcan_type>
void subscription_manager_callback(const lcm_recv_buf_t *rbuf,
                                   const char *channel, const lcm_type *msg, void *user)
{
    SubscriptionManager<lcm_type, vulcan_type>* manager = static_cast<SubscriptionManager<lcm_type, vulcan_type>*>(user);
...
template <typename lcm_type, typename vulcan_type>
void SubscriptionManager<lcm_type, vulcan_type>::addChannelSubscriber(lcm_t* lcm, const channel_subscriber_t<vulcan_type>& newSubscriber)
{
    ChannelSubscribersMap* channels = channelSubscribers(lcm);
...
template <typename lcm_type, typename vulcan_type>
void SubscriptionManager<lcm_type, vulcan_type>::sendDataToSubscribers(lcm_t* lcm, const std::string& channel, const lcm_type& data)
{
