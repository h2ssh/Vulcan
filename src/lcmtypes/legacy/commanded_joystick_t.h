...
void convert_lcm_to_vulcan(const vulcan_lcm_commanded_joystick_t& joystickMessage,
                           vulcan::robot::commanded_joystick_t& joystick);
void convert_vulcan_to_lcm(const vulcan::robot::commanded_joystick_t& joystick,
                           vulcan_lcm_commanded_joystick_t& joystickMessage);

void publish_data(lcm_t* lcm,
                  const vulcan::robot::commanded_joystick_t& joystick,
                  std::string channel = COMMANDED_JOYSTICK_CHANNEL);
void subscribe_to_message(lcm_t* lcm,
                          void (*callback)(const vulcan::robot::commanded_joystick_t&, const std::string&, void*),
                          void* userdata,
                          std::string channel = COMMANDED_JOYSTICK_CHANNEL);
