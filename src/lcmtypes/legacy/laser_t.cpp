...
void subscribe_to_channel(lcm_t* lcm,
                          void (*callback)(const polar_laser_scan_t&, const std::string&, void*),
                          void* userdata,
                          const std::string& channel);
void subscribe_to_old_channels(lcm_t* lcm,
                               void (*callback)(const polar_laser_scan_t&, const std::string&, void*),
                               void* userdata,
                               bool front);
...
{
    // For the old messages, if offset.x is less than 0, the laser is on the back of the robot, so just hard-code that
    // for the old datasets
...
void convert_lcm_to_vulcan(const vulcan_lcm_laser_with_intensity_t& laserMessage, polar_laser_scan_t& scan)
{
    if (laserMessage.num_intensities) {
        ...
        std::copy(laserMessage.intensities,
                  laserMessage.intensities + laserMessage.num_intensities,
                          ...
                          const std::string& channel)
{
    ...
    if (!subscribers.isSubscribedToChannel(lcm, channel)) {
        ...
        vulcan_lcm_laser_t_subscribe(lcm,
                                     channel.c_str(),
                                     subscription_manager_callback<vulcan_lcm_laser_t, polar_laser_scan_t>,
                                     &subscribers);
    } else {
        subscribers.addChannelSubscriber(lcm, newSubscriber);
    }
                               ...
                               bool front)
{
    ...
    if (!oldSubscribers.isSubscribedToChannel(lcm, channel)) {
        ...
        vulcan_lcm_laser_old_t_subscribe(lcm,
                                         channel.c_str(),
                                         subscription_manager_callback<vulcan_lcm_laser_old_t, polar_laser_scan_t>,
                                         &oldSubscribers);
    } else {
        oldSubscribers.addChannelSubscriber(lcm, newSubscriber);
    }
    ...
    if (!threeDofSubscribers.isSubscribedToChannel(lcm, threeDofChannel)) {
        ...
        vulcan_lcm_laser_3dof_t_subscribe(lcm,
                                          threeDofChannel.c_str(),
                                          subscription_manager_callback<vulcan_lcm_laser_3dof_t, polar_laser_scan_t>,
                                          &threeDofSubscribers);
    } else {
        threeDofSubscribers.addChannelSubscriber(lcm, threeDofSubscriber);
    }
    ...
    if (!intensitySubscribers.isSubscribedToChannel(lcm, intensityChannel)) {
        ...
        vulcan_lcm_laser_with_intensity_t_subscribe(
          lcm,
          intensityChannel.c_str(),
          subscription_manager_callback<vulcan_lcm_laser_with_intensity_t, polar_laser_scan_t>,
          &intensitySubscribers);
    } else {
        intensitySubscribers.addChannelSubscriber(lcm, intensitySubscriber);
    }
}
