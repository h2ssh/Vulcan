...
void vulcan::lcm::subscribe_to_message(lcm_t* lcm, void (*callback)(const robot::elevator_t&, const std::string&, void*), void* userdata, std::string channel)
{
    ...
    if(!subscribers.isSubscribedToChannel(lcm, channel))
    {
        ...
        vulcan_lcm_elevator_t_subscribe(lcm, channel.c_str(), subscription_manager_callback<vulcan_lcm_elevator_t, robot::elevator_t>, &subscribers);
    }
    else
    {
        subscribers.addChannelSubscriber(lcm, newSubscriber);
