    ...
    if (config.getValueAsDouble(ROBOT_CONFIGURATION_HEADING, IS_ACTIVATED_KEY)) {
        ...
        if (case_no != 0) {
            params.script_path = config.getValueAsString(ROBOT_CONFIGURATION_HEADING, SCRIPT_PATH_KEY)
              + readingCaseSpt(ROBOT_CONFIGURATION_HEADING, case_no);
        } else {
            params.script_path = config.getValueAsString(ROBOT_CONFIGURATION_HEADING, SCRIPT_PATH_KEY);
        }
    }
...
std::string readingCaseSpt(const std::string HEADING, int case_no)
{
    ...
    if (HEADING == "MainRobot") {
        case_file_name += "main.spt";
    } else if (HEADING == "FirstRobot") {
        case_file_name += "one.spt";
    } else if (HEADING == "SecondRobot") {
        case_file_name += "two.spt";
    } else if (HEADING == "ThirdRobot") {
        case_file_name += "three.spt";
    }
