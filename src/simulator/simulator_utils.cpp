               ...
               hssh::LocalPerceptualMap& lpm)
{
    ...
    // double maxdist = 40.0;
    endpointsgrid = utils::trace_range_until_condition(
      startgrid,
      fake_ray_shot,
    ...
    for (unsigned i = 0; i < (endpointsgrid.size() - 1); i++) {
        ...
        endpointglobalbuffer =
          utils::grid_point_to_global_point(endpointgridbuffer, lpm);   // Transfer the endpoints to global coords
                       ...
                       system::ModuleCommunicator& connector)
{
    ...
    // printf("Current state of robot is in %f, %f, %f\n", vulcan_state.pose.x,vulcan_state.pose.y
    // ,vulcan_state.pose.theta); printf("Former state of robot is in %f, %f, %f\n",
    // vulcan_former_state.pose.x,vulcan_former_state.pose.y ,vulcan_former_state.pose.theta); printf("%s the delta of
    // left wheel is %f and right wheel is %f\n", "encoder data sent.", fake_encoder_data.deltaLeftWheel,
    // fake_encoder_data.deltaRightWheel);
                        ...
                        system::ModuleCommunicator& connector)
{
    ...
    if (vulcan_former_state.pose.x == vulcan_state.pose.x && vulcan_former_state.pose.y == vulcan_state.pose.y
        && vulcan_former_state.pose.theta == vulcan_state.pose.theta) {
    ...
    // ,vulcan_state.pose.theta); printf("Former state of robot is in %f, %f, %f\n",
    // vulcan_former_state.pose.x,vulcan_former_state.pose.y ,vulcan_former_state.pose.theta); printf("Translation and
