...
void RobotObjectGroup::handleData(const simulator_robot_group_message_t& simulator_robot_group_message, const std::string& channel)
{
...
void RobotObjectGroup::initRobotObject(robot_params_t robot_params, hssh::LocalPerceptualMap lpm, int8_t robot_id, pose_t pose, bool hasPose)
{
	system::ModuleCommunicator* communicator = new system::ModuleCommunicator(robot_params.system_url,robot_params.system_url);
	...
	if (!hasPose)
	{
		robots_.push_back(new RobotObject(robot_params,
	...
	{
		robots_.push_back(new RobotObject(pose,
										robot_params,
	 						 		  	wheel_encoder_params,
	   						 		  	plantModel,
	   						 		  	false,
