...
void display_help_if_needed(const utils::CommandLine& commandLine)
{
    bool helpNeeded = !commandLine.argumentExists(JOYSTICK_PORT) || !commandLine.argumentExists(CONTROLLER_PORT)
      || !commandLine.argumentExists(IMU) || !commandLine.argumentExists(CONFIG_FILE)
...
void run_test(test_configuration_t& config, std::vector<test_sample_t>& samples)
{
    ...
    std::cout << "INFO: Starting test: Command:(" << config.command.forward << ',' << config.command.left
    ...
    for (int n = config.testSamples; --n >= 0;) {
        ...
        if (isJoystickCentered(humanJoystickCommand))   // set command if there is no external joystick command.
        {
            ...
            std::cout << "INFO: OVERRIDE: Manual joystick command detected: (" << humanJoystickCommand.forward << ','
                      << humanJoystickCommand.left << ')' << '\n';
        }
...
void save_test_samples(std::vector<test_sample_t>& samples, std::ofstream& file)
{
    for (unsigned int n = 0; n < samples.size(); n++) {
        file << samples[n].command.timestamp << ' ' << samples[n].command.forward << ' ' << samples[n].command.left
...
void run_maneuvers(test_configuration_t& config, std::ofstream& file)
{
    ...
    for (i = 20; i < 61; i += 10) {
        for (j = 10; j > -11; j -= 20) {
            ...
            std::cout << "Command set: (" << config.command.forward << ',' << config.command.left << ")\n";
            std::cout << "WARNING!!: The wheelchair will be driving in straight line in approximately 2.5 seconds. "
                         "Please make sure to have a clear path.\n";
