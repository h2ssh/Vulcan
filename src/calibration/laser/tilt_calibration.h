...
struct tilt_t
{
    ...
    explicit tilt_t(double pitch = 0.0, double roll = 0.0) : pitch(pitch), roll(roll) { }
};
 ...
 *
 * 1) Point the laser perpendicular to a wall that's reasonably long and record a bunch of data with the robot sitting
 * still. 2) Identify the start and end rays for the wall. 3) Calibrate via calibrate_laser_pitch:
