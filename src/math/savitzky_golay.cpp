                           ...
                           double normalizer)
{
    ...
    for (int n = 0; n < kRadius; ++n) {
        ...
        *output++ =
          std::inner_product(begin, begin + n + kRadius + 1, coefficients.begin() + wraparound, value) * normalizer;
    }

    // Handle the elements in the range [kRadius, end-kRadius)
    for (int n = kRadius; n < kDataSize - kRadius; ++n) {
        *output++ =
          std::inner_product(begin + n - kRadius, begin + n + kRadius + 1, coefficients.begin(), 0.0) * normalizer;
    }
