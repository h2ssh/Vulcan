...
public:
     ...
     * boost::none.
     */
    bool hasEnoughDataForExtremum(void) const
    {
        return static_cast<int>(values_.size()) > kExtremumWidth_ + kFilterRadius_;
    }
...
boost::optional<sequence_extremum_t<T>> SmoothedDerivativeSequence<T>::extremumAfter(int begin) const
{
    ...
    for (int n = begin, end = static_cast<int>(secondSmoothedDerivs_.size()); n < end; ++n) {
        ...
        // extremum width, then
            ...
            extremum.index =
              std::distance(values_.begin(), std::max_element(values_.begin() + begin, values_.begin() + n))
              - beginIndexOffset_;   //; signFlipIndex - beginIndexOffset_; // the index offset remains hidden to
                                     //callers
            extremum.end = n;
