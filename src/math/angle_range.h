...
struct angle_range_t
{
    ...
    explicit angle_range_t(float start = 0.0f, float extent = 0.0f)
    ...
    */
    ...
    {
        return wrap_to_2pi(angle - start) < extent;
    }
    ...
    */
    void expand(float angle)
    {
        ...
        {
            // Move the start if the angle being added is closer to the start than the end of the range and the angle is to the right of
            // the start angle. The range is always positive, i.e. to the left of the start, so that invariant needs to be maintained
            // If there isn't an extent yet, then move such that the range is positive
            ...
            // Otherwise if there's an extent and the distance to start is less than that to the end, then move the start
