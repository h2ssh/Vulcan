 ...
 */
Line<double>
  line_from_least_squares(Point<double> p1, Point<double> p2, double xAverage, double yIntercept, double slope);
 ...
 */
Line<float> simple_least_squares(std::vector<Point<float>>::const_iterator begin,
...
Line<double> total_least_squares(PointIterator begin, PointIterator end)
{
    ...
    double avgY = std::accumulate(begin,
                                  end,
                                  0.0,
                                  [](double total, const PointType& p) {
                                      return total + p.y;
                                  })
      / numPoints;
    double sumY2 = std::accumulate(begin, end, 0.0, [](double total, const PointType& p) {
        return total + static_cast<double>(p.y) * p.y;
    });
    double avgX = std::accumulate(begin,
                                  end,
                                  0.0,
                                  [](double total, const PointType& p) {
                                      return total + p.x;
      ...
      / numPoints;
    double sumX2 = std::accumulate(begin, end, 0.0, [](double total, const PointType& p) {
        return total + static_cast<double>(p.x) * p.x;
    });
    double sumXY = std::accumulate(begin, end, 0.0, [](double total, const PointType& p) {
        return total + static_cast<double>(p.y) * p.x;
    });

    /*
     * NOTE: There are two potential lines that are created by this method for the regression, so which should be used?
     *       My strategy is to generate the line for both possibilities and then choose the one that minimizes the
    ...
    if (yErr == 0)   // yErr == 0 means that we're dealing with a flat line
    {
        ...
        line.second = 0;
               ...
               || (denom
                   == 0))   // xErr == 0 means that the line is vertical. denom = 0 gives line of infinite slope as well
    {
        line.first = 0;
        ...
        if ((dist_to_line(*begin, a1, b1) + dist_to_line(*(end - 1), a1, b1))
            < (dist_to_line(*begin, a2, b2) + dist_to_line(*(end - 1), a2, b2))) {
            line.first = a1;
            line.second = b1;
        } else {
            line.first = a2;
