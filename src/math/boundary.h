...
/**
 * boundary_heading_probability computes the probability of a given angle range when provided with the uncertainty of
 * the object's heading.
 *
 * The probability is computed using a truncated Gaussian distribution that computes probabilities in the range
 * [-pi,pi]. For angles that wrap around, the probability is the sum of [-pi,negangle] + [pi,posangle], as there's some
