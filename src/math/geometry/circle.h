...
public:
     ...
     */
    explicit Circle(double radius = 1.0, Point<T> center = Point<T>(0, 0)) : radius_(radius), center_(center)
    {
        assert(radius_ > 0.0);
    }

    template <typename U>
    Circle(const Circle<U>& toCopy) : radius_(toCopy.radius())
    {
    }
     ...
     */
    virtual Point<T> center(void) const { return center_; }
     ...
     */
    virtual double area(void) const { return M_PI * radius_ * radius_; }

    /**
     * perimeter calculates the perimeter of the shape.
     */
    virtual double perimeter(void) const { return M_PI * radius_ * 2.0; }
     ...
     */
    virtual bool contains(const Point<T>& point) const { return contains(point.x, point.y); }
     ...
     */
    virtual bool intersects(const Line<T>& line) const { return distance_to_line_segment(center_, line) <= radius_; }
    ...
    virtual bool intersections(const Line<T>& line, std::vector<Point<T>>& intersections) const
    {
        ...
        {
            // calculate the two t-values from the quadratic equation -- either value being in the [0,1] range means it
            // intersects if the t-values are the same, then we have degenerate case where the line is tangent to the
            // circle, which means a single intersection
