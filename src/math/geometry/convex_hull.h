                       ...
                       typename std::vector<Point<T>>::const_iterator end)
{
    ...
    // Do some swapping here to push the "erased" elements to the end of the vector -- takes more memory, but saves the
    // copying from erasing
    for (std::size_t n = 2; n < uniquePoints.size(); ++n) {
        ...
        while (
          upperHullSize > 2u
          && left_of_line(upperHull[upperHullSize - 3], upperHull[upperHullSize - 2], upperHull[upperHullSize - 1])) {
    ...
    for (int n = uniquePoints.size() - 2; --n >= 0;) {
        ...
        while (
          lowerHullSize > 2u
          && left_of_line(lowerHull[lowerHullSize - 3], lowerHull[lowerHullSize - 2], lowerHull[lowerHullSize - 1])) {
