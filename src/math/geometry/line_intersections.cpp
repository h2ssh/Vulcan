...
{
    return absolute_fuzzy_equal(lhs.x, rhs.x, FUZZY_EQUAL_TOLERANCE)
      && absolute_fuzzy_equal(lhs.y, rhs.y, FUZZY_EQUAL_TOLERANCE);
}
...
struct sweep_line_value_t
{
    ...
    sweep_line_value_t(void) : lineIndex(0) { }

    sweep_line_value_t(int index, Line<double> line) : lineIndex(index), line(line) { }
    ...
    double distanceToEvent(void) const
    {
        ...
        if (line.a.x == line.b.x) {
            ...
            // For a horizontal line, the endpoint will fall either left of the start point or between the start point
            // and the endpoint When the line is between the start point and the end point, then the distance to the
            // event point becomes 0 because the line intersects all events that occur in this region
            eventDistance = (start_point(line).x >= eventPoint.x) ? (eventPoint.x - start_point(line).x) : 0.0;
        } else {
            eventDistance =
              eventPoint.x - (line.a.x + ((line.b.x - line.a.x) * (eventPoint.y - line.a.y) / (line.b.y - line.a.y)));
        }
...
{
    bool operator()(const Point<double>& lhs, const Point<double>& rhs) const { return lexicographic_less(lhs, rhs); }
};
...
struct sweep_line_comparison
{
    bool operator()(const sweep_line_value_t& lhs, const sweep_line_value_t& rhs) const
    {
        ...
        return !absolute_fuzzy_equal(lhsDistance, rhsDistance, FUZZY_EQUAL_TOLERANCE)
          && (lhs.distanceToEvent() > rhs.distanceToEvent());
    }
};
...
{
    intersection_state_t(const std::vector<Line<double>>& lines) : lines(lines) { }
...
void check_for_event(sweep_line_t::const_iterator leftLine,
                     sweep_line_t::const_iterator rightLine,
...
void queue_initial_events(intersection_state_t& state)
{
    ...
    for (size_t n = 0; n < state.lines.size(); ++n) {
        if (state.lines[n].a.x != state.lines[n].b.x || state.lines[n].a.y != state.lines[n].b.y) {
...
void handle_next_event(intersection_state_t& state)
{
    ...
    for (auto pointIt = containsRange.first; pointIt != containsRange.second; ++pointIt) {
        // Don't add ending lines to the collection of intersect lines, as they would be reinserted into the sweep line
        // -- bad!
        if (event.endLines.find(pointIt->lineIndex) == event.endLines.end()
            && event.startLines.find(pointIt->lineIndex) == event.startLines.end()) {
...
void adjust_sweep_line_order(const sweep_line_value_t& point, line_event_t& event, intersection_state_t& state)
{
     ...
     * through them in left-to-right order immediately after the point. This condition is the equivalent of the
     * endpoints being sorted radially from left-to-right in a coordinate frame rotated pi radians, s.t. negative x-axis
     * becomes the positive x-axis centered at the event point. All angles are in the range [0, pi) because points above
     * the sweep line have been considered.
     *
    ...
    if (!event.intersectLines.empty()) {
        std::set<int> toErase;
        for (auto lineIt = event.intersectLines.begin(), lineEnd = event.intersectLines.end(); lineIt != lineEnd;
             ++lineIt) {
    ...
    for (auto orderedIt = orderedPoints.begin(), orderedEnd = orderedPoints.end(); orderedIt != orderedEnd;
         ++orderedIt) {
...
void erase_points_from_sweep_line(const sweep_line_value_t& event, const std::set<int>& lines, sweep_line_t& sweepLine)
{
    ...
    if (pointsRange.first == pointsRange.second && !lines.empty()) {
        std::cerr << "ERROR:Failed to find event! Point:" << event.eventPoint << " Dist:" << event.distanceToEvent()
                  << " Index:" << event.lineIndex << '\n';
    }
    ...
    if (pointsErased != lines.size()) {
        std::cout << "WARNING:Wanted to erase " << lines.size() << " points, but only erased " << pointsErased
                  << " points. Performing brute force search" << std::endl;
                     ...
                     intersection_state_t& state)
{
...
#ifdef DEBUG_INTERSECTION
                std::cout << "DEBUG: New intersection at existing event point: " << intersectionPoint
                          << " Indices:" << leftLine->lineIndex << ',' << rightLine->lineIndex << '\n';
#endif
            } else {
...
#ifdef DEBUG_INTERSECTION
                std::cout << "DEBUG: New intersection at new event point: " << intersectionPoint
                          << " Indices:" << leftLine->lineIndex << ',' << rightLine->lineIndex << '\n';
#endif
            }
        } else if (absolute_fuzzy_equal(intersectionPoint.y, sweep_line_value_t::eventPoint.y, FUZZY_EQUAL_TOLERANCE)
                   && intersectionPoint.y < sweep_line_value_t::eventPoint.y) {
            std::cerr << "WARNING:Equal lines, but not really. Diff:"
                      << (sweep_line_value_t::eventPoint.y - intersectionPoint.y) << '\n';
        }
    }
    ...
    if (line_segment_intersection_point(left, right, intersectionPoint)) {
        if (lexicographic_less(intersectionPoint, sweep_line_value_t::eventPoint)
            && !point_fuzzy_equal(intersectionPoint, sweep_line_value_t::eventPoint))
        //         if(!((absolute_fuzzy_equal(intersectionPoint.y, sweep_line_value_t::eventPoint.y) ||
        //         intersectionPoint.y < sweep_line_value_t::eventPoint.y)))
        {
            std::cout << "DEBUG: Intersection above sweep line: sweep:" << sweep_line_value_t::eventPoint
                      << " intersection:" << intersectionPoint << " Indices:" << leftLine->lineIndex << ','
                      << rightLine->lineIndex << '\n';

            if (absolute_fuzzy_equal(intersectionPoint.y, sweep_line_value_t::eventPoint.y, FUZZY_EQUAL_TOLERANCE)
                && intersectionPoint.y < sweep_line_value_t::eventPoint.y) {
...
void print_event(const Point<double>& eventPoint, const line_event_t& event)
{
    ...
    std::cout << " intersect:";
    for (auto indexIt = event.intersectLines.begin(), indexEnd = event.intersectLines.end(); indexIt != indexEnd;
         ++indexIt) {
        std::cout << ' ' << *indexIt;
    }
}
...
void print_sweep_line(const sweep_line_t& line)
{
    ...
    for (auto lineIt = line.begin(), lineEnd = line.end(); lineIt != lineEnd; ++lineIt) {
        error |= !(absolute_fuzzy_equal(lastDistance, lineIt->distanceToEvent(), FUZZY_EQUAL_TOLERANCE)
                   || (lineIt->distanceToEvent() < lastDistance));
