...
Rectangle<float>
  rotating_calipers_bounding_rectangle(PointIter begin, PointIter end, const Polygon<float>& hull, ErrorFunc errorFunc);
...
Rectangle<float> minimum_area_bounding_rectangle(PointIter begin, PointIter end)
{
    return minimum_area_bounding_rectangle(begin, end, convex_hull<float>(begin, end));
}


Rectangle<float> minimum_area_bounding_rectangle(PointIter begin, PointIter end, const Polygon<float>& hull)
{
    return rotating_calipers_bounding_rectangle(begin,
                                                end,
                                                hull,
...
Rectangle<float> minimum_geometric_error_bounding_rectangle(PointIter begin, PointIter end)
{
    return minimum_geometric_error_bounding_rectangle(begin, end, convex_hull<float>(begin, end));
}


Rectangle<float> minimum_geometric_error_bounding_rectangle(PointIter begin, PointIter end, const Polygon<float>& hull)
{
    return rotating_calipers_bounding_rectangle(
      begin,
      end,
      hull,
  ...
  rotating_calipers_bounding_rectangle(PointIter begin, PointIter end, const Polygon<float>& hull, ErrorFunc errorFunc)
{
    ...
    if (state.hullVertices.size() <= 2) {
        std::cerr << "WARNING:Too few points for proper minimum area bounding rectangle. Using axis-aligned bounding "
                     "rectangle instead.\n";
    ...
    std::cout << "Hull:";
    for (auto pointIt = state.hullVertices.begin(), pointEnd = state.hullVertices.end(); pointIt != pointEnd;
         ++pointIt) {
        std::cout << ' ' << *pointIt;
    }
    std::cout << '\n';
    ...
    std::cout << "INFO:bounding_rectangle:" << state.minErrorRectangle.topRight << ','
              << state.minErrorRectangle.bottomRight << ',' << state.minErrorRectangle.bottomLeft << ','
...
#ifdef DEBUG_CALIPER_ROTATION
        std::cout << "DEBUG:caliper_rotation:" << n << " Angle:" << caliper.angle << " Hull:" << hullAngle
                  << " Rot:" << caliperRotation << " caliper:" << caliper.line << " hull:"
                  << Line<float>(state.hullVertices[caliper.vertexIndex], state.hullVertices[previousVertexIndex])
                  << '\n';
#endif
...
void rotate_calipers(caliper_rotation_t rotation, rotating_calipers_state_t& state)
{
    ...
    state.calipers[rotation.index].vertexIndex =
      previous_index(state.calipers[rotation.index].vertexIndex, state.hullVertices.size());
...
Rectangle<float> create_bounding_rectangle(rotating_calipers_state_t& state)
{
    ...
    if (!line_intersection_point(state.calipers[BOTTOM_INDEX].line,
                                 state.calipers[LEFT_INDEX].line,
                                 boundary.bottomLeft)) {
    ...
    if (!line_intersection_point(state.calipers[BOTTOM_INDEX].line,
                                 state.calipers[RIGHT_INDEX].line,
    ...
    std::cout << "DEBUG:bounding_rectangle:" << boundary.topRight << ',' << boundary.bottomRight << ','
              << boundary.bottomLeft << ',' << boundary.topLeft << " area:" << boundary.area() << '\n';
#endif
