...
float three_corner_intersection(const Circle<float>& circle, const Rectangle<float>& rectangle)
{
    // TODO
    auto squareApprox =
      Rectangle<float>(Point<float>(circle.center().x - circle.radius(), circle.center().y - circle.radius()),
                       Point<float>(circle.center().x + circle.radius(), circle.center().y + circle.radius()));
