...
public:
    ...
    using ConstIter = typename std::vector<Point<T>>::const_iterator;   ///< Iterator type for the vertices of the polygon
    ...
    */
    ...
    Polygon(PointIterator begin, PointIterator end)
    : vertices_(begin, end)
    {
    ...
    */
    template <typename U>
    explicit Polygon(const std::vector<Point<U>>& points)
    ...
    */
    template <typename U>
    explicit Polygon(const Polygon<U>& rhs)
    : Polygon(rhs.begin(), rhs.end())
    {
    }
        ...
        for(int i = 0, j = vertices_.size() - 1, end = vertices_.size(); i < end; ++i)
        {
            ...
            if(yflag0 != yflag1)
            {
                if(((vertices_[i].y - y) * (vertices_[j].x - vertices_[i].x) >=
                    (vertices_[i].x - x) * (vertices_[j].y - vertices_[i].y)) == yflag1)
                {
                    inside = !inside;
                }
            }
...
private:
    ...
    // INVARIANT: Vertices are stored such that the shape is traced out in counter-clockwise order around the polygons perimeter
