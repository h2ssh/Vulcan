...
T find_single_root(ErrorFunc func, T initial, T precision)
{
    if (std::is_floating_point<T>::value) {
        ...
        std::cout
          << "INFO::find_single_root: Unable to find roots with non-floating point values. Returning initial.\n";
        return initial;
    }
}
...
public:
     ...
     */
    NewtonRaphsonErrorFunc(Func func, Func derivative) : func_(func), derivative_(derivative) { }

    // Callable concept
    T operator()(T value)
    {
        std::cout << "Newton: x:" << value << " f(x):" << func_(value) << " f'(x):" << derivative_(value)
                  << " err:" << (func_(value) / derivative_(value)) << '\n';
