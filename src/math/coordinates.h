...
template<typename T>
Point<T> convert_reference_frame(const Point<T>& pointInFromFrame, ReferenceFrame from, ReferenceFrame to, const pose_t& originInToFrame)
{
    if(from == to)
    {
    ...
    {
        float rotatedX = pointInFromFrame.x*cos(originInToFrame.theta) - pointInFromFrame.y*sin(originInToFrame.theta);
        float rotatedY = pointInFromFrame.x*sin(originInToFrame.theta) + pointInFromFrame.y*cos(originInToFrame.theta);
...
inline pose_t convert_reference_frame(const pose_t& poseInFromFrame, ReferenceFrame from, ReferenceFrame to, const pose_t& originInToFrame)
{
...
template <typename T>
Rectangle<T> convert_reference_frame(const math::Rectangle<T>& rectInFromFrame, ReferenceFrame from, ReferenceFrame to, const pose_t& originInToFrame)
{
...
template <typename T>
Polygon<T> convert_reference_frame(const math::Polygon<T>& polyInFromFrame, ReferenceFrame from, ReferenceFrame to, const pose_t& originInToFrame)
{
...
Point<T> robot_frame_to_global_frame(const Point<T>& pointInRobotFrame, const pose_t& robotGlobalPose)
{
    ...
    float rotatedX = pointInRobotFrame.x*cos(robotGlobalPose.theta) - pointInRobotFrame.y*sin(robotGlobalPose.theta);
    float rotatedY = pointInRobotFrame.x*sin(robotGlobalPose.theta) + pointInRobotFrame.y*cos(robotGlobalPose.theta);
...
Point<T> global_frame_to_robot_frame(const Point<T>& pointInGlobalFrame, const pose_t& robotGlobalPose)
{
    ...
    float rotatedX = pointInGlobalFrame.x*cos(robotGlobalPose.theta)  + pointInGlobalFrame.y*sin(robotGlobalPose.theta);
    float rotatedY = -pointInGlobalFrame.x*sin(robotGlobalPose.theta) + pointInGlobalFrame.y*cos(robotGlobalPose.theta);
...
inline pose_t global_frame_to_robot_frame(const pose_t& poseInGlobalFrame, const pose_t& robotGlobalPose)
{
    ...
    float rotatedX = poseInGlobalFrame.x*cos(robotGlobalPose.theta)  + poseInGlobalFrame.y*sin(robotGlobalPose.theta);
    float rotatedY = -poseInGlobalFrame.x*sin(robotGlobalPose.theta) + poseInGlobalFrame.y*cos(robotGlobalPose.theta);
