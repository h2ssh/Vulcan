...
ZernikeMoments<N>::ZernikeMoments(void)
{
    ...
    for(int n = 0; n <= N; ++n)
    {
        for(int m = n-2; m >= 0; m -= 2)
        {
            ...
            h1_[n][m] = (((m + 4) * (m + 3)) / 2) - ((m + 4) * h2_[n][m]) + (h3_[n][m] * ((n + m + 6) *
                (n - m - 4) / 8));
        }
    }
}
...
//     assert(kScaleFactor > kDiameter/2);
    ...
    for(auto pointIt = begin; pointIt != end; ++pointIt)
    {
        ...
        auto normalized = Point<double>((pointIt->x - center.x) * kPointNormalizer,
                                              (pointIt->y - center.y) * kPointNormalizer);
        ...
        if(pointRadius > 1.0)
        {
            std::cout << "Point:" << *pointIt << " Center:" << center << " Dist to center:"
                << distance_between_points(*pointIt, center)
...
std::size_t ZernikeMoments<N>::numMoments(void) const
{
    ...
    * viewed as two acending sequences for the even and odd numbers, where the maximum number of moments in the sequence
    * is M = (N / 2) + 1. There are also (N / 2) + 1 even numbers in the sequence (0 counts). Thus, there are
    * (M * (M+1)) / 2 even moments and odd moments. The total moments is M * (M+1) if N is odd and M*(M+1)/2 + M*(M-1)/2
    * if N is even.
    */
...
template <int N>
void ZernikeMoments<N>::computeCosAndSinTable(double x,
                                              double y,
                                              double radius,
                                              CacheArray& cosT,
                                              CacheArray& sinT) const
{
    double a = x / radius;
    double b = y / radius;
