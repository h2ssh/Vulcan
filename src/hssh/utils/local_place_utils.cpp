...
LPM rotate_lpm(const LocalPerceptualMap& lpm, float angle)
{
    ...
    */
    ...
    float newHalfWidth  = ceil(radius * std::max(std::abs(cos(radiusAngle + angle)), std::abs(cos(-radiusAngle + angle))));
    float newHalfHeight = ceil(radius * std::max(std::abs(sin(radiusAngle + angle)), std::abs(sin(-radiusAngle + angle))));
...
SmallScaleStar rotate_star(const SmallScaleStar& star, float angle)
{
    ...
    std::for_each(rotated.begin(), rotated.end(), [angle](local_path_fragment_t& fragment) { fragment.gateway = rotate_gateway(fragment.gateway, angle); });
...
std::vector<Gateway> rotate_gateways(const std::vector<Gateway>& gateways, float angle)
{
    ...
    std::for_each(rotated.begin(), rotated.end(), [angle](Gateway& gateway) { gateway = rotate_gateway(gateway, angle); });
...
Gateway rotate_gateway(const Gateway& gateway, float angle)
{
    return Gateway(gateway.id(),
                   Line<double>(rotate(gateway.boundary.a, angle), 
                                      rotate(gateway.boundary.b, angle)),
