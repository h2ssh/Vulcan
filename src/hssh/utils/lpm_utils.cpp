...
// 
//     int newHalfWidth  = ceil(radius * std::max(std::abs(cos(radiusAngle + rotationAngle)), std::abs(cos(-radiusAngle + rotationAngle))));
//     int newHalfHeight = ceil(radius * std::max(std::abs(sin(radiusAngle + rotationAngle)), std::abs(sin(-radiusAngle + rotationAngle))));
// 
// #ifdef DEBUG_CHANGE_FRAME
//     std::cout<<"INFO:change_lpm_reference_frame:Old dim:("<<halfWidth<<','<<halfHeight<<") New dim:("<<newHalfWidth<<','<<newHalfHeight<<")\n";
// #endif
// 
//     LPM rotated(lpm);
...
//     rotated.setBottomLeft(Point<float>(oldCenter.x - newHalfWidth*rotated.metersPerCell()  - newReference.x,
//                                              oldCenter.y - newHalfHeight*rotated.metersPerCell() - newReference.y));
// 
// #ifdef DEBUG_CHANGE_FRAME
//     std::cout<<"INFO:change_lpm_reference_frame:Old BL:"<<lpm.getBottomLeft()<<" New BL:"<<rotated.getBottomLeft()<<'\n';
// #endif
// 
//     return rotated;
// }
