...
* \param    closeAngleThreshold         Threshold for the difference in angles for two gateways to be considered the same (optional, default = pi/4)
* \param    endpointDistanceThreshold   Threshold for the difference in distance for closest gateway
*                                           endpoints for gateways to be considered the same (optional, default = 0.2m)
* \return True if the gateway angles and minimum endpoint distance are less than the defined thresholds.
*/
...
/**
* is_gateway_traversable checks if the robot can fit on both sides of the gateway. If not, then it can't be an affordance.
* The check sweeps through the skeleton and checks that there are enough cells on each side of the gateway to ensure
* that it can be traversed by the robot. The length of the robot is used to determine if the robot can fit or not.
*
...
/**
* adjust_gateway_for_new_skeleton creates a new gateway from an existing gateway by adjusting the gateway boundary cells
* and skeleton cells by the minimal amount needed to make them satisfy the gateway properties -- endpoints on
