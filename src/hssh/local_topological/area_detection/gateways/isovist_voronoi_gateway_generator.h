...
/**
 * IsovistVoronoiGatewayGenerator generates Gateways from isovist_local_maximum_t. One Gateway exists for each local
 * maximum. The generation process for the isovists guarantees that the gateway crosses the skeleton in the direction of
 * the gradient.
 *
 * When generating the gateways, gateways for all maxima are generated at the same time to ensure they are consistent
 * with one another. In particular, gateways can't be intersecting, so if they intersect, then get rid of them.
 */
