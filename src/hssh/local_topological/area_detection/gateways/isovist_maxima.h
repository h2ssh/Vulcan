...
struct isovist_local_maximum_t
{
    ...
    std::vector<position_value_t>
      skeletonCells;      ///< All skeleton cells associated with one of the maxima cells along the
...
// inline bool operator<(const isovist_local_maximum_t& lhs, const isovist_local_maximum_t& rhs) { return
// lhs.totalChange < rhs.totalChange; };
inline bool operator<(const isovist_local_maximum_t& lhs, const isovist_local_maximum_t& rhs)
{
...
public:
     ...
     * \param    numAboveMean            Retrieve the number of gradient values that must be above the mean value for a
     * maxima to be valid \param    saveData                Flag indicating if data should be saved for debugging
