 ...
 *   max_endpoint_dist_from_frontier_edge_m = maximum distance an endpoint can be from the edge of a frontier for it to
 * be considered valid
 */
...
public:
    ...
    /**
     * isValidEndpoint checks to see if an endpoint at the given cell is valid given the conditions described in the
     * class description.
     *
    ...
    bool isValidEndpoint(cell_t endpoint) const
    {
        ...
        //             && (grid_->getClassification(endpoint.x, endpoint.y) & (SKELETON_CELL_OCCUPIED |
        //             SKELETON_CELL_FRONTIER));
    }
