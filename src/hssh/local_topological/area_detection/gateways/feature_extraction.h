...
/**
 * extract_gateway_features_default extract a feature vector for each cell in the skeleton that is at least distance >=
 * 'radius' from a junction point.
 *
 ...
 */
SkeletonFeatures
  extract_gateway_features_default(const VoronoiEdges& edges, const VoronoiIsovistField& isovists, int radius = 1);
