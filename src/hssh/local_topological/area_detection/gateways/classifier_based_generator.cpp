...
struct ClassifierBasedGenerator::Impl
{
                                 ...
                                 std::vector<WeightedGateway>& gateways)
    {
        ...
        auto non_sentinel_func = [kSentinel](int idx) { return idx != kSentinel; };
        ...
        {
            auto regionEndIt = std::find_if(regionStartIt,
                                            endIt,
                                            [regionStartIt](int idx) { return idx != *regionStartIt; });
            ...
            auto maxIt = std::max_element(edgeProb_.begin() + regionIdx_.front(),
                                          edgeProb_.begin() + regionIdx_.back() + 1);
            ...
            if(debug)
            {
                std::cout << "Region:" << regionCells_.front() << " -> " << regionCells_.back() << " Max: "
                    << maxCell << " prob:" << *maxIt << " Gwy? " << (regionGateway != boost::none) << '\n';
            }
...
//     }
        ...
        {
            skeletonNormal = angle_sum(isovists.at(maxCell).scalar(utils::Isovist::kMinDistOrientation),
                                              M_PI_2);
        }
        ...
        for(std::size_t n = 0; n < regionCells_.size(); ++n)
        {
            ...
            for(auto& source : bestSources)
            {
                ...
                auto gateway = create_gateway_at_cell(cell,
                                                      normal,
                                                      regionId,
                                                      skeleton);
...
std::vector<WeightedGateway> ClassifierBasedGenerator::generateGateways(const std::vector<WeightedGateway>& priorGateways,
                                                                        const VoronoiIsovistField& isovists,
                                                                        const VoronoiSkeletonGrid& skeleton,
                                                                        const EndpointValidator& validator)
{
