...
const uint8_t LPM_OCCUPIED =
  kOccupiedOccGridCell | kHazardOccGridCell | kLimitedVisibilityOccGridCell;   // | kDynamicOccGridCell;
...
void BrushfireSkeletonBuilder::extractSkeleton(const LocalPerceptualMap& map,
...
void BrushfireSkeletonBuilder::enqueueObstacleBoundaries(const LocalPerceptualMap& map,
                                                         VoronoiSkeletonGrid& grid,
                                                         IslandDetector& islands)
{
        ...
        for (cell.x = 0; cell.x < width; ++cell.x) {
            ...
            if ((classification & ~SKELETON_CELL_FREE) && islands.cellIsOnIsland(cell, grid)) {
                ...
                if (((classification & SKELETON_CELL_FRONTIER) && !is_cell_on_map_edge(cell, map))
                    ...
                    grid.setClassification(cell.x,
                                           cell.y,
                                           SKELETON_CELL_UNKNOWN | (classification & SKELETON_CELL_FRONTIER));
                }
            }
        }
    }
...
void BrushfireSkeletonBuilder::handleCollisionCell(const brushfire_node_t& node, VoronoiSkeletonGrid& grid)
{
    ...
    if (utils::contains_if(anchors, [&node](cell_t cell) {
            return !are_cells_separated(cell, node.origin);
        })) {
        return;
    }
...
void BrushfireSkeletonBuilder::pruneSkeleton(VoronoiSkeletonGrid& grid)
{
    ...
    std::sort(unprunedSkeletonCells.begin(),
              unprunedSkeletonCells.end(),
              [](const CellDistPair& lhs, const CellDistPair& rhs) {
...
void BrushfireSkeletonBuilder::setCoastalDistance(float cellScale)
{
    ...
    coastalDistanceInCells =
      requestedCoastalDist > MAXIMUM_SEARCH_DISTANCE ? MAXIMUM_SEARCH_DISTANCE : requestedCoastalDist;
...
bool isnt_clump(cell_t skeleton, const VoronoiSkeletonGrid& grid)
{
    ...
    if ((!isSkeleton[0] && isSkeleton[1] && !isSkeleton[2]) || (!isSkeleton[2] && isSkeleton[3] && !isSkeleton[4])
        || (!isSkeleton[4] && isSkeleton[5] && !isSkeleton[6]) || (!isSkeleton[6] && isSkeleton[7] && !isSkeleton[0])) {
        return true;
    }
