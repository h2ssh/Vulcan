 ...
 *
 * 0) Extract the underlying graph structure from the cell-based skeleton. Build a traditional edges-vertices
 * representation. 1) Reduce the graph using a SkeletonGraphReducer. Either minimum-spanning tree or all-paths at the
 * moment. 2) Rasterize the reduced graph back into the cell grid.
 */
...
public:
     ...
     */
    void
      pruneSkeleton(VoronoiSkeletonGrid& grid, const pose_t& pose, const std::vector<Point<float>>& pointsOfInterest);
