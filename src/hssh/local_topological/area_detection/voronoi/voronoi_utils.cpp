...
cell_t trace_ray_to_cell(float                      angle,
                         ...
                         const VoronoiSkeletonGrid& grid)
{
    return utils::trace_ray_until_condition<VoronoiSkeletonGrid>(start,
                                                                 angle,
...
std::vector<cell_t> trace_rays_in_range(const utils::ray_trace_range_t& range,
                                        ...
                                        const VoronoiSkeletonGrid&      grid)
{
    auto endpoints = utils::trace_range_until_condition<VoronoiSkeletonGrid>(start,
                                                                             range,
                                    ...
                                    bool                       followOnlyReduced)
{
    ...
    std::size_t      numNeighbors = neighbor_cells_with_classification(cell, mask, grid, FOUR_THEN_EIGHT_WAY, skeletonNeighbors);
    ...
    for(std::size_t n = 0; n < numNeighbors; ++n)
    {
        ...
        follow_branch(skeletonNeighbors[n], grid, maxTraceLength - 1, numJunctionsBeforeHalt, mask, pointsVisited, trace.traces);
    }
...
#ifdef DEBUG_TRACE
    std::cout << "DEBUG::trace_voronoi_graph: Cell:" << cell << " Halt junctions:" << numJunctionsBeforeHalt << " Num traces:" << trace.traces.size() << ":\n";
    for(auto& t : trace.traces)
    {
...
{
    NeighborArray neighbors;
    int numNeighbors = neighbor_cells_with_classification(start,
                                       ...
                                       NeighborArray&             neighbors)
{
    ...
    if(!useFourWay)
    {
        ...
        if((cell.x+1 < width) && (cell.y > 0) && comp(grid.getClassification(cell.x+1, cell.y-1), classification))
        {
            neighbors[numNeighbors++] = cell_t(cell.x+1, cell.y-1);
        }

        if((cell.y+1 < height) && (cell.x > 0) && comp(grid.getClassification(cell.x-1, cell.y+1), classification))
        {
            neighbors[numNeighbors++] = cell_t(cell.x-1, cell.y+1);
        }

        if((cell.y+1 < height) && (cell.x+1 < width) && comp(grid.getClassification(cell.x+1, cell.y+1), classification))
        {
            neighbors[numNeighbors++] = cell_t(cell.x+1, cell.y+1);
        }
    }
                                         ...
                                         NeighborArray&             neighbors)
{
    ...
    */
    ...
    for(int n = 2; --n >= 0;)
    {
        ...
        if((vertexConsidered.x >= 0)               &&
            (vertexConsidered.x < width) &&
    ...
    for(int n = 2; --n >= 0;)
    {
        ...
        if((vertexConsidered.y >= 0)               &&
           (vertexConsidered.y < height) &&
           comp(grid.getClassification(vertexConsidered.x, vertexConsidered.y), classification))
        {
            yDir[n] = 0;
            neighbors[numNeighbors++] = vertexConsidered;
                  ...
                  std::vector<voronoi_trace_cells_t>& traces)
{
    ...
    while((numPoints < maxTraceDistance) && haveNewNeighbor)
    {
        ...
        numNeighbors = neighbor_cells_with_classification(currentCell, neighborMask, grid, FOUR_THEN_EIGHT_WAY, skeletonNeighbors);

        // only consider cells with fewer than three neighbors. If at least three neighbors, then some sort of Voronoi junction encountered
        if(numNeighbors > 2)
        {
            break;
        }
    ...
    // If a junction was encountered and allowed to follow the branches, then do so. For each unvisited neighbor, follow that branch. After the process
    // finishes, prepend the trace for this branch to the children branches to create the continuous line of cells for the DFS along that particular
    ...
    if((numNeighbors > 2) && (numJunctionsBeforeHalt > 0) && (numPoints < maxTraceDistance))
    {
        for(std::size_t n = 0; n < numNeighbors; ++n)
        {
            if(visited.find(skeletonNeighbors[n]) == visited.end())
            {
                follow_branch(skeletonNeighbors[n], grid, maxTraceDistance - branchTrace.points.size(), numJunctionsBeforeHalt-1, neighborMask, visited, traces);
            }
        }
    ...
    {
        branchTrace.endCause = (numNeighbors > 2) ? TRACE_JUNCTION : (numPoints == maxTraceDistance) ? TRACE_DISTANCE : TRACE_DEAD_END;
        traces.push_back(std::move(branchTrace));
    }
