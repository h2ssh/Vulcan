...
/**
 * voronoi_cell_t describes a cell in the Voronoi skeleton -- the cell position in the grid and its distance from the
 * nearest wall.
 */
...
public:
     ...
     * \param    onBoundary          Flag indicating if the node exists on the boundary/edge of the map (optional,
     * default = false)
     */
    ...
    /**
     * getType retrieves the mask for the type of node. The type is a bitmask, so it can be both a junction and a
     * gateway, for example.
     */
 ...
 * distance from a wall along the edge. The cells from the skeleton associated with the edge can also be accessed if
 * needed.
 */
class AreaEdge
{
public:
     ...
     *
     * \pre  for each cell[i], adjacent(cell[i-1], cell[i]) i.e. the cells are a connected sequence, as extracted from
     * the skeleton \pre  cells.size() > 1
     */
     ...
     */
    AreaNodePtr getEndpoint(int index) const
    {
        assert(index < 2);
...
/**
 * AreaGraph is an abstraction to a graph of nodes and edges of the Voronoi skeleton constructed via the brushfire
 * algorithm. This graph consists of nodes located at the gateways and junction points. The edges are the portion of the
 */
class AreaGraph
{
public:
...
{
    ar(cell.cell, cell.distance, cell.isFrontier);
}
