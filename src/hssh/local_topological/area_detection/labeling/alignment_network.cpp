...
void AlignmentNetwork::addConstraint(const AlignmentConstraint& constraint)
{
    ...
    if (insideArea.endpoints.size() > 2) {
        std::cerr << "ERROR: AlignmentNetwork: Somehow found more than two endpoints for an area: Id: " << insideArea.id
                  << " Endpoints:\n";
...
CSPSolution AlignmentNetwork::searchForSolution(CSPDebugInfo* debug)
{
    ...
    do {
        ...
        for (auto& change : possibleChanges) {
            std::cout << "Change:" << change.sourceId << " to " << change.newLabels.front()
                      << ": Strain: " << change.strain << " Type: ";
...
void AlignmentNetwork::selectActiveComponent(const ConstraintVec& constraints)
{
    ...
    if (numComponents > 1) {
        ...
        activeIndex =
          std::distance(overlapCounts.begin(), std::max_element(overlapCounts.begin(), overlapCounts.end()));
    }
...
void AlignmentNetwork::generatePossibleChanges(ActiveArea& toChange, SplitMode splitMode, ChangeVec& changes)
{
    ...
    boost::push_back(changeCache_[toChange.id],
                     boost::make_iterator_range(changes.begin() + initialChangeSize, changes.end()));
}
...
int AlignmentNetwork::selectChangeToApply(const ChangeVec& changes, const std::deque<Id>& evalSequence)
{
    ...
    for (int i = 3; i <= 3; ++i) {
        ...
        for (std::size_t n = 0; n < changes.size(); ++n) {
            ...
            std::size_t lastChanged =
              std::distance(evalSequence.rbegin(),
                            std::find(evalSequence.rbegin(), evalSequence.rend(), change.sourceId));
            bool valid = !utils::contains(evalSequence, change.sourceId)
              || (evalSequence.size() < kMinRepeat || (lastChanged >= kMinRepeat) || (changed.size() < kMinRepeat));
            ...
            if (valid) {
                ...
                bool inSequence = utils::contains(evalSequence, change.sourceId)
    ...
    for (const auto& area : original_) {
        ...
        if (area.id == area.active) {
            ...
            std::transform(active(area.id).areas.begin(),
                           ...
                           [this](Id id) {
                               return original_[id].hypothesis;
                           });
...
#ifdef DEBUG_CHANGES
    std::cout << "Dist:" << area.distribution.path << ',' << area.distribution.destination << ','
              << area.distribution.decision << '\n';
...
#ifdef DEBUG_STRAIN
    std::cout << "Strain: (" << maxAppropriatenessStrain << ',' << constraintStrain << ") -> " << totalStrain << '\n';
#endif
