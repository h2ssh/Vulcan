...
void enqueue_mergeable_boundaries(const std::vector<AreaHypothesisBoundary*>& boundaries,
                                  BoundaryMergeCondition condition,
                                  std::queue<AreaHypothesisBoundary*>& queue);
...
std::vector<const AreaHypothesisBoundary*> HypothesisGraph::findMergeableRegion(const AreaHypothesis* start) const
{
    ...
    // Find one of the kSameType mergeable boundaries associated with the start hypothesis and use it as the basis for
    // the findMergeableRegion work method
    for (auto boundaryIt = start->beginBoundary(), boundaryEnd = start->endBoundary(); boundaryIt != boundaryEnd;
         ++boundaryIt) {
    ...
    // Convert the non-const pointers to const pointer for the output. The helper internally needs the non-const for
    // some of the modifying operations
...
void HypothesisGraph::addHypothesisToNode(AreaNode* node, AreaHypothesis* hypothesis)
{
    ...
    if (nodeIt == nodeToHypotheses.end()) {
        ...
        if (nodeIt->second.second) {
            std::cerr << "ERROR::HypothesisGraph: Trying to assign all sorts of things to a node: "
                      << node->getPosition() << " : " << nodeIt->second.first->rectangleBoundary() << " and "
                      << nodeIt->second.second->rectangleBoundary()
                      << " Trying to add:" << hypothesis->rectangleBoundary() << '\n';
...
void HypothesisGraph::createBoundaries(void)
{
    ...
    for (auto& nodePair : nodeToHypotheses) {
        ...
        if (!nodePair.second.first || !nodePair.second.second) {
            ...
            std::cout << "INFO::HypothesisGraph: Located invalid gateway at " << nodePair.first->getPosition()
                      << ". Both edges in the same hypothesis.\n";
            continue;
        }

        boundaries.emplace_back(
          new AreaHypothesisBoundary(nodePair.first, Hypotheses{{nodePair.second.first, nodePair.second.second}}));
...
std::vector<AreaHypothesisBoundary*> HypothesisGraph::findMergeableRegion(const AreaHypothesisBoundary* start,
...
{
    // Find all boundaries contained within the mergeable region, then find the hypotheses associated with those
    // boundaries. The merge constructor for AreaHypothesis can be used to make the new area. Once created, all the
    // boundaries adjacent to the new area hypothesis need to have one of their areas changed to replace the old area
    // with the newly merged area After that operation is complete, there are no more references to either the merged
    // hypotheses or merged boundaries in the graph, so they can be safely removed.
    ...
    // Use the type from one of the starting areas for the type of the merged area. If the condition is kSameType, we
    // know they're already the same type. If they are frontiers or invalid, then the type will be Area anyhow. The
    // constructor wants const-boundaries, so create those. Easier to do it here than create two constructors that take
    // a const or non-const boundaries. Can only access const-boundaries outside of HypothesisGraph!
    ...
    for (auto hyp : hypsToMerge) {
        hypotheses.erase(
          std::find_if(hypotheses.begin(), hypotheses.end(), utils::UniqueToRawComp<AreaHypothesis>(hyp)));
        hypToBoundaries.erase(hyp);
    }

    for (auto boundary : boundariesToMerge) {
        boundaries.erase(
          std::find_if(boundaries.begin(), boundaries.end(), utils::UniqueToRawComp<AreaHypothesisBoundary>(boundary)));
    }
...
void enqueue_edges_on_same_side_of_gateway(const AreaNode* gatewayNode,
                                           ...
                                           std::deque<AreaEdge*>& edgeQueue)
{
    // Determine which side of the gateway the current hypothesis is on. Only enqueue edges on the same side of the
    // gateway Usually there won't be any more edges. Only in the case where the gateway node is also a junction will
    // there possibly end up being an extra edge pushed into the queue for the area hypothesis
    ...
    for (auto edgeIt = gatewayNode->beginEdge(), edgeEnd = gatewayNode->endEdge(); edgeIt != edgeEnd; ++edgeIt) {
        ...
        if (nodeEdge->isLeftOfGateway(*gatewayNode) == isOnLeft) {
            auto otherEnd =
              (nodeEdge->getEndpoint(0).get() == gatewayNode) ? nodeEdge->getEndpoint(1) : nodeEdge->getEndpoint(0);
...
void enqueue_mergeable_boundaries(const std::vector<AreaHypothesisBoundary*>& boundaries,
                                  ...
                                  std::queue<AreaHypothesisBoundary*>& queue)
{
    // Any boundaries satisfying the merge condition should be put in the queue. Elsewhere in the search, duplicate
    // boundaries in the queue will be addressed
