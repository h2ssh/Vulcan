...
HypothesisType const_type_func(HypothesisType type, const AreaHypothesis& h) { return type; }
                                    ...
                                    local_area_debug_info_t&    debug)
{
    ...
    std::unique_ptr<HypothesisGraph> hypGraph(new HypothesisGraph(*areaGraph,
                                                                  &grid,
                                                                  &isovistField,
                                                                  starBuilder_));
...
void AreaParser::handleTransition(int exitedId, int enteredId, const boost::optional<Gateway>& gateway)
{
    ...
    for(AreaHypothesis* hyp : boost::make_iterator_range(prevHypGraph_->beginHypothesis(),
                                                         prevHypGraph_->endHypothesis()))
    {
                                                            ...
                                                            const AreaHypothesisAssociation& hypAssoc)
{
    ...
    for(AreaHypothesis* prior : boost::make_iterator_range(prevHypGraph_->beginHypothesis(),
                                                           prevHypGraph_->endHypothesis()))
    {
...
AreaHypothesis* AreaParser::findExitedArea(HypothesisGraph& currentGraph,
                                           const HypToHypVec& hypAssoc)
{
                                              ...
                                              const VoronoiSkeletonGrid& grid)
{
    ...
    {
        std::cout << "WARNING: AreaParser: Failed to find the entered area. Something must have changed. Not fixing any areas.\n";
        return HypVec();
    }
...
//     {
//         std::cerr << "WARNING: AreaParser: Fixed areas are invalid. Searching for new solution without any fixed areas.\n";
//         mutableAreas.clear();
//         boost::push_back(mutableAreas, boost::make_iterator_range(graph.beginHypothesis(), graph.endHypothesis()));
//         HypVec noFixedAreas;
//         solution = maxLikelihoodSolver_.solve(noFixedAreas, mutableAreas, nullptr, nullptr, *boundaryClassifier_);
//     }
...
void AreaParser::storeDebuggingHypotheses(HypothesisGraph& graph, local_area_debug_info_t& debug)
{
    ...
    assign_types_to_graph(graph, [this](const AreaHypothesis& h) { return hypClassifier_->classify(h.features()); });
...
void assign_types_to_graph(HypothesisGraph& graph, TypeFunc assigner)
{
    std::for_each(graph.beginHypothesis(),
                  graph.endHypothesis(),
                  [&assigner](AreaHypothesis* h) { h->setType(assigner(*h)); });
}
