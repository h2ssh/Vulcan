 ...
 *   - The areas on both sides of the boundary have the same type (only useful AFTER the final assignment is chosen)
 * (kSameType)
...
public:
     ...
     *
     * getOtherHypothesis uses the isSimilar method to compare the hypotheses. Using this method allows a superset or
     * subset of a hypothesis to still be considered. This is particularly useful when considering if a boundary is
     * valid.
     *
     * \param    hypothesis        Hypothesis on one side of the boundary
     * \return   Return hypothesis on the other side of the boundary. NULL if hypothesis is not associated with the
     * boundary.
     */
    ...
    // Operations
    /**
     * shouldMerge checks to see if the boundary is valid based on the current state of its associated hypotheses. The
     * validity check uses the conditions detailed in the class description. The validty of a boundary can change if the
     * classification of the hypotheses changes, thus the result of shouldMerge might be different between invocations.
     *
...
private:
     ...
     * to be replaced in the hypothesis, or the hypothesis can be swapped here. Making the local change is easier and
     * the validity can be confirmed by looking at the hypothesis, so all is well.
     *
