                              ...
                              const AreaGraph& graph)
{
    ...
    */
    ...
    std::cout << "DEBUG: find_path_endpoints: Searching for endpoints with axis:" << pathAxis << " and dir: "
        << pathDirection << " with points:\n";
    for(auto& p : points)
    {
        std::cout << p.point << '\n';
    }
...
#ifdef DEBUG_ENDPOINT_SEARCH
            std::cout << "Alignment: (p1,p2,p1Align,p2Align,pair,p1Proj,p2Proj,dist): " << points[n].point << ',' << points[i].point
                << ',' << p1Alignment << ',' << p2Alignment << ',' << pair_alignment(points[n], points[i], pathDirection)
                << ',' << p1Proj << ',' << p2Proj << ',' << distance_between_points(p1Proj, p2Proj)
                << ',' << alignment << '\n';
#endif
        }
    }
...
#ifdef DEBUG_ENDPOINT_SEARCH
    std::cout << "Selected: " << bestPair.first.point << " -> " << bestPair.second.point << " align:" << bestAlignment << " dist:" << bestDistance << "\n\n";
#endif
