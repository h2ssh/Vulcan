...
bool gateway_angles_are_similar(const gateway_endpoints_t& first,
                                const gateway_endpoints_t& second,
                                float similarityThreshold);
void connect_intersecting_gateways(gateway_endpoints_t& first, gateway_endpoints_t& second);

std::vector<local_path_fragment_t>
  create_path_fragments_from_endpoints(const std::vector<gateway_endpoints_t>& endpoints);
...
path_fragment_direction_t calculate_single_fragment_direction(const gateway_endpoints_t& endpoint,
                                                              const Point<double>& center);
path_fragment_direction_t calculate_two_fragment_direction(const gateway_endpoints_t& endpoint,
                                                           const gateway_endpoints_t& otherEndpoint);
...
inline bool path_is_ambiguous(const gateway_endpoints_t& gateway, const std::vector<gateway_endpoints_t>& endpoints)
{
    return (gateway.otherEndpointIndex != -1)
      && (gateway.ambiguousGateway || endpoints[gateway.otherEndpointIndex].ambiguousGateway);
}


BeamStarBuilder::BeamStarBuilder(const beam_star_builder_params_t& params) : params(params)
{
}
...
bool gateways_intersect(const gateway_endpoints_t& first, const gateway_endpoints_t& second, float similarityThreshold)
{
    // Intersection only occurs if beams of both gateways intersect each other and the angles are close enough to one
    // another
...
bool beam_intersects_gateway(const gateway_endpoints_t& beam, const Line<double>& gateway)
{
    ...
    return (beamA > 1.0 && beamB < 0.0) || (beamA < 0.0 && beamB > 1.0) || (beamA >= 0.0 && beamA <= 1.0)
      || (beamB >= 0.0 && beamB <= 1.0);
}
...
void connect_intersecting_gateways(gateway_endpoints_t& first, gateway_endpoints_t& second)
{
    ...
    std::cout << "DEBUG:SSStarBuilder:Connect:" << first.endpointIndex << "->" << second.endpointIndex
              << " ambiguous:" << (first.ambiguousGateway || second.ambiguousGateway) << '\n';
#endif
}
                                                              ...
                                                              const Point<double>& center)
{
    ...
    // In the case of a single fragment, nothing to compare against, so just look at the center of the gateways for
    // comparison The direction isn't particularly important, it's only real function is to distinguish between the two
    // fragments on the same path More of an identifier than a direction
...
path_fragment_direction_t calculate_two_fragment_direction(const gateway_endpoints_t& endpoint,
                                                           const gateway_endpoints_t& otherEndpoint)
{
    // The PLUS direction is the rightmost gateway. If the gateways are stacked vertically, the one with a larger y
    // value is used
...
void print_fragment(const local_path_fragment_t& fragment)
{
    std::cout << "Angle:" << fragment.gateway.direction() << " Dir:" << fragment.direction
              << " Traverse:" << fragment.navigable;
}
