...
public:
    ...
    Endpoints getPathEndpoints(void) const
    {
        assert(type_ == HypothesisType::kPath);
        return endpoints_;
    }
    ...
    double featureValue(std::size_t index) const
    {
        return (index < featureValues_.size()) ? featureValues_[index] : 0.0;
    }
