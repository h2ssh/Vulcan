...
bool AlignmentConstraint::isSatisfied(const AlignmentGraph& network, HypothesisType insideType) const
{
    ...
    if (!satisfied) {
        std::cout << "Checking constraint:" << *this << " Path End:" << numPathEnd << " Path:" << numPathDest
                  << " Dest:" << numDest << " Decision: " << numDecision << " Inside:" << insideType
                  << " Fixed:" << fixedType_ << std::boolalpha << " Satisfied? " << satisfied << '\n';
    }
