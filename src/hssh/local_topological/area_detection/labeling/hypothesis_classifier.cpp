...
namespace
{
std::string boosting_name(std::string filename) { return filename + kBoostingExtension; }
std::string boosting_isovist_name(std::string filename) { return filename + kIsovistExtension + kBoostingExtension; }
}
...
std::unique_ptr<HypothesisClassifier> HypothesisClassifier::LearnClassifier(const LabeledAreaData& examples)
{
    ...
    pathClassifier.isovistClassifier = learn_feature_classifier("path isovist classifier",
                                                                isovistFeatures,
                                                                isovistLabels,
                                                                kTrainIsovists);
    ...
    destClassifier.isovistClassifier = learn_feature_classifier("dest isovist classifier",
                                                                isovistFeatures,
                                                                isovistLabels,
                                                                kTrainIsovists);
    ...
    decisionClassifier.isovistClassifier = learn_feature_classifier("decision isovist classifier",
                                                                    isovistFeatures,
                                                                    isovistLabels,
                                                                    kTrainIsovists);

    return std::unique_ptr<HypothesisClassifier>( new HypothesisClassifier(std::move(pathClassifier),
                                                                                   std::move(destClassifier),
                                                                                   std::move(decisionClassifier)));
}
...
HypothesisTypeDistribution HypothesisClassifier::calculateRawBoostingDistribution(const HypothesisFeatures& features) const
{
    return probs_to_dist(boostingVotes(features.featureVector()));
}


HypothesisTypeDistribution HypothesisClassifier::calculateRawIsovistDistribution(const HypothesisFeatures& features) const
{
...
bool HypothesisClassifier::load(const std::string& filename)
{
    ...
    if(!pathSuccess)
    {
        std::cerr << "ERROR: HypothesisClassifier: Failed to load path classifier at:"
            << baseName << kPathExtension << '\n';
    }
    else
    {
    ...
    if(!destSuccess)
    {
        std::cerr << "ERROR: HypothesisClassifier: Failed to load destination classifier at:"
            << baseName << kDestExtension << '\n';
    }
    else
    {
    ...
    if(!decisionSuccess)
    {
        std::cerr << "ERROR: HypothesisClassifier: Failed to load decision classifier at:"
            << baseName << kDecisionExtension << '\n';
    }
    else
    {
                                                 ...
                                                 bool learnAdaBoost)
{
    ...
    std::cout << "INFO: HypothesisClassifier::LearnClassifier: Learning classifiers with "
        << posIdx.n_elem << " pos and " << negIdx.n_elem << " neg examples.\n";
