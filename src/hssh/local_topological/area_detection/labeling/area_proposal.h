...
A    * yet, then the extent will have a zero area boundary.
    ...
    */
    ...
    /**
    * getGlobalGateways retrieves all gateways associated with the proposal transformed back into the global reference frame
    ...
    */
    std::vector<Gateway> getAllGateways(const VoronoiSkeletonGrid& grid, math::ReferenceFrame frame = math::ReferenceFrame::GLOBAL) const;
    ...
    * \param    frame           Reference frame for the gateways being retrieved  (default = GLOBAL)
    */
    std::vector<Gateway> getPathGateways(math::ReferenceFrame frame = math::ReferenceFrame::GLOBAL) const { return getGatewaysOfType(AreaType::path_segment, frame); }
    ...
    * \param    frame           Reference frame for the gateways being retrieved  (default = GLOBAL)
    */
    std::vector<Gateway> getDestinationGateways(math::ReferenceFrame frame = math::ReferenceFrame::GLOBAL) const { return getGatewaysOfType(AreaType::destination, frame); }
    ...
    * \param    frame           Reference frame for the gateways being retrieved  (default = GLOBAL)
    */
    std::vector<Gateway> getDecisionGateways(math::ReferenceFrame frame = math::ReferenceFrame::GLOBAL) const { return getGatewaysOfType(AreaType::decision_point, frame); }
...
private:
    ...
    bool hasInvalidGatewayAssociation(const Gateway& gateway, const std::vector<proposal_gateway_t>& rhsGateways, const AreaExtent& rhsExtent) const;
