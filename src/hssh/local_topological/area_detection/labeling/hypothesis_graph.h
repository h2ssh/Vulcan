...
public:
    ...
    using BoundaryConstIter   = utils::SmartToRawConstIter<AreaHypothesisBoundary>;  // dereference type: const AreaHypotheisBoundary*
    ...
    */
    ...
    /**
    * simplify searches through the graph for all mergeable regions using the specified condition. The number of regions merged is
    * returned.
    *
    * NOTE: simplify calls mergeRegion if any regions are found. Therefore, if the number of merged regions returned is not 0, then
    *       all post-conditions from mergeRegion apply.
    *
    ...
    *
    * \param    grid            Grid to use for creating the AreaProposal via the toAreaProposal method in AreaHypothesis
    */
...
private:
    ...
    std::map<AreaHypothesis*, std::vector<AreaHypothesisBoundary*>> hypToBoundaries;   // keep track of edges/boundaries associated with a hypothesis

    std::map<AreaNode*, HypothesisPair> nodeToHypotheses;       // temporary storage for efficiently constructing the boundaries
    ...
    std::vector<AreaHypothesisBoundary*> findMergeableRegion(const AreaHypothesisBoundary* start, BoundaryMergeCondition condition) const;
