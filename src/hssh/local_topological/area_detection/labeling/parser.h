...
struct ParserResult
{
    ...
    explicit ParserResult(LabelingError error) : result(error) { assert(result != LabelingError::success); }
};
 ...
 *
 * Each hypothesis is classified using a set of rules described in AreaHypothesis. Each boundary is checked for
 * validity, that is, whether the areas on each side of the boundary can be adjacent within the representation. Whenever
 * the classification of a hypothesis or the validity of a boundary changes, the adjacent areas are marked as dirty and
 * must be reconsidered. This process iterates until the no changes are made.
 */
class AreaParser
{
public:
    ...
    /**
     * handleTransition incorporates information about the latest transition into the parser. The area ids are
     * associated with the most recently returned AreaProposals.
     *
     ...
     * \param    gateway             Gateway that was crossed from exited to entered (will be none on the initial
     * entered)
     */
