...
AreaHypothesis* AreaHypothesisBoundary::getOtherHypothesis(const AreaHypothesis& hypothesis) const
{
    ...
    std::cerr << "WARNING:AreaHypothesisBoundary: getOtherHypothesis called with hypothesis parameter that was neither "
                 "of the associated hypotheses.\n";
    assert(false);
#endif
...
{
    return (hypotheses[0]->getType() == hypotheses[1]->getType())
      && (hypotheses[0]->getType() != HypothesisType::kArea);
}
