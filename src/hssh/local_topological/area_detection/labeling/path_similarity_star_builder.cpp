...
float PathSimilarityStarBuilder::angleSimilarity(const Gateway& source, const Gateway& potential) const
{
    ...
    std::cout<<"DEBUG:angle_sim:"<<source.id()<<"->"<<potential.id()<<':'<<-cos(sourceDirection - potentialDirection)
             <<" dirs:("<<sourceDirection<<','<<potentialDirection<<")\n";
#endif
...
SmallScaleStar PathSimilarityStarBuilder::establishPaths(void) const
{
    ...
    std::sort(gatewayOrder.begin(), gatewayOrder.end(), [this](int lhsIdx, int rhsIdx) {
        return *std::max_element(statistics_[lhsIdx].similarityScores.begin(),
            ...
            *std::max_element(statistics_[rhsIdx].similarityScores.begin(),
                              statistics_[rhsIdx].similarityScores.end());
    });
    ...
    for(int n = 0, end = gatewayOrder.size();  n < end; ++n)
    {
        ...
        if(statistics_[index].mostSimilarGateway != -1)
        {
            ...
            // Only create a two fragment path if there is an unambiguous match between the two gateway, i.e. they point to each
            // other for their most similar gateways
...
//             if(!inPath[mostSimilarIndex])
    ...
    {
        std::cout<<"id:"<<(int)fragIt->pathId<<" dir:"<<fragIt->direction<<" nav:" << fragIt->navigable << " orient:"<<fragIt->gateway.direction()<<" gateway:"<<fragIt->gateway.boundary()<<'\n';
    }
#endif
...
{
    local_path_fragment_t gatewayFrag = create_path_fragment(pathId,
                                                             angle_to_point(boundary_.center(), gateway.center()),
                                                             gateway);
    gatewayFrag.navigable = true;
    fragments.push_back(gatewayFrag);

    local_path_fragment_t otherFrag = create_path_fragment(pathId,
                                                           angle_to_point(gateway.center(), boundary_.center()),
                                                           gateway);
...
{
    local_path_fragment_t gatewayFrag = create_path_fragment(pathId,
                                                             angle_to_point(otherGateway.center(),
                                                                                  gateway.center()),
                                                             gateway);
    gatewayFrag.navigable = true;
    fragments.push_back(gatewayFrag);
...
double distance_similarity_score(const Gateway& gateway, double direction)
{
    Line<double> normal(gateway.center(),
                              Point<double>(gateway.center().x + std::cos(direction),
                                                  gateway.center().y + std::sin(direction)));
    ...
    {
        float normalizedIntersection = distance_between_points(gateway.center(), intersectionPoint) /
            gateway.length();
...
local_path_fragment_t create_path_fragment(int index, float direction, const Gateway& gateway)
{
    ...
    if(angle_diff_abs(direction, gateway.rightDirection()) <
        angle_diff_abs(direction, gateway.leftDirection()))
    {
