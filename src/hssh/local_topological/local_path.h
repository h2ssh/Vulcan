...
public:
    ...
    */
    LocalPath(uint32_t start, uint32_t end, const Lambda& lambda)
        : startId(start)
        , endId(end)
        , lambda(lambda)
    {
    ...
    * Constructor for LocalPath.
    */
    LocalPath(uint32_t startId, uint32_t endId, const Point<float>& start, const Point<float>& end, const Lambda& lambda)
        : startId(startId)
        , endId(endId)
        , startPoint(start)
        , endPoint(end)
