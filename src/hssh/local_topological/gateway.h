...
/**
 * Gateway defines the location and properties of a gateway in the world. A gateway is the boundary between two areas in
 * metric space, usually a path and a place. The gateway forms a bridge between the metric and topological descriptions
 * of space, as topological places are based on symbols derived from gateways in the place.
 *
 * Being that the gateway abstraction separates two areas in small-scale space, the gateway representation fundamentally
 * consists of a boundary, a direction, and two normals emanating from the center of the gateway. In addition to the
 * metric coordinates of the boundary and center, the gateway contains the cells it occupies in the map it was built
 * from. The cell representation is useful when comparing a gateway to the Voronoi skeleton, which is a cell-based
 * representation.
 *
 * The boundary for a gateway is such that the .a of the boundary line is sorted topographically, so boundary.a <
 * boundary.b
 *
 ...
 *   - direction     : direction of the interior of the place from the center of the gateway -- the normal of the
 * gateway pointing into the place
...
public:
     ...
     *
     * Creates a Gateway using the associated cells in the Voronoi skeleton. The metric representation of the gateway is
     * found using the provided grid. The direction and normals are calculated.
     *
     ...
     * \param    grid            Grid in which the gateway exists -- used to determine the cells of the transformed
     * gateway \return   A new Gateway with the boundary and center transformed.
     */
     ...
     * in particular if methods will rely on the direction() method, which always is associated with the
     * leftDirection().
     */
    ...
    // Operations
    /**
     * isCellToLeft determines if the entirety of the provided cell is to the left, right, or on the gateway cell
     * boundary. The boundary is treated as a line splitting the plane in two. The cell representation is discretized,
     * so a cell is to the left only if all four corners of the cell are to the left of the cell boundary. If only some
     * corners are left of the boundary, then the cell is on the boundary. If no corners are left of the boundary, the
     * cell is right of the boundary.
     *
    ...
    /**
     * isPointToLeft determines if a metric point is to the left, right, or on the gateway boundary. The boundary is
     * treated as a line splitting the plane of the world into two. The point is assumed to be infinitesimally small.
     * Thus, the point must fall exactly on the boundary line in order to be considered on the line.
     *
    ...
    /**
     * intersectsWithCellBoundary finds the intersection between a line and the gateway if one exists. If it does exist,
     * the result is stored in intersection.
     *
    ...
    /**
     * intersectsWithBoundary finds the intersection between a line and the gateway if one exists. If it does exist, the
     * result is stored in intersection.
     *
     * \param        line            Line to check for intersection
...
struct SimilarGatewayComp
{
    ...
    bool operator()(const Gateway& rhs) { return lhs.isSimilarTo(rhs); }
};
