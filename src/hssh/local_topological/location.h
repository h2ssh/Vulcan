...
public:
     ...
     * \param    areaId          Id of the area entered
     */
    LocalLocation(int32_t mapId, int32_t areaId) : mapId_(mapId), areaId_(areaId), isInitialArea_(true) { }
     ...
     */
    LocalLocation(void) : mapId_(-1), areaId_(-1) { }
    ...
    // Operators
    /**
     * Two LocalLocations are the same if they belong to the same map, have the same area id, and the same entry
     * gateway.
     */
    ...
    bool operator!=(const LocalLocation& rhs) const { return !(*this == rhs); }
...
private:
    ...
    {
        ar(mapId_, areaId_, entry_, isInitialArea_);
    }
};
