...
isovist_orientation_gateway_generator_params_t
  load_isovist_orientation_gateways_params(const utils::ConfigFile& config);
...
skeleton_builder_params_t load_skeleton_builder_params(const utils::ConfigFile& config)
{
    ...
    params.minDeadEndAngleSeparation =
      config.getValueAsFloat(SKELETON_BUILDER_HEADING, MIN_DEAD_END_ANGLE_KEY) * M_PI / 180.0f;
...
isovist_voronoi_gateway_generator_params_t load_isovist_voronoi_gateways_params(const utils::ConfigFile& config)
{
    ...
    params.minGatewayStraightness =
      config.getValueAsDouble(ISOVIST_GATEWAYS_HEADING, GATEWAY_STRAIGHTNESS_KEY) * M_PI / 180.0;
...
area_transition_detector_params_t load_area_transition_params(const utils::ConfigFile& config)
{
    ...
    params.minDistBetweenSameGatewayTransitions =
      config.getValueAsDouble(kTransitionDetectorHeading, kMinTransitionDistKey);
