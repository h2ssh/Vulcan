...
TEST(GatewayAnglesCloseTest, CloseAnglesAreClose)
{
    ...
    Line<double> boundary(0, 0, kBoundaryLength, 0);
    Line<double> belowPosThresholdBoundary(0,
                                           0,
                                           kBoundaryLength * std::cos(closeAngle),
                                           kBoundaryLength * std::sin(closeAngle));
    Line<double> belowNegThresholdBoundary(0,
                                           0,
                                           kBoundaryLength * std::cos(-closeAngle),
                                           kBoundaryLength * std::sin(-closeAngle));
    Line<double> onPosThresholdBoundary(0, 0, kBoundaryLength * std::cos(onAngle), kBoundaryLength * std::sin(onAngle));
    Line<double> onNegThresholdBoundary(0,
                                        0,
                                        kBoundaryLength * std::cos(-onAngle),
                                        kBoundaryLength * std::sin(-onAngle));
    Line<double> abovePosThresholdBoundary(0,
                                           0,
                                           kBoundaryLength * std::cos(aboveAngle),
                                           kBoundaryLength * std::sin(aboveAngle));
    Line<double> aboveNegThresholdBoundary(0,
                                           0,
                                           kBoundaryLength * std::cos(-aboveAngle),
                                           kBoundaryLength * std::sin(-aboveAngle));
