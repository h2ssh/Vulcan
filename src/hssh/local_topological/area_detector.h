...
struct AreaDetectorResult
{
    ...
    explicit AreaDetectorResult(const LocalTopoMap& map) : map(map), result(LabelingError::success) { }

    // For errors, pass the error code. Error can't be success because there must be proposals!
    explicit AreaDetectorResult(LabelingError error) : map(boost::none), result(error)
    {
 ...
 *
 * Because the areas in the map are tracked over time, calls to detectAreas() with new maps will build the
 * representation over time. As such, if the LPM changes in some dramatic way, say a log is restarted, then the areas
 * should be reset via resetAreas() to ensure that the newly detected areas are invalidated by pre-existing state
 * information.
 */
class AreaDetector
{
public:
     ...
     */
    AreaDetector(const utils::ConfigFile& config, const utils::CommandLine& cmdLine, const std::string& mapName);
