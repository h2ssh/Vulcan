...
/**
* LocalPathSegment describes a path segment running between two decision points. A path segment is a portion of the environment
* of approximately constant width that is longer than it is wide, usually substantially so. The path segment is described by
* the places on either end and the change in the reference frame of those places, called lambda, which is the distance of the
* path segment and the change in the principal axes of small-scale space.
*
* A path segment contains two sets of destinations, those on the left and right side of the segment. Left and right are based on
* the direction from the entry to the exit. For example, if the exit is north of the entry, then the left destinations are on the
* west side of the segment and the right destinations are on the east side.
*
* The entry of a path segment corresponds to the plus place. The exit is the minus place. Thus, for a normal traversal of a path
* segment the robot moves in the MINUS direction.
*
