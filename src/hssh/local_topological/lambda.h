...
struct Lambda
{
    ...
    // Constructors
    explicit Lambda(float x = 0,
                    float y = 0,
                    float theta = 0,
                    float xVariance = 0.5f,
                    float yVariance = 0.5f,
                    float thetaVariance = 0.1f);
...
private:
    ...
    {
        ar(x, y, theta, xVariance, yVariance, thetaVariance, transformDistribution);
...
{
    out << "mean: (" << lambda.x << ',' << lambda.y << ',' << lambda.theta << ") var: (" << lambda.xVariance << ','
        << lambda.yVariance << ',' << lambda.thetaVariance << ')';
    return out;
}
