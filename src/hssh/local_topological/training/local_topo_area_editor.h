...
public:
    ...
    /**
     * buildSkeleton constructs the VoronoiSkeletonGrid from the provided LPM. The constructed skeleton will
     * subsequently be used for any editing operations.
     *
     ...
     * after the areas have been labeled. Once labeled, the simplification process will dramatically reduce the number
     * of hypotheses that might need further attention.
     */
    void simplifyAreas(void);

    /**
     * mergeAreas merges a collection of areas into a single area. Note that merged areas can not be merged further.
     * Therefore, take care that the merge contains all the desired areas from the get-go. Furthermore, merged areas can
     * not be further simplified. Consequently, merging should only take place after labeling and simplification of the
     * areas.
     *
    ...
    /**
     * resetAreas resets all merges and labeling operations that have been performed, returning the editor to its
     * original state.
     */
...
private:
    ...
    std::vector<std::unique_ptr<AreaHypothesis>>
      mergedHypotheses_;   // the hypotheses that have been merged and are owned by the editor, not the HypGraph
    ...
    // INVARIANT: hypotheses_.size() <= mergeHypotheses_.capacity() --- allows us to know that mergedHypotheses_ will
