...
int main(int argc, char** argv)
{
    ...
    arguments.push_back(
      {utils::kConfigFileArgument, "Configuration file for calculating gateway features", true, "local_topo_hssh.cfg"});
                                                              ...
                                                              const LabelsFile::BuildingLabels& labels)
{
    ...
    for (auto& mapName : boost::make_iterator_range(features.data.beginMaps(), features.data.endMaps())) {
        ...
        assert(storedInfo);
        mapGwyFeatures[mapName] =
          extract_gateway_features_default(*storedInfo->edges, *storedInfo->isovists, features.radius);
    }
    ...
    for (double minProb = 0.5; minProb <= 1.0; minProb += 0.02) {
        ...
        classifier->save(kClassifierTempFile);
        //         GatewayLocator locator(std::make_unique<ClassifierBasedGenerator>(features.radius,
        //         kClassifierTempFile));
        ...
        for (auto& mapName : boost::make_iterator_range(features.data.beginMaps(), features.data.endMaps())) {
            ...
            for (auto& isovist : *storedInfo->isovists) {
                if (gwyFeatures.find(isovist.cell()) != gwyFeatures.end()) {
                    auto classification =
                      classifier->classifyGateway(gwyFeatures.at(isovist.cell()), GatewayClassifier::adaboost);
    ...
    for (auto& res : results) {
        double precision =
          std::get<kTruePosIdx>(res) / static_cast<double>(std::get<kTruePosIdx>(res) + std::get<kFalsePosIdx>(res));
        double recall =
          std::get<kTruePosIdx>(res) / static_cast<double>(std::get<kTruePosIdx>(res) + std::get<kFalseNegIdx>(res));

        out << std::get<kThreshIdx>(res) << '\t' << std::get<kErrorIdx>(res) << "\t\t" << std::get<kFalseNegIdx>(res)
            << '\t' << std::get<kTruePosIdx>(res) << '\t' << std::get<kFalsePosIdx>(res) << '\t' << precision << '\t'
            << recall << '\t' << std::endl;
    }
