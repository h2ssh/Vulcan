...
{
    lpm_export_header_t header ={lpm.getWidthInCells(),
                                 lpm.getHeightInCells(),
                                 lpm.metersPerCell(),
                                 lpm.getBoundary()
    };
    assert(lpm.getWidthInCells());
    assert(lpm.getHeightInCells());
    assert(header.width);
    std::size_t headerRows=(sizeof(lpm_export_header_t)+header.width*3-1)/(header.width*3);

    Image image(header.width, header.height+headerRows, Colorspace::RGB);

    unsigned char* pt=image.getPixelBuffer();
    memcpy(pt, &header, sizeof(lpm_export_header_t));
    pt+=header.width*3*headerRows;

    for(std::size_t y = lpm.getHeightInCells()-1; y < lpm.getHeightInCells() ; --y)
    {
        for(std::size_t x = 0; x < lpm.getWidthInCells(); ++x, pt+=3)
        {
                pt[0]=0;
                pt[1]=lpm.getCostNoCheck(Point<int>(x, y));
                pt[2]=0;
        }
    }

    return vision::save_image_to_file(image, filename);
}
...
bool import_lpm_labels_from_image(const std::string& filename, LocalPerceptualMap& lpm){
    ...
    {
        std::cerr<<"ERROR::import_lpm_labels_from_image: Loaded image was not RGB: "<<filename<<'\n';
        return false;
    }

    lpm_export_header_t header;
    unsigned char* pt=image.getPixelBuffer();
    memcpy(&header, pt, sizeof(lpm_export_header_t));
    std::size_t headerRows=(sizeof(lpm_export_header_t)+header.width*3-1)/(header.width*3);
    pt+=header.width*3*headerRows;

    lpm.setGridSizeInCells(header.width,header.height);
    lpm.changeBoundary(header.boundary);

    for(std::size_t y = header.height - 1; y < header.height; --y)
    {
        for(std::size_t x = 0; x < header.width; ++x, pt+=3)
        {
            char r=pt[0];
            char g=pt[1];
            char b=pt[2];
            if(r==0 && b==0){
                if(g==0){
                    //nothing at all
                    lpm.setTypeNoCheck(Point<int>(x, y),kUnseen);
                }
                else
                {
                    //unlabeled, nop
                    lpm.setTypeNoCheck(Point<int>(x, y),kUnlabeled);
                }
            }
            else if(b>r && r>g){
                //BGR: questionable
                lpm.setTypeNoCheck(Point<int>(x, y),kQuestionable);
            }
            else if(r>g && g>b){
                //RGB: motion
                lpm.setTypeNoCheck(Point<int>(x, y),kMoving);
            }
            else if(r>b && b>g){
                //RBG: Diffuse core
                lpm.setTypeNoCheck(Point<int>(x, y),kDiffuseCore);
            }
            else if(b>r && r>g){
                //BRG: Glass core
                lpm.setTypeNoCheck(Point<int>(x, y),kGlassCore);
            }
            else
            {
                assert(!"Error:unhandled label type imported");
            }
        }
    }
    return true;
