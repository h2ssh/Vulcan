...
public:
    ...
    MapSaver(const std::string& basename, double percentToSave)
    : basename_(basename)
    , percentToSave_(percentToSave)
    {
        srand48(std::time(0));
...
int main(int argc, char** argv)
{
    ...
    utils::CommandLine cmdLine(argc, argv, { { kPercent, "Percent of maps to be saved", true, "1" },
                                             { kBase, "Base of the filename for maps", false, "error" } });
