...
#ifdef DEBUG
    std::cout << "DEBUG:MultiFloorMapper: Entered! Floor:" << currentFloor->getId()
              << " Elevator:" << currentElevator->getId()
...
{
    std::shared_ptr<Elevator> newElevator(
      new Elevator(nextElevatorId++, input.pose, currentFloor->getId(), input.mapper.getLPM()));
...
{
    MultiFloorMapIO::save(MultiFloorMap(currentFloor->getId(), currentElevator->getId(), floors, elevators),
                          "multifloor.map");
}
