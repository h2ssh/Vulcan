...
std::shared_ptr<Floor> load_floor(const utils::TaggedMap& tags,
...
void save_elevator(const std::shared_ptr<Elevator>& elevator, std::ofstream& out)
{
    ...
    for (const auto& boundary : elevator->getBoundaries()) {
        ...
        out << "\t<" << ELEVATOR_BOUNDARY_TAG << ">" << boundary.first << ' ' << boundary.second.bottomLeft.x << ' '
            << boundary.second.bottomLeft.y << ' ' << boundary.second.topRight.x << ' ' << boundary.second.topRight.y
    ...
    for (const auto& floor : elevator->getTransitions()) {
        for (const auto& transition : floor.second) {
            out << "\t<" << ELEVATOR_TRANSITION_TAG << ">" << floor.first << ' ' << transition.endFloor << ' '
                << transition.height << "</" << ELEVATOR_TRANSITION_TAG << ">\n";
        }
    }
...
std::shared_ptr<Floor> load_floor(const utils::TaggedMap& tags, const std::vector<std::shared_ptr<Elevator>>& elevators)
{
    ...
    for (auto str : strings) {
        ...
        auto elevatorComp = [elevatorId](const std::shared_ptr<Elevator>& elevator) {
            return elevator->getId() == elevatorId;
        };
