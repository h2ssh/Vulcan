 ...
 *
 * Aside from accessors for stored properties of the map (areas + transitions), the map can be manipulated in three
 * ways:
 *
 *   - addArea : adds a new area to the map and connects it to an existing area via the specified transition
 *   - closeLoop : establishes that two areas in the map actually share a transition and marks the transition
 * accordingly
...
private:
    ...
    pose_distribution_t estimateReferenceFrame(const GlobalArea& exitedArea, const GlobalTransition& exitTransition);
