...
void TopologicalVisitSequence::visitAreaTransition(const AreaTransitionEvent& event)
{
    ...
    if (currentVisit_) {
        ...
        if (currentVisit_->areaId() != event.exitedId()) {
            std::cerr
              << "WARNING: TopologicalVisitSequence: The currently visited area was exited by the next area transition."
...
void TopologicalVisitSequence::visitTurnAround(const TurnAroundEvent& event)
{
    ...
    if (currentVisit_) {
        ...
        if (currentVisit_->areaId() != event.areaId()) {
            std::cerr << "WARNING: TopologicalVisitSequence: Received a TurnAroundEvent for a different area than the "
                         "current visit:"
