...
GlobalLocationDistribution TopologicalLocalizer::localize(const TopologicalState& state,
                                                          const TopologicalVisit& visit)
{
    ...
    {
        newLocation.pathDirection = handleTurnAroundEvents(state,
                                                           next_path_event(state, visit),
                                                           visit.endPathEvents());
    }
                                                                 ...
                                                                 const TopologicalVisit& visit)
{
    ...
    auto newPlace = create_global_place_from_local_place(GlobalArea(),
                                                         visit.entryEvent().transitionGateway(),
                                                         *visit.localArea());
    bool areCompatible = are_cycles_compatible(place->cycle(),
                                               location.entryTransition,
                                               newPlace.first.cycle(),
                                               newPlace.second);
    if(!areCompatible)
    {
#ifdef DEBUG_COMPATIBILITY
        std::cerr << "DEBUG: TopologicalLocalizer: Threw away place after getting there because it wasn't actually compatible.\n"
            << " From map: (" << place->cycle() << " entry: " << location.entryTransition
            << " From visit: (" << newPlace.first.cycle() << " entry: " << newPlace.second << '\n';
