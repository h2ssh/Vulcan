...
void DeterministicActionLocalizer::handleEnteredPlaceEvent(const entered_place_action_t& event, const TopoMapPtr& map)
{
...
#ifdef DEBUG_PLACE_EVENT
        std::cout<<"DEBUG:DeterministicActionLocalizer:Arrived at frontier place along path "<<state.pathId<<" from place "<<state.pathState.entryPlaceId<<'\n';
#endif
    }
    else // must be at some place
    {
        ...
        {
            std::cerr<<"ERROR:DeterministicActionLocalizer:Hypothesis map doesn't contain the target place:"<<state.pathState.targetPlaceId<<std::endl;
            assert(false);
        }
    }
...
void DeterministicActionLocalizer::handleExitedPlaceEvent(const exited_place_action_t& event, const TopoMapPtr& map)
{
    ...
    if(map->hasPlace(state.placeId))
    {
        ...
        global_path_fragment_t exitFragment = exitPlace.getStar().findExitFragment(event.topology, event.entryPath, event.exitPath);

        if(map->hasPath(exitFragment.pathId))
        {
            const GlobalPath& currentPath = map->getPath(exitFragment.pathId);
            GlobalPathSegment segment     = currentPath.nextSegmentAlongPath(path_transition_t(exitPlace.getId(), exitFragment.fragmentId), exitFragment.direction);
            ...
            state.pathState.targetPlaceId   = (exitFragment.direction == PATH_MINUS) ? segment.getMinusTransition().placeId : segment.getPlusTransition().placeId;
        ...
        {
            std::cerr<<"ERROR:DeterministicActionLocalizer:Failed to find path "<<exitFragment.pathId<<" associated with exit segment "<<exitFragment.fragmentId<<std::endl;
            assert(false);
        }
    }
    else
    {
        std::cerr<<"ERROR:DeterministicActionLocalizer: Failed to find prior place in the map:"<<state.placeId<<std::endl;
        assert(false);
    }
...
void DeterministicActionLocalizer::handlePathEvent(const reverse_path_direction_action_t& event, const TopoMapPtr& map)
{
    ...
    if(state.onPath)
    {
        ...
        std::cout<<"DEBUG:DeterministicActionLocalizer: Reversing path direction. New target:"<<state.pathState.targetPlaceId<<'\n';
#endif
    ...
    {
        std::cerr<<"ERROR:DeterministicActionLocalizer: Tried to apply reverse path direction while not on a path"<<std::endl;
        assert(state.onPath);
    }
}


GlobalLocation DeterministicActionLocalizer::updateTargetPlaceState(const GlobalLocation& state, const LargeScaleStar& enteredPlaceStar, GlobalPlace& targetPlace)
{
    ...
    // When entering a place from a path, the direction of the segment of entry will be opposite the motion along the path
...
#ifdef DEBUG_PLACE_EVENT
    std::cout<<"DEBUG:DeterministicActionLocalizer:Arrived at expected place "<<targetPlace.getId()<<" along path "<<updatedState.pathId
                <<" through segment "<<updatedState.placeState.entryFragmentId<<'\n';
#endif

    // If the place and target don't match, just issue a warning because the non-match will cause the place to be pruned by the mapper
    if(!targetStar.areStarsCompatible(enteredPlaceStar))
    {
        ...
        std::cerr<<"WARNING:DeterministicActionLocalizer:Failed to match entered place topology with expected topology.\n";
    }
