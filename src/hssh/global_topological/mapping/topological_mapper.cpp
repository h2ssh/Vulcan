...
TopologicalMapper::TopologicalMapper(MetricMapCache& manager) : manager(manager)
{
}
                                                 ...
                                                 TopoMapPtr hypothesis)
{
    ...
    // Use the rotation from the previous place because the defined the relative direction the robot was moving in
    // between places Once arrived, then want the transform in the current place because appearance models, etc. need
    // that correlation
    GlobalLocation location = hypothesis->getGlobalLocation();

#ifdef DEBUG_NEW_PLACE
    std::cout << "DEBUG:TopologicalMapper:Moved to known place " << newPlace.getId() << " with lambda "
              << lambda.rotate(location.localToReferenceTransform.theta) << '\n';
#endif

    TopoMapPtr newHypothesis =
      tree.copyMapToNextDepth(hypothesis, lambda.rotate(location.localToReferenceTransform.theta));

    location = newHypothesis->getGlobalLocation();
    location.localToReferenceTransform = findTransformBetweenPlaces(newPlace,
                                                                    newHypothesis->getPlace(location.placeId),
                                                                    star.getEntryPathFragment().fragmentId);
...
#ifdef DEBUG_NEW_PLACE
    std::cout << "DEBUG:TopologicalMapper:Adding new place " << newPlace.getId() << " with lambda "
              << lambda.rotate(location.localToReferenceTransform.theta) << '\n';
#endif

    TopoMapPtr newHypothesis = tree.addNewPlace(hypothesis,
                                                newPlace,
                                                star.getEntryPathFragment().fragmentId,
                                                lambda.rotate(location.localToReferenceTransform.theta));
...
{
    /*
     * Take a slow approach for now, and iterate through all places, checking each to see if it is a frontier or not,
     * and if so, whether the stars match. If the hypothesis has a better internal rep, this search can be drastically
     * reduced.
     *
     * The LargeScaleStar happily contains potentialEntrySegments() method that will determine all possible links
     * between two stars, given the topology and layout of frontiers. For each of these segments, create a
     * place_connection_t and spawn a new map hypothesis.
     */
    ...
    place_connection_t start = {previousPlace.getId(),
                                previousPlace.getStar().getFragmentWithId(location.pathState.entryFragmentId)};
    ...
    for (auto placeIt = places.begin(), placeEnd = places.end(); placeIt != placeEnd; ++placeIt) {
        ...
        if (placeStar == star && !potentialFragments.empty()) {
            std::cout << "DEBUG:TopologicalMapper:Potential match with " << placeIt->second.getId()
                      << " Num segments:" << potentialFragments.size() << '\n';
        }
#endif

        for (auto segmentIt = potentialFragments.begin(), segmentEnd = potentialFragments.end();
...
pose_t TopologicalMapper::findTransformBetweenPlaces(const GlobalPlace& currentPlace,
                                                     const GlobalPlace& referencePlace,
                                                     int referenceId)
{
