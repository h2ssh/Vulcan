...
public:
    ...
    /**
     * changeRootState changes the root of the tree. All nodes that are not descendents of this root state are
     * discarded. Changing the root state amounts to asserting some absolutely true condition about the environment. It
     * can dramatically reduce the size of the tree, even down to a single node.
     *
...
private:
    ...
    struct TreeNode
    {
        ...
        {
            ar(id, depth, parentId, childIds, hadChildren, state);
        }
    };
