...
public:
    ...
    // Mutators
    /**
     * setGlobalLocation sets the global location of the robot in the current hypothesis. The global location can change
     * without a hypothesis branching.
     */
...
private:
    ...
    int createNewPath(GlobalPlace& newPlace,
                      const global_path_fragment_t& plusFragment,
                      const global_path_fragment_t& minusFragment);

    // Create a new loop closure
    void connectPlaces(const place_connection_t& to,
                       const place_connection_t& from,
                       const Lambda& lambda,
                       const pose_t& transform);
    void
      connectPlacesOnDifferentPaths(const place_connection_t& to, const place_connection_t& from, const Lambda& lambda);
