...
void ExhaustiveSearchMapper::updateMap(const topology_action_t& action, const topology_measurements_t& measurements)
{
     ...
     *           create a new map hypothesis where the previous place on the path and the matching place are joined by
     * the specified segment add the new hypothesis to maps to evaluate 2) Eliminate all non-planar graphs (can this be
     * done on-the-fly?) 3) Calculate the likelihood of the new hypotheses
     */
...
void ExhaustiveSearchMapper::createNewHypotheses(const LargeScaleStar& actionStar,
                                                 const topology_measurements_t& measurements)
{
    // Create a copy of the active hypotheses, otherwise the addition of new hypotheses will snowball into an infinite
    // loop
    ...
    for (auto mapIt = leaves.begin(), mapEnd = leaves.end(); mapIt != mapEnd; ++mapIt) {
        ...
        if (atFrontierPlace(*mapIt)) {
            std::vector<TopoMapPtr> closures =
              createLoopClosureHypotheses(actionStar, measurements.lambda, measurements.placeId, *mapIt);
            TopoMapPtr newPlace =
              createNewPlaceHypothesis(actionStar, measurements.lambda, measurements.placeId, *mapIt);
