...
void transform_rectangle(math::Rectangle<float>& rect,
                         const pose_t& center);   // move a rectangle to be centered around a new position

////////////////////////////////////// EdgeLengthEvaluator definition ////////////////////////////////////////
EdgeLengthEvaluator::EdgeLengthEvaluator(const edge_length_evaluator_params_t& params) : params(params)
{
}
...
LPMMatchEvaluator::LPMMatchEvaluator(const lpm_match_evaluator_params_t& params) : params(params)
{
}
...
///////////////////////////////////////////
PlaceLayoutCompatibilityEvaluator::PlaceLayoutCompatibilityEvaluator(
  const place_layout_compatibility_evaluator_params_t& params)
: params(params)
{
}
...
ChiLikelihoodEvaluator::ChiLikelihoodEvaluator(const chi_likelihood_evaluator_params_t& params) : params(params)
{
}
...
double ChiLikelihoodEvaluator::calculateLogLikelihood(const TopologicalState& state, const MetricMapCache& places)
{
    ...
    for (auto idToSeg : state.map->segments()) {
        ...
        std::cout << "Segment chi lambda:" << idToSeg.first << '\n'
                  << lambda.transformDistribution.getMean() << lambda.transformDistribution.getCovariance();
        ...
        for (auto& meas : segment.getAllLambdas()) {
            ...
            std::cout << "meas lambda prob:" << measLoglihood << '\n'
                      << meas.transformDistribution.getMean() << meas.transformDistribution.getCovariance();
                                     ...
                                     const CachedMap& placeB)
{
    ...
    if (boundaryA.overlap(boundaryB) > 0) {
        ...
        std::cout << "DEBUG:PlaceCompatibility:BoundaryA:" << boundaryA << " BoundaryB:" << boundaryB
                  << " Overlap:" << boundaryA.overlap(boundaryB) << '\n';
    }
#endif
