...
private:
    // When calculating the residuals, need to know which values in the p-vector correspond to the measurements in the
    // x-vector When creating p-vector, store which placeId corresponds to which p-index. Then when building the
    // x-vector, the path segments give the placeIds involved and idToPIndex can be used to determine the correct
    // p-index to use for calculating the new lambdas when calculateLambdas is called by levmar_func
