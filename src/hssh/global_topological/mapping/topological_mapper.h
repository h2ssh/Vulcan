...
public:
     ...
     */
    virtual void updateMap(const topology_action_t& action, const topology_measurements_t& measurements) = 0;
...
protected:
     ...
     * \param    localPlaceId    Id of the LocalPlace which will be matched to the stored place for each loop closure
     * hypothesis \param    hypothesis      Map hypothesis to be extended with new loop closures \return   A vector of
     * map hypotheses representing all of discovered possible loop closures.
     */
...
private:
    pose_t
      findTransformBetweenPlaces(const GlobalPlace& currentPlace, const GlobalPlace& referencePlace, int referenceId);
};
