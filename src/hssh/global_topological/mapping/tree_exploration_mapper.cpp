...
TreeExplorationMapper::TreeExplorationMapper(MetricMapCache& manager) : TopologicalMapper(manager)
{
}
...
void TreeExplorationMapper::updateMap(const topology_action_t& action, const topology_measurements_t& measurements)
{
                                               ...
                                               boost::shared_ptr<TopologicalMapHypothesis> hypothesis)
{
    ...
    if (state.placeId == PLACE_FRONTIER_ID) {
        ...
        boost::shared_ptr<TopologicalMapHypothesis> newHypothesis =
          tree.addNewPlace(hypothesis, newPlace, newPlace.getEntryFragment().fragmentId, measurements.lambda);
...
void TreeExplorationMapper::createInitialMap(const entered_place_action_t& action,
                                             const topology_measurements_t& measurements)
{
    LargeScaleStar largeStar(action.topology, action.entryPath);
    GlobalPlace newPlace(PLACE_FRONTIER_ID, measurements.placeId, largeStar);
