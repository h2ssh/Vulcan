 ...
 *
 * The measurement likelihood is calculated by some collection of HypothesisLikelihoodEvaluator instances. The
 * evaluators are assumed to be independent so the Naive Bayes assumption can be used: p(measurements | map, actions) =
 * p(measurement_A | map, actions) * ... * p(measurement_N)
 *
 ...
 *
 *   hypothesis_probability_t calculateProbability(const TopologicalMapHypothesis& map, const topology_measurements_t&
 * measurements);
