...
*   - a direction indicating the side of the path on which the fragment lies. Each path has a direction. If you draw a line
*     through the place from + to - along the path, the star fragment first intersected would be + and the second would be -
...
public:
    ...
    */
    ...
    /**
    * toSmallScaleStar creates a SmallScaleStar with the equivalent topology and fragment indices to the LargeScaleStar.
    * In the created small-scale star, the local_path_fragment_t ids will be the same as the associated global_path_fragment_t ids.
    */
    ...
    *                                   moving to this fragment from inside the place is moving in direction along the path
    */
    ...
    */
    std::vector<global_path_fragment_t> potentialEntryFragments(const LargeScaleStar& largeStar, const global_path_fragment_t& entryFragment) const;

    /**
    * areStarsCompatible checks to see if the star passed in as an argument is compatible with this star. A compatible star
    * will having a matching topology and a compatible entry fragment. For the entry fragments to be compatible, they need
    * to be equivalent using only clockwise rotations of the star.
    *
