 ...
 * the environment. A GlobalPlace is represented symbolically as a GlobalTransitionCycle. The GlobalPlace contains a
 * GlobalPath for each path segment in the large-scale cycle_. A GlobalPlace corresponds to a node in the graph.
 */
class GlobalPlace
{
public:
     ...
     */
    GlobalPlace(Id id, AreaType type, Id metricId, const GlobalTransitionCycle& cycle);
...
private:
    ...
    {
        ar(id_, type_, cycle_, metricPlaceIds_);
    }
};
