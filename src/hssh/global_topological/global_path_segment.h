 ...
 * to motion in that direction. For example, moving into a Place from the GlobalPathSegment through the plus transition
 * is motion in the plus direction. A GlobalPathSegment also contains a lambda value, which is the measured transform of
 * the plus place to the minus place in the plus place frame.
 *
 * Each GlobalPathSegment is assigned a unique id on construction. The unique id can be used to in a std::map to make
 * sure the correct path segment is referred to. The id is not used for testing equality. Equality relies on the
 * GlobalTransitions. The unique id is only assigned in the non-default constructors.
 */
...
private:
    ...
    {
        ar(id_, plusTransition_, minusTransition_, leftSequence_, rightSequence_, initialLambda_, lambdas_);
    }
};
