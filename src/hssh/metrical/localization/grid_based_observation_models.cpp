                                              ...
                                              particle_filter_debug_info_t*     debug)
{
    ...
    for(std::size_t i = 0; i < movingScan.size(); ++i)
    {
        ...
        double rayLikelihood =  (rayWeight > params.minRayWeight) ? beamLikelihood(ray.range,
                                                                                   rayBearing,
                                                                                   ray.position,
                                                                                   map) : 0.0;
...
double BeamObservationModel::computeLineWeight(const laser::laser_scan_lines_t& scan, int rayIndex, double rayAngle)
{
    ...
    if(rayLineIndex != -1)
    {
        ...
        if(lineLength > 0.05) // ignore very short lines
        {
            ...
            lineWeight = std::abs(rayAngleX*lineOrientationX + rayAngleY*lineOrientationY); // weight based on beam incident angle
        }
    }
                                            ...
                                            const OccupancyGrid&       map)
{
    ...
    return pHit.likelihood(radius)*params.zHit      +
            pShort.likelihood(radius)*params.zShort +
            pRand.likelihood(radius)*params.zRand   +
            zMaxLikelihood*params.zMax;
}
