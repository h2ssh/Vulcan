                                                 ...
                                                 particle_filter_debug_info_t* debug)
{
    ...
    bool hasRobotMoved =
      motionModel->calculateUpdateMotion(data, prior.gaussianApproximation, data.endTime - data.startTime);
    ...
    std::cout << "INFO: ParticleFilter: Mean:" << currentPose.toPose() << " Std:(" << stdX << ',' << stdY << ','
              << stdTheta << ")\n";
#endif
...
void ParticleFilter::changeReferenceFrame(const pose_t& referenceFrame)
{
    ...
    // HACK: Not quite correct because the covariance would need to be transformed by the Jacobian of the rotation
    // matrix but should be fine for just the single update the will happen before the new reference frame has kicked
    // fully into gear
                                                    ...
                                                    particle_filter_debug_info_t* debug)
{
    ...
    //
    //     int samplesPerThread = numSamples;// / kNumThreads; // Add one to the positions/thread to account for
    //     truncating due to integer division
    //                                                     // The last thread will take up to 3 fewer isovist
    //                                                     calculations
    ...
    // //     plot[scan.scan.laserId] << "plot '-' with lines title 'Dist Weights', '-' with lines title 'Line
    // Weights'\n";
    //     plot.send1d(weights);
...
void ParticleFilter::calculateProposalDistribution(const MultivariateGaussian& motionTransform)
{
    ...
    Matrix proposalCovariance =
      currentPose.uncertainty.getCovariance() + arma::trans(rotation) * motionTransform.getCovariance() * rotation;
