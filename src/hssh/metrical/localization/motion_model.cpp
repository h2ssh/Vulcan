...
MotionModel::MotionModel(const motion_model_params_t& params)
: params_(params)
{
                                        ...
                                        int64_t                           deltaTime)
{
    ...
    MotionModelDistribution odomDist(data.odometry.front(), data.odometry.back(), data.imu.begin(), data.imu.end(), params_);
                                                 ...
                                                 std::vector<imu_data_t>& interpolatedImu)
{
    ...
    for(std::size_t odomIdx = 0; odomIdx < odom.size(); ++odomIdx)
    {
        ...
        for(; imuIdx < imu.size(); ++imuIdx)
        {
            ...
            {
                auto newImu = interpolate_imu_data(imu[imuIdx-1],
                                                            imu[imuIdx],
                                                            odom[odomIdx].timestamp);
