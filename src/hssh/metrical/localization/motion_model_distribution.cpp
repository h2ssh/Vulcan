    ...
    if(haveImu)
    {
        if((startOdom.timestamp != beginImu->timestamp) || (endOdom.timestamp != (endImu-1)->timestamp))
        {
            std::cout << "ERROR: Model mismatch between IMU and odometry! Odom:" << startOdom.timestamp << "->"
                << endOdom.timestamp << " IMU:" << beginImu->timestamp << "->" << (endImu-1)->timestamp << std::endl;
        }
...
//         imuTheta *= -1;
    ...
    if(moved_)
    {
        ...
        if(haveImu)
        {
            ...
            odomCov(kTransIndex, kTransIndex) = kMinNoiseTrans + std::pow(params.alpha3 * trans, 2) + std::pow(params.alpha4 * (std::abs(rot1) + std::abs(rot2)), 2);
            odomCov(kRot2Index, kRot2Index)   = kMinNoiseRot + imuVar;
        }
        else
        {
            odomCov(kRot1Index, kRot1Index)   = kMinNoiseRot + std::pow(params.alpha1 * std::abs(rot1), 2) + std::pow(params.alpha2 * trans, 2);
            odomCov(kTransIndex, kTransIndex) = kMinNoiseTrans + std::pow(params.alpha3 * trans, 2) + std::pow(params. alpha4 * (std::abs(rot1) + std::abs(rot2)), 2);
            odomCov(kRot2Index, kRot2Index)   = kMinNoiseRot + std::pow(params.alpha1 * std::abs(rot2), 2) + std::pow(params.alpha2 * trans, 2);
        }
    ...
    std::cout << "DEBUG:MotionModel:Mean:\n(" << rot1 << ',' << trans << ',' << rot2 << ")\n Cov:\n" << odomCov
        << " IMU:" << imuTheta << " Encoders:" << odometryTheta <<'\n'
