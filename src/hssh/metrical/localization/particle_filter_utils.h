 ...
 */
void draw_samples_from_gaussian(const MultivariateGaussian& distribution,
                                std::size_t numSamplesToDraw,
                                std::vector<particle_t>& samples);

/**
 * draw_samples_from_cdf draws samples from a distribution represented by a weighted set of samples. The samples are
 * drawn using the low-variance sampling algorithm in ProbRob Table 4.4, pg. 110.
 *
 * \param[in]    cdf                     Distribution represented by samples
 * \param[in]    numSamplesToDraw        Number of samples to be drawn
 * \param[out]   samples                 Sample set in which to put the newly drawn samples
 */
void draw_samples_from_cdf(const std::vector<particle_t>& cdf,
                           std::size_t numSamplesToDraw,
                           std::vector<particle_t>& samples);
...
/**
 * calculate_sample_set_covariance calculates the weighted covariance of the distribution using the normalized sample
 * weights.
 *
 * \param    samples             Samples in the distribution
 ...
 */
Matrix calculate_sample_set_covariance(const std::vector<particle_t>& samples, const Vector& mean);
