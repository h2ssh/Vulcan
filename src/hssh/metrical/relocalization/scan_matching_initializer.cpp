...
void assign_particle_errors_to_weights(ParticleIter begin,
                                       ParticleIter end,
                                       const metric_slam_data_t& data,
...
std::vector<particle_t> ScanMatchingInitializer::generateInitialSamples(const OccupancyGrid& grid,
                                                                        const metric_slam_data_t& data) const
{
    ...
    // If there are more free space samples than the number of region samples, sort in descending order of weight so the
    // best samples are easily found
    ...
    for (std::size_t n = 0, end = std::min(kNumRegionSamples, freeSpaceSamples.size()); n < end; ++n) {
        if (!is_sample_in_existing_region(freeSpaceSamples[n], regionsForBestSamples)) {
            ...
            auto regionSamples = RegionFilterInitializer(region, kNumSamplesPerRegion, kNumPosesPerPosition)
                                   .generateInitialSamples(grid, data);
                                       ...
                                       const OccupancyGrid& grid)
{
    ...
    std::size_t positionsPerThread = numParticles / kNumThreads
    ...
    std::cout << "DEBUG: ScanMatchingInitializer: Calculating weights for " << numParticles << " particles using "
              << kNumThreads << " threads.\n";
#endif
    ...
    for (int n = 0; n < kNumThreads; ++n) {
        ...
        particleFutures.push_back(
          std::async(std::launch::async, ParticleErrorFunc(), begin + start, begin + end, data, grid));
    }
...
double calculate_particle_error(const particle_t& particle, const metric_slam_data_t& data, const OccupancyGrid& grid)
{
    ...
    //
    //     utils::ray_trace_range_t traceRange(wrap_to_pi(data.laser.startAngle + data.laser.offset.theta +
    //     particle.pose.theta),
