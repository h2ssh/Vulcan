 ...
 * indicates a decreased likelihood of occupancy. The endpoints will have a positive value because they indicate
 * occupied space. In the case of overlapping rays, an occupied cell always trumps a free cell. For example, if a cell
 * has been marked as occupied and another ray would mark it free, the cell retains the value associated with an
 * occupied cell.
 *
 ...
 * doesn't work well when traveling down a hallway because the middle of the hallway will remain unmapped. However,
 * mapping the full ray can degrade the map far from the laser. The currently implemented solution is as follows:
 *
 ...
 *
 * By using this approach, the middle of the hallway will be marked free to about the same distance as the furthest
 * valid scan on either side of the hallway.
 *
 * The scan rasterizer can also incorporate covariance-weighting for the rays. The covariance-weighting is described in
 * more detail in hssh/utils/covariance_based_scan_weighter.h. The basic idea is incorporate the robot's pose
 * uncertainty into the update values for each cell. A scan from very sure pose will update the map significantly more
 * than from an unsure pose. Therefore, updating the map is less likely to break during times of poor localization.
 *
 ...
 *
 * The actual values taken correspond to the LPM cost changes. Other builders only need to know the flag, so the above
 * will work for that.
 */
...
public:
     ...
     * \param    initialUpdate       Flag indicating if this is the initial update for the given map, and thus use the
     * initial update changes instead of default \return   Rasterized area for the scan.
     */
...
private:
    ...
    void calculateScanBoundary(const laser::MovingLaserScan& scan, const Point<float>& lpmReferenceFrame);
