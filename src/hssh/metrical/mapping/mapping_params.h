...
struct glass_map_builder_params_t
{
    ...
    bool canSeeCellsFromBothSides;      // have the cells represent 180 degrees, not 360 degrees.
                                        // Implemented such that a hit at theta or theta + 180 results in changing the count for the angle bin
    ...
    double minVisibleOccupiedRange;     // [radians] minimum visible range for a cell to be considered occupied in the flattened map
    double minHighlyVisibleRange;       // [radians] minimum visible range for a cell to be highly visible in the dynamic object filter
    uint16_t minHighlyVisibleIntensity; // [sensor-specific counts] minimum intensity reading for a cell to be considered highly-visible glass
};
