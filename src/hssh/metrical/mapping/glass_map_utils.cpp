...
math::Rectangle<int> active_region_in_flat_map(const GlassMap& map)
{
    ...
    math::Rectangle<int> flatActiveRegion(activeRegion.bottomLeft + offset, activeRegion.topRight + offset);
    flatActiveRegion.bottomLeft.x =
      clamp(flatActiveRegion.bottomLeft.x, 0, static_cast<int>(map.flattenedMap().getWidthInCells()));
    flatActiveRegion.bottomLeft.y =
      clamp(flatActiveRegion.bottomLeft.y, 0, static_cast<int>(map.flattenedMap().getHeightInCells()));
