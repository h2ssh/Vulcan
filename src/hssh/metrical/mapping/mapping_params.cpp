...
glass_map_builder_params_t load_glass_builder_params(const utils::ConfigFile& config)
{
    ...
    params.minVisibleOccupiedRange =
      config.getValueAsDouble(GLASS_BUILDER_HEADING, VISIBLE_OCC_RANGE_KEY) * M_PI / 180.0;
    params.minHighlyVisibleRange =
      config.getValueAsDouble(GLASS_BUILDER_HEADING, HIGH_VISIBLE_OCC_RANGE_KEY) * M_PI / 180.0;
