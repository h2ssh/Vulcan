...
void resetState(Size s)
{
    ...
    flips.create(s);    // the number of times a cell has changed from believed occupied to believed unoccupied(will be
                       // usefull for static occupancy calculation)
    ...
    phimiss =
      sector(s);   // the range of angles from which we have not recently seen this cell and could(should?) have seen it

    // Sets
    active.clear();   // the cells that are actively needing processing this time around, kept updated to corespond to
...
bool isOverlapping(double& mlo, double& mhi, bool& mvalid, double& vlo, double& vhi, bool& vvalid, double u)
{
    ...
    if (C(mlo - vlo) >= C(vhi - vlo) && C(vlo - mlo) >= C(mhi - mlo))
        return false;
...
{
    return isOverlapping(m.lo(0, cell),
                         m.hi(0, cell),
                         m.valid(0, cell),
                         v.lo(0, cell),
                         v.hi(0, cell),
...
// Figures out what cells were transparent(seen through), seen, unscannable/occluded(saw something closer), and
// unsure(caused by quantization error) I use the naive algorithm here, where I simply make elliptic arcs instead of
// proper polylines. Thus, I don't get the unsure regions quite right, and it's probably slow, we'll see.
// Algorithm:
...
void calculateLabels()
{
    ...
    //		ellipse( label, pt, Size(r,r), laser.ang[i]*180/M_PI, -br*180/M_PI*5, br*180/M_PI*5, Scalar(1), -1);//mark the
    //transparent region
    //	}
    //	for(int i=0; i<n;i++){
    //
    ...
    //			r=32767;//really big r, but small enough to safely square
    //		}
    //		ellipse( label, pt, Size(r,r), laser.ang[i]*180/M_PI, -br*180/M_PI, br*180/M_PI, Scalar(2), 2);//mark the
    //unsure region
    //    }
    //	for(int i=0; i<n;i++){
    //
    ...
    //			r=32767;//really big r, but small enough to safely square
    //		}
    //		ellipse( label, pt, Size(r,r), laser.ang[i]*180/M_PI, -br*180/M_PI, br*180/M_PI, Scalar(3), 1);//mark the seen
    //region 		label(laser.y[i],laser.x[i])=4;//debug
    //	}

    ellipse(label, pt, Size(0, 0), 0, 0, 0, Scalar(2), 2);   // clean up the origin(will have the unusable points from
                                                             // NaNs,etc.
}
...
// We take it as obvious that if two sectors overlap then at least one endpoint of one sector lies inside or on the
// boundary of the other sector Note that the formula for "angle a is outside [vlo,vhi]" in a circle is:
...
//
//  (mlo-vlo)%2pi+(mhi-mlo)%2pi = (mhi-mlo+mlo-vlo)%2pi [lemma 2] and (mlo-vlo)%2pi+(mhi-mlo)%2pi>(vhi-vlo)%2pi [lemma
//  3]so:
