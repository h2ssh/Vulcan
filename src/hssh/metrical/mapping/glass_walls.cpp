...
GlassWall::GlassWall(const CellVector& cells, const GlassMap& map) : cells_(cells.size())
{
...
void GlassWall::computeWall(const GlassMap& map)
{
    ...
    std::cout << "Wall with " << cells_.size() << " cells at " << wall_ << " Normal:" << normal_ << " Error:" << error_
              << '\n';
}
...
CellVector bfs_glass_map(cell_t start, const GlassMap& map, CellSet& visited, Compare comp)
{
    ...
    while (!searchQueue.empty()) {
        ...
        if (debug) {
            ...
            std::cout << "Offset:" << glassToFlatOffset << " Glass origin:" << map.getBottomLeft()
                      << " Flat origin:" << flatGrid.getBottomLeft();
        }
        ...
        for (int n = 0; n < 8; ++n) {
            ...
            if (debug) {
                std::cout << "Adj:" << adjacentAngleCell
                          << " Visited:" << (visited.find(adjacentCell) != visited.end());
            }
            ...
            assert(is_cell_in_region(adjacentCell, flatActiveRegion)
...
double distance_between_walls(const GlassWall& lhs, const GlassWall& rhs)
{
    ...
    return std::min(lhsA, std::min(lhsB, std::min(rhsA, rhsB)));
}
