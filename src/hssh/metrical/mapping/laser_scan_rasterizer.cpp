...
max_range_interval_t
  find_max_range_interval(std::size_t startIndex, const laser::MovingLaserScan& scan, float maxDistance);
...
void LaserScanRasterizer::scoreRay(float radius, float angle, const Point<float>& laserCell)
{
    ...
    // is important because it can help avoid small discrepencies in the map from occurring because the summation might
    // hit different cells with the scale of the map Coarsely trace the ray based on the cell scale. This update WILL
    // miss some cells, but they would be less important anyway as the beam only glances by them. Everything until the
    // final discretized position is going to be considered free space
