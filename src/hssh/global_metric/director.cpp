...
, lastTransmissionTime_(0)
{
    ...
    if (commandLine.argumentExists(kMapArg) && commandLine.argumentExists(kInitialRectArg)) {
        ...
        std::cout << "INFO: GlobalMetricDirector: Initializing relocalization in " << mapName << " inside " << mapRect
                  << '\n';
...
void GlobalMetricDirector::handleData(const global_metric_relocalization_request_message_t& message,
...
void GlobalMetricDirector::updateRelocalization(const metric_slam_data_t& data, system::ModuleCommunicator& transmitter)
{
    ...
    if (progress.status == RelocalizationStatus::Success) {
        ...
        std::cerr << "SUCCESS: GlobalMetricDirector: Relocalized in map " << map_.name() << " at "
                  << progress.relocalizedPose << '\n';
